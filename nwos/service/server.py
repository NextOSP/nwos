...
from nwos import api, sql_db
from nwos.modules.registry import Registry
from nwos.release import nt_service_name
from nwos.tools import config, gc, osutil, OrderedSet, profiler
from nwos.tools.cache import log_ormcache_stats
from nwos.tools.misc import stripped_sys_argv, dumpstacks
from nwos.tools.osutil import memory_info
...
def set_limit_memory_hard():
    ...
    import nwos  # for eventd
    if nwos.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(CommonRequestHandler):
    def setup(self):
        ...
        me.name = 'nwos.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The NWOS_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("NWOS_MAX_HTTP_THREADS")
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        import nwos.addons  # noqa: PLC0415
        for path in nwos.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="nwos.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from nwos.addons.base.models.ir_cron import IrCron  # noqa: PLC0415
    ...
    def cron_spawn(self):
        ...
        for i in range(config['max_cron_threads']):
            t = threading.Thread(target=self.cron_thread, args=(i,), name=f"nwos.service.cron.cron{i}")
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from nwos.tests.result import _logger as logger  # noqa: PLC0415
...
class PreforkServer(CommonServer):
    ...
    def start(self):
        ...
        if config['http_enable']:
            ...
            if os.environ.get('NWOS_HTTP_SOCKET_FD'):
                # reload
                self.socket = socket.socket(fileno=int(os.environ.pop('NWOS_HTTP_SOCKET_FD')))
    ...
    def fork_and_reload(self):
        ...
        if pid != 0:
            ...
            os.environ['NWOS_HTTP_SOCKET_FD'] = str(http_socket_fileno)
            os.environ['NWOS_READY_SIGHUP_PID'] = str(pid)
    ...
    def run(self, preload, stop):
        ...
        if os.environ.get('NWOS_READY_SIGHUP_PID'):
            os.kill(int(os.environ.pop('NWOS_READY_SIGHUP_PID')), signal.SIGHUP)
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('nwos: %s %s %s' % (self.__class__.__name__, self.pid, title))
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The NWOS_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("NWOS_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def start(self):
        ...
        if registries_size := os.environ.get('NWOS_REGISTRY_LRU_SIZE_CRON'):
...
def load_server_wide_modules():
    from nwos.modules.module import load_nwos_module  # noqa: PLC0415
    with gc.disabling_gc():
        for m in config['server_wide_modules']:
            try:
                ...
                if m == 'web':
                    msg = """
    The `web` module is provided by the addons found in the `nwos-web` project.
    Maybe you forgot to add those addons in your addons_path configuration."""
                _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)


def _reexec(updated_modules=None):
    """reexecute nwos-server process with (nearly) the same arguments"""
...
def preload_registries(dbnames):
    ...
    registries_size = int(os.environ.get('NWOS_REGISTRY_LRU_SIZE') or 0)
    ...
    for dbname in dbnames:
        if os.environ.get('NWOS_PROFILE_PRELOAD'):
            interval = float(os.environ.get('NWOS_PROFILE_PRELOAD_INTERVAL', '0.1'))
            collectors = [profiler.PeriodicCollector(interval=interval)]
            if os.environ.get('NWOS_PROFILE_PRELOAD_SQL'):
        ...
        try:
            with preload_profiler:
                ...
                if config['test_enable']:
                    from nwos.tests import loader  # noqa: PLC0415
                    t0 = time.time()
...
def start(preload=None, stop=False):
    """ Start the nwos http server and cron processor.
    """
    global server

    load_server_wide_modules()
    import nwos.http  # noqa: PLC0415

    if nwos.evented:
        server = GeventServer(nwos.http.root)
    elif config['workers']:
        if config['test_enable']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(nwos.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which NWOS is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(nwos.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not nwos.evented:
        if inotify:
