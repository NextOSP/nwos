...
from nwos.release import version_info
from nwos.sql_db import db_connect
from nwos.tools import osutil, SQL
from nwos.tools.misc import exec_pg_environ, find_pg_tool
...
def check_db_management_enabled(func, /):
    ...
    def if_db_mgt_enabled(*args, **kwargs):
        if not nwos.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and nwos.tools.config.verify_admin_password(passwd):
        return True
    raise nwos.exceptions.AccessDenied()


# This should be moved to nwos.modules.db, along side initialize().
def _initialize_db(db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        nwos.tools.config['load_language'] = lang

        registry = nwos.modules.registry.Registry.new(db_name, update_module=True, new_db_demo=demo)

        with closing(registry.cursor()) as cr:
            env = nwos.api.Environment(cr, nwos.api.SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = nwos.tools.email_split(login)
                if emails:
...
def _check_faketime_mode(db_name):
    if os.getenv('NWOS_FAKETIME_TEST_MODE') and db_name in nwos.tools.config['db_name']:
        try:
            db = nwos.sql_db.db_connect(db_name)
...
def _create_empty_database(name):
    db = nwos.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = nwos.tools.config['db_template']
    ...
    try:
        db = nwos.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if nwos.tools.config['unaccent']:
                ...
                # But in the case of NWOS, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    nwos.sql_db.close_db(db_original_name)
    db = nwos.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = nwos.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = nwos.api.Environment(cr, nwos.api.SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            nwos.modules.neutralize.neutralize_database(cr)

    from_fs = nwos.tools.config.filestore(db_original_name)
    to_fs = nwos.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    nwos.modules.registry.Registry.delete(db_name)
    nwos.sql_db.close_db(db_name)
    ...
    fs = nwos.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'nwos_dump': '1',
        'db_name': cr.dbname,
        'version': nwos.release.version,
        'version_info': nwos.release.version_info,
        'major_version': nwos.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip', with_filestore=True):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            if with_filestore:
                filestore = nwos.tools.config.filestore(db_name)
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = nwos.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = nwos.api.Environment(cr, nwos.api.SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    nwos.modules.registry.Registry.delete(old_name)
    nwos.sql_db.close_db(old_name)
    ...
    old_fs = nwos.tools.config.filestore(old_name)
    new_fs = nwos.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    nwos.tools.config.set_admin_password(new_password)
    nwos.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        nwos.modules.registry.Registry.new(db, update_module=True, upgrade_modules={'base'})
    return True
...
@nwos.tools.mute_logger('nwos.sql_db')
...
def list_dbs(force=False):
    if not nwos.tools.config['list_db'] and not force:
        raise nwos.exceptions.AccessDenied()

    if not nwos.tools.config['dbfilter'] and nwos.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, NWOS will not
        ...
        return sorted(nwos.tools.config['db_name'])
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of NWOS
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if nwos.tools.sql.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        nwos.sql_db.close_db(database_name)
...
def exp_list_lang():
    return nwos.tools.misc.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(nwos.tools.config.root_path, 'addons/base/data/res_country_data.xml')).getroot()
