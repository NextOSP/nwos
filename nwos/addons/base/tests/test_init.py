...
class TestInit(BaseCase):
    ...
    def nwos_modules_to_test(self):
        import nwos.cli  # noqa: PLC0415
        for path in (*nwos.__path__, *nwos.cli.__path__):
            parent = Path(path)
            for module in parent.iterdir():
                if (module.is_dir() or module.suffix == '.py') and '__' not in module.name:
                    if parent.name == 'nwos':
                        yield f"nwos.{module.stem}"
                    else:
                        yield f"nwos.{parent.name}.{module.stem}"
    ...
    def test_import(self):
        ...
        for module in sorted(self.nwos_modules_to_test()):
