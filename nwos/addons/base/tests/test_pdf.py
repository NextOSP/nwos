...
from nwos.tools import pdf
from nwos.tools.misc import file_open
from nwos.tools.pdf import reshape_text
import io
...
class TestPdf(TransactionCase):
    ...
    def setUp(self):
        ...
        self.minimal_pdf_reader = pdf.NWOSPdfFileReader(self.minimal_reader_buffer)

    def test_nwos_pdf_file_reader(self):
        ...
        r = pdf.NWOSPdfFileReader(io.BytesIO(out.getvalue()))
        self.assertEqual(len(list(r.getAttachments())), 1)

    def test_nwos_pdf_file_writer(self):
        attachments = list(self.minimal_pdf_reader.getAttachments())
        self.assertEqual(len(attachments), 0)
        ...
        for count, (name, data) in enumerate([
            ...
            pdf_writer = pdf.NWOSPdfFileWriter()
    ...
    def test_nwos_pdf_file_reader_with_owner_encryption(self):
        pdf_writer = pdf.NWOSPdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
        ...
        with io.BytesIO(encrypted_content) as reader_buffer:
            pdf_reader = pdf.NWOSPdfFileReader(reader_buffer)
            attachments = list(pdf_reader.getAttachments())

        self.assertEqual(len(attachments), 2)

    def test_nwos_pdf_file_reader_with_nested_attachments(self):
        with file_open('base/tests/multi_kids_embedded.pdf', 'rb') as file:
            pdf_reader = pdf.NWOSPdfFileReader(file, strict=False)
            attachments = list(pdf_reader.getAttachments())
        self.assertEqual(len(attachments), 2)
    ...
    def test_merge_pdf(self):
        ...
        merged_pdf_reader = pdf.NWOSPdfFileReader(merged_reader_buffer)
    ...
    def test_branded_file_writer(self):
        ...
        self.assertEqual(pdf_info['/Producer'], 'NWOS')
        self.assertEqual(pdf_info['/Creator'], 'NWOS')
        reader_buffer.close()
