...
from nwos.http import _request_stack
from nwos.tests import Form, TransactionCase, new_test_user, tagged, HttpCase, users, warmup
...
class TestUsers(UsersCommonCase):
    ...
    @mute_logger('nwos.sql_db', 'nwos.addons.base.models.res_users_deletion')
    ...
    def test_context_get_lang(self):
        ...
        request_patch = patch('nwos.addons.base.models.res_users.request', request)
...
class TestUsers2(UsersCommonCase):
    ...
    @users('user_internal')
    def test_preferences_action_get_targets_current_user(self):
        action = self.env['res.users'].action_get()
        self.assertEqual(action['res_model'], 'res.users')
        self.assertEqual(action['res_id'], self.env.uid)
    ...
    def test_view_group_hierarchy(self):
        ...
        with patch('nwos.addons.base.models.res_groups.ResGroups._get_view_group_hierarchy') as mock:
...
class TestUsersIdentitycheck(HttpCase):
    ...
    def test_revoke_all_devices(self):
        ...
        self.env.user.password = "admin@nwos"

        # Create a first session that will be used to revoke other sessions
        session = self.authenticate('admin', 'admin@nwos', session_extra={'_trace_disable': False})

        # Create a second session that will be used to check it has been revoked
        self.authenticate('admin', 'admin@nwos', session_extra={'_trace_disable': False})
