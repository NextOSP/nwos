...
from nwos import api, fields, models, modules, tools, _
from nwos.exceptions import UserError, AccessError, RedirectWarning, ValidationError
from nwos.fields import Domain
from nwos.service import security
from nwos.http import request, root
from nwos.tools import config, is_html_empty, parse_version, split_every
from nwos.tools.barcode import check_barcode_encoding, createBarcodeDrawing, get_barcode_font
from nwos.tools.misc import find_in_path
from nwos.tools.pdf import PdfFileReader, PdfFileWriter, PdfReadError
from nwos.tools.safe_eval import safe_eval, time
...
def _wkhtml() -> WkhtmlInfo:
    ...
    try:
        ...
        if match:
            ...
            if config['workers'] == 1:
                _logger.info('You need to start NWOS with at least two workers to print a pdf version of the reports.')
                state = 'workers'
        else:
    ...
    try:
        ...
        if match:
            ...
            if config['workers'] == 1:
                _logger.info('You need to start NWOS with at least two workers to convert images to html.')
        else:
...
class IrActionsReport(models.Model):
    ...
    def get_paperformat(self):
        if self.env.context.get('report_paperformat_id'):
            return self.env['report.paperformat'].browse(self.env.context['report_paperformat_id'])
    ...
    def _handle_merge_pdfs_error(self, error=None, error_stream=None):
        raise UserError(_("NWOS is unable to merge the generated PDFs."))
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if error_record_ids:
            ...
            raise RedirectWarning(
                message=_('NWOS is unable to merge the generated PDFs because of %(num_errors)s corrupted file(s)', num_errors=num_errors),
                action=action,
