...
from nwos import api, fields, models, sql_db
from nwos.exceptions import LockError, UserError
from nwos.http import serialize_exception
from nwos.modules import Manifest
from nwos.modules.registry import Registry
from nwos.tools import SQL
from nwos.tools.constants import GC_UNLINK_LIMIT
...
NWOS_NOTIFY_FUNCTION = os.getenv('NWOS_NOTIFY_FUNCTION', 'pg_notify')
...
class IrCron(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['usage'] = 'ir_cron'
        if os.getenv('NWOS_NOTIFY_CRON_CHANGES'):
    ...
    def _process_jobs(db_name: str) -> None:
        ...
        try:
            ...
            # The table ir_cron does not exist; this is probably not an NWOS database.
    ...
    def _check_modules_state(cr, jobs):
        ...
        # reset_module_states.
        from nwos.modules.loading import reset_modules_state  # noqa: PLC0415
    ...
    def write(self, vals):
        ...
        if ('nextcall' in vals or vals.get('active')) and os.getenv('NWOS_NOTIFY_CRON_CHANGES'):
            self.env.cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list: list[datetime]):
        ...
        if min(at_list) <= now or os.getenv('NWOS_NOTIFY_CRON_CHANGES'):
            self.env.cr.postcommit.add(self._notifydb)
        return triggers
    ...
    def _notifydb(self):
        ...
        The NWOS_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        IrCron modification and on trigger creation (regardless of call_at)
        """
        with sql_db.db_connect('postgres').cursor() as cr:
            cr.execute(SQL("SELECT %s('cron_trigger', %s)", SQL.identifier(NWOS_NOTIFY_FUNCTION), self.env.cr.dbname))
