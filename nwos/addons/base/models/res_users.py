...
from nwos import api, fields, models, tools, _
from nwos.api import SUPERUSER_ID
from nwos.exceptions import AccessDenied, AccessError, UserError, ValidationError
from nwos.fields import Command, Domain
from nwos.http import request, DEFAULT_LANG
from nwos.tools import email_domain_extract, is_html_empty, frozendict, reset_cached_properties, str2bool, SQL
...
class ResUsers(models.Model):
    """ User class. A res.users record models an NWOS user and is different
        from an employee.
    ...
    def _check_credentials(self, credential, env):
        ...
        * catch :class:`~nwos.exceptions.AccessDenied` and perform their
          own checking
        * (re)raise :class:`~nwos.exceptions.AccessDenied` if the
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by NWOS (updates, module installation, ...)'))
    ...
    @api.model
    def action_get(self):
        action = self.sudo().env.ref('base.action_res_users_my').read()[0]
        action['res_id'] = self.env.uid
        return action
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: nwos.exceptions.AccessDenied when old password is wrong
        :raise: nwos.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this NWOS portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your NWOS is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "NWOS in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.nwos.com/documentation/latest/administration/install/deploy.html#https for details.",
                    source
                )
...
def _check_apikey_credentials(cr, *, scope, key, table='res_users_apikeys'):
    ...
    :param nwos.sql_db.BaseCursor cr: database cursor
