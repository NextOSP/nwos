...
from nwos.tools import OrderedSet, misc, profiler
from nwos.tools.constants import SCRIPT_EXTENSIONS, STYLE_EXTENSIONS
from nwos.tools.json import scriptsafe as json
from nwos.tools.misc import file_open, file_path
...
class AssetsBundle(object):
            ...
            if self.templates:
                ...
                template_bundle = textwrap.dedent(f"""
                    ...
                    *******************************************/
                    ...
                    nwos.define("{self.name}.bundle.xml", ["@web/core/templates"], function(require) {{
                        "use strict";
    ...
    def js_with_sourcemap(self, template_bundle=None):
        ...
        """
        from nwos.tools.sourcemap_generator import SourceMapGenerator  # noqa: PLC0415
        ...
        * 't-inherit-mode':  'primary' or 'extension'.
        ...
        for asset in self.templates:
            ...
            try:
                ...
                template = content if content.startswith('<nwos>') else f'<templates>{asset.content}</templates>'
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            if '.' not in ref and line not in imports and not ref.startswith(('.', '/', '~')):
                imports.append(line)
                return line
            msg = "Local import '%s' is forbidden for security reasons. Please remove all @import {your_file} imports in your custom files. In NWOS you have to import all files in the assets, and not through the @import statement." % ref
            _logger.warning(msg)
...
class JavascriptAsset(WebAsset):
    ...
    def is_transpiled(self):
        if self._is_transpiled is None:
            from nwos.tools.js_transpiler import is_nwos_module  # noqa: PLC0415
            self._is_transpiled = bool(is_nwos_module(self.url, super().content))
    ...
    def content(self):
        ...
        if self.is_transpiled:
            if not self._converted_content:
                from nwos.tools.js_transpiler import transpile_javascript  # noqa: PLC0415
