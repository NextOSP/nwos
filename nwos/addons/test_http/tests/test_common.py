...
import nwos
from nwos.http import Session
from nwos.addons.base.tests.common import HttpCaseWithUserDemo
from nwos.tools import config, reset_cached_properties
from nwos.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(nwos.http.Application, 'session_store', session_store)
        cls.classPatch(nwos.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(nwos.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        nwos.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('nwos.http.db_list') as db_list, \
             patch('nwos.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('nwos.http.db_list') as db_list, \
             patch('nwos.http.db_filter') as db_filter, \
             patch('nwos.http.Registry') as Registry:
