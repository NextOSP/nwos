...
from nwos.tools.which import which

from . import _nwos_checker_sql_injection
...
class TestGetTextLint(TestPylintChecks):
    def check(self, testtext):
        return super().check(testtext, "_nwos_checker_gettext", "gettext-placeholders")
...
class TestSqlLint(TestPylintChecks):
    def check(self, testtext):
        return super().check(testtext, "_nwos_checker_sql_injection", "sql-injection")
    ...
    def test_sql_injection_detection(self):
        ...
        checker = _nwos_checker_sql_injection.NWOSBaseChecker(self.linter)
...
class TestI18nChecks(TestPylintChecks):
    def check(self, test_content):
        return super().check(
            test_content, "_nwos_checker_gettext", "missing-gettext,gettext-variable,gettext-placeholders,gettext-repr"
        )
