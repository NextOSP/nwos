...
r"""\
NWOS HTTP layer / WSGI application
...
:class:`~nwos.http.Controller`: class and its companion the
:func:`~nwos.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an NWOS http request. The NWOS http request is
...
import nwos.init  # import first for core setup
...
NWOS URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.nwos.com/documentation/master/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through NWOS via py-QWeb form, embed a CSRF
...
<!-- Alternatively, use the X-NWOS-Database header. -->
"""
...
SESSION_ROTATION_INTERVAL_HEADER_SKIP = "X-NWOS-Skip-Session-Rotation-Interval"
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~nwos.service.db.list_dbs`:
    ...
    """
    try:
        dbs = nwos.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, NWOS will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': nwos.service.common.dispatch,
        'db': nwos.service.db.dispatch,
        'object': nwos.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~nwos.tools.file_path`
        :param filter_ext: See :func:`~nwos.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`nwos.tools._vendor.send_file.send_file` instead of
...
def _get_addon_name(cls):
    path = cls.__module__.split('.')
    if len(path) >= 3 and path[1] == 'addons' and path[0] in ('nwos', 'nwos'):
        return path[2]
    return ''
...
class Controller:
    ...
    :class:`~nwos.http.Controller` can use the :func:`~nwos.http.route`:
    ...
    :func:`~nwos.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(nwos.http.Controller):
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~nwos.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        module = _get_addon_name(cls)
        return bool(module and module in modules)
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are NWOS addons.
        """
        # Controllers defined outside of nwos addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of nwos addons can be extended in
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~nwos.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> nwos_ip = socket.gethostbyname('nwos.com')
        >>> GeoIP(nwos_ip).country.iso_code
        'FR'
    """
...
class _Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~nwos.http.Response`.
        :rtype: Response
...
class Response(Proxy):
    ...
    # nwos.http._response attributes
    load = ProxyFunc()
...
class Request:
    ...
    def _get_session_and_dbname(self):
        ...
        header_dbname = self.httprequest.headers.get('X-NWOS-Database')
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~nwos.addons.base.models.res_users.ResUsers>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~nwos.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = nwos.api.Environment(cr, self.session.uid, self.session.context)
            try:
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~nwos.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "NWOS Session Expired"
...
class Application:
    ...
    def initialize(self):
        ...
        from nwos.service.server import load_server_wide_modules  # noqa: PLC0415
    ...
    def session_store(self):
        path = nwos.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if nwos.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/nwos/')
                            or httprequest.path.startswith('/nwos/')
                            or httprequest.path in (
                                '/nwos', '/nwos', '/web', '/web/login', '/test_http/ensure_db',
                            )):
