...
import nwos.upgrade
from nwos.modules.module import load_script
from nwos.orm.registry import Registry
...
VERSION_RE = re.compile(
    r"""^
        # Optional prefix with NWOS version
        ((
            6\.1|
        ...
        # After NWOS version we allow precisely 2 or 3 parts
...
class MigrationManager:
    ...
    def _get_files(self) -> None:
        def _get_upgrade_path(pkg: str) -> Iterator[str]:
            for path in nwos.upgrade.__path__:
    ...
    def migrate_module(self, pkg: module_graph.ModuleNode, stage: typing.Literal['pre', 'post', 'end']) -> None:
        ...
        def compare(version: str) -> bool:
            ...
            if majorless_version:
                # We should not re-execute major-less scripts when upgrading to new NWOS version
...
def exec_script(cr, installed_version, pyfile, addon, stage, version=None):
    ...
    if ext.lower() != '.py':
        return
    try:
        mod = load_script(pyfile, f"nwos.upgrade.{addon}.{version}.{name}")
