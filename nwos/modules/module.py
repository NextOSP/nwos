...
class UpgradeHook:
    """Makes the legacy `migrations` package being `nwos.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^nwos\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("nwos.addons.base.maintenance.migrations", "nwos.upgrade")
...
def initialize_sys_path() -> None:
    """
    Setup the addons path ``nwos.addons.__path__`` with various defaults
    ...
    for path in (
        ...
        *tools.config['addons_path'],
        ...
        if os.access(path, os.R_OK) and path not in nwos.addons.__path__:
            nwos.addons.__path__.append(path)

    # hook nwos.upgrade on upgrade-path
    ...
    for up in tools.config['upgrade_path'] or [legacy_upgrade_path]:
        if up not in nwos.upgrade.__path__:
            nwos.upgrade.__path__.append(up)

    # create decrecated module alias from nwos.addons.base.maintenance.migrations to nwos.upgrade
    spec = importlib.machinery.ModuleSpec("nwos.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = nwos.upgrade  # type: ignore
    sys.modules["nwos.addons.base.maintenance"] = maintenance_pkg
    sys.modules["nwos.addons.base.maintenance.migrations"] = nwos.upgrade

    # hook for upgrades and namespace freeze
    if not getattr(initialize_sys_path, 'called', False):  # only initialize once
        nwos.addons.__path__._path_finder = lambda *a: None  # prevent path invalidation
        nwos.upgrade.__path__._path_finder = lambda *a: None  # prevent path invalidation
...
class Manifest(Mapping[str, typing.Any]):
    ...
    def _get_manifest_from_addons(module: str) -> Manifest | None:
        ...
        for adp in nwos.addons.__path__:
...
def get_resource_from_path(path: str) -> tuple[str, str, str] | None:
    ...
    """
    resource = None
    sorted_paths = sorted(nwos.addons.__path__, key=len, reverse=True)
...
def load_nwos_module(module_name: str) -> None:
    """ Load an NWOS module, if not already loaded.
    ...
    qualname = f'nwos.addons.{module_name}'
