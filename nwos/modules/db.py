...
""" Initialize the database for module management and NWOS installation. """
...
if typing.TYPE_CHECKING:
    from nwos.sql_db import BaseCursor, Cursor
...
def is_initialized(cr: Cursor) -> bool:
    ...
    """
    return nwos.tools.sql.table_exists(cr, 'ir_module_module')
...
def initialize(cr: Cursor) -> None:
    ...
    """
    try:
        f = nwos.tools.misc.file_path('base/data/base_data.sql')
    except FileNotFoundError:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise OSError(m)

    with nwos.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for info in nwos.modules.Manifest.all_addon_manifests():
    ...
    from nwos.tools import config  # noqa: PLC0415
...
def has_unaccent(cr: BaseCursor) -> FunctionStatus:
    ...
    module but any similar function will be picked by NWOS.
...
def has_trigram(cr: BaseCursor) -> bool:
    ...
    pg_trgm module but any similar function will be picked by NWOS.
