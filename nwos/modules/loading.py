...
import nwos.sql_db
import nwos.tools.sql
import nwos.tools.translate
from nwos import api, tools
from nwos.tools import OrderedSet
from nwos.tools.convert import convert_file, IdRef, ConvertMode as LoadMode

from . import db as modules_db
from .migration import MigrationManager
from .module import adapt_version, initialize_sys_path, load_nwos_module
from .module_graph import ModuleGraph
from .registry import Registry

if typing.TYPE_CHECKING:
    from collections.abc import Collection, Iterable
    from nwos.api import Environment
...
def load_data(env: Environment, idref: IdRef, mode: LoadMode, kind: LoadKind, package: ModuleNode) -> bool:
    ...
    for k in keys:
        if k == 'init_xml' and package.manifest[k]:
            _logger.warning("module %s: key 'init_xml' is deprecated in NWOS 19.", package.name)
...
def load_module_graph(
    ...
    report: NWOSTestResult | None = None,
    ...
    assert isinstance(env.cr, nwos.sql_db.Cursor), "Need for a real Cursor to load modules"
    ...
    t0 = time.time()
    loading_extra_query_count = nwos.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = nwos.sql_db.sql_counter
        ...
        load_nwos_module(package.name)

        if update_operation == 'install':
            py_module = sys.modules['nwos.addons.%s' % (module_name,)]
        ...
        if tools.config['test_enable'] and (update_operation or not update_from_config):
            ...
            if suite.countTestCases():
                ...
                # Python tests
                tests_t0, tests_q0 = time.time(), nwos.sql_db.sql_counter
                ...
                test_queries = nwos.sql_db.sql_counter - tests_q0

                # tests may have reset the environment
                module = env['ir.module.module'].browse(module_id)


        extra_queries = nwos.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   nwos.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
    ...
    with registry.cursor() as cr:
        ...
        if update_module and upgrade_modules:
            for pyfile in tools.config['pre_upgrade_scripts']:
                nwos.modules.migration.exec_script(cr, graph['base'].installed_version, pyfile, 'base', 'pre')
        ...
        if update_module and tools.sql.table_exists(cr, 'ir_model_fields'):
            ...
            if nwos.tools.sql.column_exists(cr, 'ir_model_fields', 'company_dependent'):
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['nwos.addons.%s' % (pkg.name,)]
...
def reset_modules_state(db_name: str) -> None:
    ...
    with db.cursor() as cr:
        if not nwos.tools.sql.table_exists(cr, 'ir_module_module'):
