...
"""
The PostgreSQL connector is a connectivity layer between the NWOS code and
...
if typing.TYPE_CHECKING:
    ...
    from nwos.orm.environments import Transaction
...
MAX_IDLE_TIMEOUT = int(os.getenv("NWOS_DB_MAX_IDLE_TIMEOUT", "600"))
...
class Cursor(BaseCursor):
    """Represents an open transaction to the PostgreSQL DB backend,
       ...
       ``cursor`` objects.
        ...
        such as NWOS, we need the best guarantees that no data
        ...
        NWOS implements its own level of locking protection
        ...
        the default transaction isolation level for NWOS cursors, as
    ...
    def __init__(self, pool: ConnectionPool, dbname: str, dsn: dict):
        ...
        if os.getenv('NWOS_FAKETIME_TEST_MODE') and self.dbname in tools.config['db_name']:
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # NWOS Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def connection_info_for(db_or_uri: str, readonly=False) -> tuple[str, dict]:
    ...
    if 'NWOS_PGAPPNAME' in os.environ:
        warnings.warn("Since 19.0, use PGAPPNAME instead of NWOS_PGAPPNAME", DeprecationWarning)
        app_name = os.environ['NWOS_PGAPPNAME']
...
def db_connect(to: str, allow_uri=False, readonly=False) -> Connection:
    ...
    maxconn = (tools.config['db_maxconn_gevent'] if hasattr(nwos, 'evented') and nwos.evented else 0) or tools.config['db_maxconn']
...
def close_db(db_name: str) -> None:
    """ You might want to call nwos.modules.registry.Registry.delete(db_name) along this function."""
    if _Pool:
