# -*- coding: utf-8 -*-
"""
The module :mod:`nwos.tests.common` provides unittest test cases and a few
...
import nwos.addons.base
import nwos.cli
import nwos.http
import nwos.models
import nwos.orm.registry
from nwos import api
from nwos.exceptions import AccessError
from nwos.fields import Command
from nwos.modules.registry import Registry, DummyRLock
from nwos.service import security
from nwos.sql_db import Cursor, Savepoint
from nwos.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from nwos.tools.mail import single_email_re
from nwos.tools.misc import find_in_path
from nwos.tools.xml_utils import _validate_xml
from nwos.addons.base.models import ir_actions_report
...
if nwos.cli.COMMAND in ('server', 'start') and not config['test_enable']:
    _logger.error(
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: nwos.tests.common.Form is deprecated, use nwos.tests.Form",
...
# The nwos library is supposed already configured.
HOST = '127.0.0.1'
...
def release_test_lock():
    ...
    try:
        ...
        if not _registry_test_lock.acquire(timeout=60):
            tag = nwos.modules.module.current_test.canonical_tag
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding NWOS module name.
    """
    def register(func):
        # register func by nwos module name
        if func.__module__.startswith('nwos.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most nwos operations;
    """
    if not login:
...
class BaseCase(case.TestCase):
    """ Subclass of TestCase for NWOS-specific code. This class is abstract and
    ...
    def __init_subclass__(cls):
        ...
        if cls.__module__.startswith('nwos.addons.'):
            if getattr(cls, 'test_tags', None) is None:
                cls.test_tags = {'standard', 'at_install'}
            cls.test_module = cls.__module__.split('.')[2]

    longMessage = True      # more verbose error message by default: https://www.nwos.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~nwos.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(nwos.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            nwos.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = nwos.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('nwos.sql_db.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/nwos/addons/" in filename:
                        filename = filename.rsplit("/nwos/addons/", 1)[1]
                    if count > expected:
        ...
        for record in records:
            ...
            for field_name in field_names:
                ...
                match record._fields[field_name]:
                    case nwos.fields.Many2one():
                        record_value = record_value.id
                    case nwos.fields.One2many() | nwos.fields.Many2many():
                        record_value = sorted(record_value.ids)
                    case nwos.fields.Float() as field if digits := field.get_digits(record.env):
                        record_value = Approx(record_value, digits[1], decorate=False)
                    case nwos.fields.Monetary() as field if currency_field_name := field.get_currency_field(record):
    ...
    def assertCanOpenTestCursor(self):
        ...
        if nwos.modules.module.current_test is not self:
            message = f"Trying to open a test cursor for {self.canonical_tag} while already in a test {nwos.modules.module.current_test.canonical_tag}"
            _logger.runbot(message)
            raise BadRequest(message)
        request = nwos.http.request
    ...
    def get_method_additional_tags(self, test_method):
        ...
        """
        additional_tags = []
        if nwos.tools.config['test_tags'] and 'is_query_count' in nwos.tools.config['test_tags']:
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | nwos.addons.base.models.res_currency.ResCurrency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    """
    muted_registry_logger = mute_logger(nwos.orm.registry._logger.name)
    freeze_time = None
    ...
    def setUpClass(cls):
        ...
        cls._crypt_context_patcher = patch('nwos.addons.base.models.res_users.ResUsersPatchedInTest._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f", loglevel=logging.RUNBOT, directory=''):
    ...
    dest = pathlib.Path(nwos.tools.config['screenshots']) / get_db_name() / directory
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        if scs := nwos.tools.config['screencasts']:
    ...
    def _chrome_start(
        ...
        switches = {
            ...
            '--enable-logging': '',
            '--v': str(int(os.environ.get("NWOS_BROWSER_LOG_VERBOSITY", "0"))),
            '--no-first-run': '',
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way NWOS widgets are created they all appear as Class(...)
...
def _find_executable():
    browser_bin_path = os.environ.get('NWOS_BROWSER_BIN')  # used for testing specific Chrome builds
...
class HttpCase(TransactionCase):
    ...
    touch_enabled = False
    session: nwos.http.Session = None
    ...
    @classmethod
    def http_port(cls):
        if nwos.service.server.server is None:
            return None
        return nwos.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('nwos.service.http.request.')]
        ...
        if request_threads:
            ...
            nwos.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        nwos.http.root.session_store.save(self.session)

    def authenticate(self, user, password, *,
        browser: ChromeBrowser = None, session_extra: dict | None = None):
        if getattr(self, 'session', None):
            nwos.http.root.session_store.delete(self.session)

        self.session = session = nwos.http.root.session_store.new()
        session.update(
            nwos.http.get_default_session(),
            db=get_db_name(),
        ...
        if user: # if authenticated
            ...
            with patch('nwos.addons.base.models.res_users.ResUsersPatchedInTest._check_credentials', new=patched_check_credentials):
        ...
        nwos.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, cookies=None, error_checker=None, watch=False, success_signal=DEFAULT_SUCCESS_SIGNAL, debug=False, cpu_throttling=None, **kw):
        ...
        with self.allow_requests(browser=browser), contextlib.ExitStack() as atexit:
            ...
            if "bus.bus" in self.env.registry:
                from nwos.addons.bus.websocket import CloseCode, _kick_all, WebsocketConnectionHandler  # noqa: PLC0415
                from nwos.addons.bus.models.bus import BusBus  # noqa: PLC0415
            ...
            cpu_throttling_os = os.environ.get('NWOS_BROWSER_CPU_THROTTLING')  # used by dedicated runbot builds
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        options = {
            ...
            'startUrl': url_path,
            'delayToCheckUndeterminisms': kwargs.pop('delay_to_check_undeterminisms', int(os.getenv("NWOS_TOUR_DELAY_TO_CHECK_UNDETERMINISMS", "0")) or 0),
        }
        code = kwargs.pop('code', f"nwos.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"nwos.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('nwos.http.Request._get_profiler_context_manager', route_profiler))
    ...
    def get_method_additional_tags(self, test_method):
        ...
        if nwos.tools.config['test_tags'] and 'is_tour' in nwos.tools.config['test_tags']:
...
def tagged(*tags):
    ...
    By default, all Test classes from nwos.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in NWOS test suites
