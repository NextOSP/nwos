...
from .result import NWOSTestResult

_logger = logging.getLogger(__name__)

TEST_MODULE_NAME_PATTERN = re.compile(r'^nwos\.addons\.\w+\.tests')


def run_tests(env, test_tags, modules=None, reload_tests=False):
    """Run tests for the given modules and test tags."""

    if nwos.cli.COMMAND != 'shell':
        _logger.error('run_tests should be used only in nwos shell')
        return

    if nwos.tools.config['workers'] != 0:
        _logger.error('run_tests should be used only in threaded mode')
        return

    from nwos.service.server import server  # noqa: PLC0415
    if not server.httpd:
    ...
    nwos.tools.config['test_tags'] = test_tags
    nwos.tools.config['test_enable'] = True
    report = _run_tests(env.cr.dbname, modules)
    nwos.tools.config['test_enable'] = None
    nwos.tools.config['test_tags'] = None
