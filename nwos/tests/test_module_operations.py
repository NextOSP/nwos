...
from nwos.tools import config, topological_sort, unique, profiler
from nwos.modules.registry import Registry
from nwos.netsvc import init_logger
from nwos.tests import standalone_tests
import nwos.tests.loader

_logger = logging.getLogger('nwos.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of NWOS modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=addons_path,
        help="Comma-separated list of paths to directories containing extra NWOS modules")
...
def test_standalone(args):
    ...
    nwos.service.db._check_faketime_mode(args.database)  # noqa: SLF001
    ...
    for module_name in registry._init_modules:
        ...
        nwos.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    if args.addons_path:
        ...
        nwos.modules.module.initialize_sys_path()
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'nwos.modules.loading': {'level': 'CRITICAL'},
            'nwos.sql_db': {'level': 'CRITICAL'},
            'nwos.models.unlink': {'level': 'WARNING'},
            'nwos.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
