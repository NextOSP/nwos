...
from nwos.tools.func import locked, reset_cached_properties
from nwos.tools.lru import LRU
from nwos.tools.misc import Collector, format_frame
...
if typing.TYPE_CHECKING:
    ...
    from nwos.fields import Field
    from nwos.models import BaseModel
    from nwos.sql_db import BaseCursor, Connection, Cursor
    from nwos.modules import module_graph


_logger = logging.getLogger('nwos.registry')
_schema = logging.getLogger('nwos.schema')
...
class Registry(Mapping[str, type["BaseModel"]]):
        ...
        *,
        ...
        try:
            ...
            with registry.cursor() as cr:
                ...
                from nwos.modules import db  # noqa: PLC0415
                if db.is_initialized(cr):
                    cr.execute("DELETE FROM ir_config_parameter WHERE key='base.partially_updated_database'")
                    if cr.rowcount:
                        update_module = True
            # This should be a method on Registry
            from nwos.modules.loading import load_modules, reset_modules_state  # noqa: PLC0415
    ...
    def init(self, db_name: str) -> None:
        ...
        if config['test_enable']:
            from nwos.tests.result import NWOSTestResult  # noqa: PLC0415
            self._assertion_report: NWOSTestResult | None = NWOSTestResult()
        else:
        ...
        for indexname, tablename, field in expected:
            ...
            if will_index:
                ...
                if index == 'trigram':
                    ...
                    from nwos.modules.db import FunctionStatus  # noqa: PLC0415
