...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "nwos.netsvc.log is deprecated starting NWOS 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self, log_db=None):
        ...
        if self._log_db != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('nwos.sql_db'), sql_db.db_connect(self._log_db, allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('nwos.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='nwos.tools.image')
    ...
    if conf:
        ...
        if not conf.get('keep_nwos_default', False):
            return
    ...
    # Check that handler.stream has a fileno() method: when running NWOS
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("NWOS_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'nwos.http.rpc.request:INFO',
    'nwos.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['nwos:DEBUG', 'nwos.sql_db:INFO', 'nwos.http.rpc:DEBUG'],
    'debug_rpc': ['nwos:DEBUG', 'nwos.sql_db:INFO', 'nwos.http.rpc.request:DEBUG'],
    'debug': ['nwos:DEBUG', 'nwos.sql_db:INFO'],
    'debug_sql': ['nwos.sql_db:DEBUG'],
    'info': [],
    'runbot': ['nwos:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['nwos:WARNING', 'werkzeug:WARNING'],
    'error': ['nwos:ERROR', 'werkzeug:ERROR'],
    'critical': ['nwos:CRITICAL', 'werkzeug:CRITICAL'],
}
...
def showwarning_with_traceback(message, category, filename, lineno, file=None, line=None):
    ...
    for frame in traceback.extract_stack():
        if frame.name == '__call__' and frame.filename.endswith('/nwos/http.py'):
