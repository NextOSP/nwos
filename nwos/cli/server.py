...
"""
NWOS - Server
NWOS is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - NWOS SA
"""
...
from nwos.release import author as __author__  # noqa: F401
from nwos.release import version as __version__  # noqa: F401
from nwos.service import server
from nwos.tools import config

from . import Command

# Also use the `nwos` logger for the main script.
_logger = logging.getLogger('nwos')
...
def report_configuration():
    ...
    import nwos.release  # noqa: PLC0415
    _logger.info("NWOS version %s", nwos.release.version)
    if os.path.isfile(config['config']):
        _logger.info("Using configuration file at %s", config['config'])
    _logger.info('addons paths: %s', nwos.addons.__path__)
    ...
    if sys.version_info[:2] > nwos.release.MAX_PY_VERSION:
        _logger.warning("Python %s is not officially supported, please use Python %s instead",
            '.'.join(map(str, sys.version_info[:2])),
            '.'.join(map(str, nwos.release.MAX_PY_VERSION))
        )
...
def setup_pid_file():
    ...
    This function assumes the configuration has been initialized.
    """
    import nwos  # for evented  # noqa: PLC0415
    if not nwos.evented and config['pidfile']:
        pid = os.getpid()
...
def main(args):
    ...
    for db_name in config['db_name']:
        from nwos.service import db  # noqa: PLC0415
        try:
...
class Server(Command):
    """Start the nwos server (default command)"""
