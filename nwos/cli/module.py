...
from nwos import SUPERUSER_ID
from nwos.api import Environment
from nwos.cli.command import Command
from nwos.modules.loading import force_demo
from nwos.modules.module import get_module_path, initialize_sys_path
from nwos.modules.registry import Registry
from nwos.tools import OrderedSet, config, parse_version
...
class Module(Command):
    ...
    def __init__(self, *args, **kwargs):
        ...
        install_parser.epilog = textwrap.dedent("""\
            Before installing modules, an NWOS database needs to be created and initialized
            on your PostgreSQL instance, using the `db init` command:

            $ nwos-bin db init <db_name>

            To get help on its parameters, see:

            $ nwos-bin db init --help
        """)
