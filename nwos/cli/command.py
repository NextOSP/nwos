...
import nwos.cli
from nwos.modules import initialize_sys_path, load_script
...
def load_internal_commands():
    """ Load ``commands`` from ``nwos.cli`` """
    for path in nwos.cli.__path__:
        for module in Path(path).iterdir():
            if module.suffix != '.py':
                continue
            __import__(f'nwos.cli.{module.stem}')
    ...
    for path in nwos.addons.__path__:
        for fullpath in Path(path).glob(f'*/cli/{command}.py'):
            if (found_command := fullpath.stem) and Command.is_valid_name(found_command):
                # loading as nwos.cli and not nwos.addons.{module}.cli
                # so it doesn't load nwos.addons.{module}.__init__
                mapping[f'nwos.cli.{found_command}'] = fullpath 
...
def find_command(name: str) -> Command | None:
    ...
    # import from nwos.cli
    with contextlib.suppress(ImportError):
        __import__(f'nwos.cli.{name}')
        return commands[name]

    # import from nwos.addons.*.cli
