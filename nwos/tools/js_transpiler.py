"""
This code is what let us use ES6-style modules in nwos.
Classic NWOS modules are composed of a top-level :samp:`nwos.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@nwos-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** nwos-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual nwos one with minimal changes.
...
from nwos.tools.misc import OrderedSet


def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom nwos modules.
    ...
    legacy_nwos_define = get_aliased_nwos_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_nwos_define, module_path, dependencies),
        partial(convert_t, url)
    ]
    for s in steps:
        content = s(content)
    if legacy_nwos_define:
        content += legacy_nwos_define
    return content
...
def url_to_module_path(url):
    """
    NWOS modules each have a name. (nwos.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_nwos_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the nwos.define call.
    ...
    """
    return f"""nwos.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
NWOS_MODULE_RE = re.compile(r"""
    \s*                                # starting white space
    \/(\*|\/)                          # /* or //
    .*                                 # any comment in between (optional)
    @(?:nwos|nwos)-module           # '@nwos-module' or '@nwos-module' statement
...
def is_nwos_module(url, content):
    """
    Detect if the file is a native nwos module.
    We look for a comment containing @nwos-module.

    :param url:
    :param content: source code
    :return: is this a nwos module that need transpilation ?
    """
    result = NWOS_MODULE_RE.match(content)
...
def get_aliased_nwos_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the nwos source base being:
    ...
    > nwos.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @nwos-module */
        // after
        /** @nwos-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @nwos-module */
        // after
        /** @nwos-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = NWOS_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\nnwos.define(`%s`, ['%s'], function (require) {
                        return require('%s');
