...
def _get_eval_context(self, env, model_str):
    from nwos import fields, release  # noqa: PLC0415
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval := node.get('eval'):
            ...
            try:
                ...
                logging.getLogger('nwos.tools.convert.init').error(
        ...
        from nwos.models import BaseModel  # noqa: PLC0415
...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        from nwos.fields import Command  # noqa: PLC0415
        groups = []
    ...
    def _tag_root(self, el):
        for rec in el:
            ...
            try:
                ...
                raise ParseError(msg) from None  # Restart with "--log-handler nwos.tools.convert:DEBUG" for complete traceback
    ...
    def parse(self, de):
        assert de.tag in self.DATA_ROOTS, "Root xml tag must be <nwos>, <nwos>, <nwos> or <data>."
        self._tag_root(de)
    DATA_ROOTS = ['nwos', 'nwos', 'data', 'nwos']
...
def convert_file(
    ...
    if kind is not None:
        warnings.warn(
            "The `kind` argument is deprecated in NWOS 19.",
            DeprecationWarning,
            stacklevel=2,
        )
    if pathname is None:
