...
from nwos.tools.func import classproperty
from . import appdirs
...
class _NWOSOption(optparse.Option):
    ...
    def __init__(self, *opts, **attrs):
        ...
        if env_name is None and is_new_option and self.file_loadable:
            ...
            self.env_name = 'NWOS_' + self.dest.upper()
...
class _FileOnlyOption(_NWOSOption):
...
class _PosixOnlyOption(_NWOSOption):
    def __init__(self, *opts, **attrs):
        if os.name != 'posix':
...
class configmanager:
    def __init__(self):
        ...
        # dictionary mapping option destination (keys in self.options) to NWOSOptions.
        self.options_index = {}
    ...
    @property
    def rcfile(self):
        self._warn("Since 19.0, use nwos.tools.config['config'] instead", DeprecationWarning, stacklevel=2)
        return self['config']

    @rcfile.setter
    def rcfile(self, rcfile):
        self._warn(f"Since 19.0, use nwos.tools.config['config'] = {rcfile!r} instead", DeprecationWarning, stacklevel=2)
        self._runtime_options['config'] = rcfile

    def _build_cli(self):
        NWOSOption = type('NWOSOption', (_NWOSOption,), {'config': self})
        FileOnlyOption = type('FileOnlyOption', (_FileOnlyOption, NWOSOption), {})
        PosixOnlyOption = type('PosixOnlyOption', (_PosixOnlyOption, NWOSOption), {})

        version = "%s %s" % (release.description, release.version)
        parser = optparse.OptionParser(version=version, option_class=NWOSOption)
        ...
        parser.add_option(FileOnlyOption(dest='publisher_warranty_url', my_default='http://services.nwos.com/publisher-warranty/', file_exportable=False))
        ...
        group.add_option("-c", "--config", dest="config", type='path', file_loadable=False, env_name='NWOS_RC',
                         help="specify alternate config file")
        group.add_option("-s", "--save", action="store_true", dest="save", my_default=False, file_loadable=False,
                         help="save configuration to ~/.nwosrc (or to ~/.nwos_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", type='path',  # sensitive default set in _load_default_options
                         help="Directory where to store NWOS data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'nwos_tests')
        ...
        group.add_option('--log-handler', action="append", type='comma', my_default=[':INFO'], metavar="MODULE:LEVEL",
                         help='setup a handler at LEVEL for a given MODULE. An empty MODULE indicates the root logger. '
                              'This option can be repeated. Example: "nwos.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const=("nwos.http:DEBUG",),
                         help='shortcut for --log-handler=nwos.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const=("nwos.sql_db:DEBUG",),
                         help='shortcut for --log-handler=nwos.sql_db:DEBUG')
        ...
        group.add_option("--db_app_name", dest="db_app_name", my_default="nwos-{pid}", env_name='PGAPPNAME',
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate NWOS to another language. "
        ...
        group.add_option('--dev', dest='dev_mode', type='comma', metavar="FEATURE,...", my_default=[], file_exportable=False, env_name='NWOS_DEV',
    ...
    def _load_default_options(self):
        ...
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'nwos.conf')
        elif os.path.isfile(rcfilepath := os.path.expanduser('~/.nwosrc')):
            pass
        elif os.path.isfile(rcfilepath := os.path.expanduser('~/.nwos_serverrc')):
            self._warn("Since ages ago, the ~/.nwos_serverrc file has been replaced by ~/.nwosrc", DeprecationWarning)
        else:
            rcfilepath = '~/.nwosrc'
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        This method initializes nwos.tools.config and nwos.conf (the
        ...
        Typical usage of this method:
            ...
            nwos.tools.config.parse_config(sys.argv[1:])
        """
        from nwos import modules, netsvc  # noqa: PLC0415
        ...
        if setup_logging is not False:
            ...
            if setup_logging is None:
                warnings.warn(
                    "As of NWOS 18, it's recommended to specify whether"
                    " you want NWOS to setup its own logging (or want to"
                    " handle it yourself)",
    ...
    def _load_env_options(self):
        ...
        if environ.get('NWOS_SERVER'):
            self._warn("Since ages ago, the NWOS_SERVER environment variable has been replaced by NWOS_RC", DeprecationWarning)

    def _load_cli_options(self, opt):
        # nwos.cli.command.main parses the config twice, the second time
