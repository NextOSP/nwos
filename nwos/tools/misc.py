...
"""
Miscellaneous tools used by NWOS.
"""
...
from nwos.loglevels import exception_to_unicode, get_encodings, ustr  # noqa: F401
...
if typing.TYPE_CHECKING:
    ...
    from nwos.addons.base.models.res_lang import LangData
...
def file_path(file_path: str, filter_ext: tuple[str, ...] = ('',), env: Environment | None = None, *, check_exists: bool = True) -> str:
    ...
    if not is_abs and (module := sys.modules.get(f'nwos.addons.{file_path_split[0]}')):
        ...
        addons_paths = [*nwos.addons.__path__, root_path, *temporary_paths]
...
def file_open(name: str, mode: str = "r", filter_ext: tuple[str, ...] = (), env: Environment | None = None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/nwos/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env: Environment):
    ...
    Examples::
        ...
        >>> with nwos.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with nwos.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
def flatten(list):
    ...
    Author: Christophe Simonis (christophe@nwos.com)
...
def scan_languages() -> list[tuple[str, str]]:
    """ Returns all languages supported by NWOS for translation
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('nwos.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('nwos.foo.bar'):
            do_suff()
    """
...
def format_date(
    ...
    if not value:
        return ''
    from nwos.fields import Datetime  # noqa: PLC0415
...
def limited_field_access_token(record, field_name, timestamp=None, *, scope):
    ...
    :type record: class:`nwos.models.Model`
