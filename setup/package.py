...
version_info = ...
nt_service_name = ...
exec(open(os.path.join(ROOTDIR, 'nwos', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/nwos && \
    groupadd -g %(group_id)s nwos && \
    useradd -u %(user_id)s -g nwos nwos -d /var/lib/nwos && \
    mkdir /data && \
    chown nwos:nwos /var/lib/nwos /data
USER nwos
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class NWOSTestTimeoutError(Exception):
    pass


class NWOSTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=7073, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise NWOSTestError("Installation of package failed")
        else:
        ...
        raise NWOSTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/nwos_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.nwos_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'nwos/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'nwos-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
        ...
        self.docker_template = Path(docker_templates[self.arch]).read_text(encoding='utf-8').replace('USER nwos', DOCKERUSER)
    ...
    def run(self, cmd, build_dir, container_name, user='nwos', exposed_port=None, detach=False, timeout=None):
    ...
    def test_nwos(self):
        logging.info('Starting to test NWOS install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'nwos.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise NWOSTestTimeoutError('NWOS pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise NWOSTestError('Error while installing/starting NWOS after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'nwos-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/nwos-*.tar.gz' % self.args.build_dir)[0], '%s/nwos_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/nwos-*.zip' % self.args.build_dir)[0], '%s/nwos_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su postgres -s /bin/bash -c "createuser -s nwos"',
            'su nwos -s /bin/bash -c "python3 -m venv /var/lib/nwos/nwosvenv"',
            'su nwos -s /bin/bash -c "/var/lib/nwos/nwosvenv/bin/python3 -m pip install --upgrade pip"',
            'su nwos -s /bin/bash -c "/var/lib/nwos/nwosvenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su nwos -s /bin/bash -c "/var/lib/nwos/nwosvenv/bin/python3 -m pip install /data/src/nwos_{VERSION}.{TSTAMP}.tar.gz"',
            'su nwos -s /bin/bash -c "createdb mycompany"',
            'su nwos -s /bin/bash -c "/var/lib/nwos/nwosvenv/bin/nwos -d mycompany -i base --stop-after-init"',
            'su nwos -s /bin/bash -c "/var/lib/nwos/nwosvenv/bin/nwos -d mycompany --pidfile=/data/src/nwos.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'nwos-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=7073, timeout=300)
        self.test_nwos()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/nwos (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../nwos_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'nwos-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f'/usr/bin/apt-get install -y /data/src/nwos_{VERSION}.{TSTAMP}_all.deb',
            'su nwos -s /bin/bash -c "nwos -d mycompany -i base --pidfile=/data/src/nwos.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'nwos-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=7073, timeout=300)
        self.test_nwos()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/nwos/rpmbuild'
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/nwos.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./nwos-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/nwos-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" /data/src/setup/rpm/nwos.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/nwos*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'nwos-rpm-build-{TSTAMP}')
        os.rename(glob('%s/dist/nwos-*.noarch.rpm' % self.args.build_dir)[0], '%s/nwos_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'sleep 5',
            'su postgres -c "createuser -s nwos"',
            'su nwos -c "createdb mycompany"',
            'dnf install -d 0 -e 0 /data/src/nwos_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su nwos -s /bin/bash -c "nwos -c /etc/nwos/nwos.conf -d mycompany -i base --stop-after-init"',
            'su nwos -s /bin/bash -c "nwos -c /etc/nwos/nwos.conf -d mycompany --pidfile=/data/src/nwos.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'nwos-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=7073, timeout=300)
        self.test_nwos()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'nwos-rpm-createrepo-%s' % TSTAMP)
...
class DockerWine(Docker):
    ...
    def build(self):
        ...
        container_python = '/var/lib/nwos/.wine/drive_c/nwosbuild/WinPy64/python-3.12.3.amd64/python.exe'
        ...
        cmds = [
            ...
            rf'wine "c:\nsis-3.11\makensis.exe" {nsis_args} "c:\nwosbuild\server\setup\win32\setup.nsi"',
            rf'wine {container_python} -m pip list'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'nwos-win-build-%s' % TSTAMP)
