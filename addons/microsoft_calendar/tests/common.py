...
from nwos.tests.common import HttpCase
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        m = mute_logger('nwos.addons.auth_signup.models.res_users')
        ...
        # simple event values to create a NWOS event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for NWOS events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to NWOS events
        # -----------------------------------------------------------------------------------------

        self.expected_nwos_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_nwos_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_nwos_recurrency_events_from_outlook = [
            {
    ...
    def sync_nwos_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in NWOS. A trick for deactivating the checking
    ...
    def assert_nwos_event(self, nwos_event, expected_values):
        """
        Assert that an NWOS event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        nwos_event_values = nwos_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(nwos_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, nwos_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_nwos_recurrence(self, nwos_recurrence, expected_values):
        """
        Assert that an NWOS recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        nwos_recurrence_values = nwos_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, nwos_recurrence_values.get(k), msg=f"'{k}' mismatch")
