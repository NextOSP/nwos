...
from nwos.addons.microsoft_calendar.tests.common import TestCommon


class TestSyncNWOS2MicrosoftMail(TestCommon, MailCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.users = []
    ...
    def test_change_organizer_pure_nwos_event(self):
        """
        Test that changing organizer on a pure NWOS event (not synced with Microsoft)
        ...
        self.assertTrue(event.active, "Pure NWOS event should not be archived when changing organizer")
