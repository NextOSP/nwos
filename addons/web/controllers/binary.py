...
try:
    ...
    from nwos.tools._vendor.send_file import send_file

import nwos
import nwos.modules.registry
from nwos import SUPERUSER_ID, _, http, api
from nwos.addons.base.models.assetsbundle import ANY_UNIQUE
from nwos.exceptions import AccessError, UserError
from nwos.http import request, Response
from nwos.tools import file_open, file_path, replace_exceptions, str2bool
from nwos.tools.image import image_guess_size_from_field_name
from nwos.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
NWOS is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however NWOS is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if nwos.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=nwos.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or nwos.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                _logger.warning("While retrieving the company logo, using the NWOS logo instead", exc_info=True)
