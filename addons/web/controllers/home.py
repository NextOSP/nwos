...
from nwos.tools.misc import hmac
...
class Home(http.Controller):
    ...
    def index(self, s_action=None, db=None, **kw):
        ...
        return request.redirect_query(BACKEND_WEBCLIENT_URL_PREFIX, query=request.params)
    ...
    @http.route([
        '/web',
        BACKEND_WEBCLIENT_URL_PREFIX,
        f'{BACKEND_WEBCLIENT_URL_PREFIX}/<path:subpath>',
        LEGACY_BACKEND_WEBCLIENT_URL_PREFIX,
        f'{LEGACY_BACKEND_WEBCLIENT_URL_PREFIX}/<path:subpath>',
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if request.httprequest.method == 'POST':
            ...
            except nwos.exceptions.AccessDenied as e:
                if e.args == nwos.exceptions.AccessDenied().args:
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = nwos.SUPERUSER_ID
    ...
    def health(self, db_server_status=False):
        ...
        if db_server_status:
            try:
                nwos.sql_db.db_connect('postgres').cursor().close()
