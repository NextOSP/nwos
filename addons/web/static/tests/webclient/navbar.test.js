import { beforeEach, destroy, expect, test } from "@nwos/hoot";
import { queryAll, queryAllAttributes, queryAllTexts, resize } from "@nwos/hoot-dom";
import { advanceTime, animationFrame, runAllTimers } from "@nwos/hoot-mock";
import {
    clearRegistry,
    contains,
    defineMenus,
...
import { Component, onRendered, xml } from "@nwos/owl";
...
test("href attribute on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item.o_app").toHaveAttribute("href", "/nwos/action-339");
});
...
test("href attribute with path on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item.o_app").toHaveAttribute("href", "/nwos/my-path");
});

test.tags("desktop");
test("apps menu groups app switcher items into sections", async () => {
    defineMenus([
        {
            id: 1,
            name: "Dashboards",
        ...
        { id: 2, name: "CRM", actionID: 340, xmlid: "crm.crm_menu_root" },
        {
            id: 3,
            name: "Attendances",
            actionID: 341,
        ...
        { id: 5, name: "Custom App", actionID: 343, xmlid: "custom.menu_root" },
    ]);

    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();

    expect(queryAllTexts(".o_app_switcher_section_title")).toEqual([
        "Overview",
        "Sales",
        "Human Resources",
        "Operations",
    ...
    expect(queryAllTexts(".o-dropdown--menu a.o_app")).toEqual([
        "Dashboards",
        "CRM",
        "Attendances",
        "Stock Requests",
...
test("data-menu-xmlid attribute on AppsMenu items", async () => {
    ...
    // check apps
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(queryAllAttributes(".o-dropdown--menu a.o_app", "data-menu-xmlid")).toEqual(["wowl", null], {
        message:
...
test("navbar can display current active app", async () => {
    ...
    // Open apps menu
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item.o_app:not(.focus)").toHaveCount(1, {
        message:
    ...
    getService("menu").setCurrentMenu(1);
    await animationFrame();
    expect(".o-dropdown--menu .dropdown-item.o_app.focus").toHaveCount(1, {
