...
import { click, queryAll } from "@nwos/hoot-dom";
...
test("Burger Menu on an App", async () => {
    ...
    await contains(".o_burger_menu_content li.o_app:contains(App1)", { root: document.body }).click();
...
test("App sidebar shows company switcher for multi-company users", async () => {
    serverState.companies = [
        { id: 1, name: "Company A", parent_id: false, child_ids: [] },
        { id: 2, name: "Company B", parent_id: false, child_ids: [] },
    ];
    ...
    expect(
        queryAll(".o_app_menu_sidebar .o_burger_menu_companies", { root: document.body })
    ).toHaveCount(1);
    expect(
        queryAll(".o_app_menu_sidebar .o_switch_company_item", { root: document.body })
    ).toHaveCount(2);
});
...
test("Burger menu closes when click on menu item", async () => {
    ...
    await contains(".o_burger_menu_app li.o_app:contains(App1)", { root: document.body }).click();
