...
import { queryOne, scroll, waitFor } from "@nwos/hoot-dom";
import { animationFrame, Deferred } from "@nwos/hoot-mock";
import { Component, onWillStart, xml } from "@nwos/owl";
import {
    contains,
    defineActions,
    defineMenus,
...
test("action domain can evaluate allowed_company_ids when user context is incomplete", async () => {
    ...
    expect(getService("action").currentController.action.domain).toEqual([["id", "in", [1]]]);
});
...
test("retrieving a stored action should remove 'allowed_company_ids' from its context (action)", async () => {
    ...
    // Prepare the URL hash to make sure the stored action will get executed.
    // Object.assign(browser.location, { search: "?model=partner&view_type=kanban" });
    redirect("/nwos/action-1?view_type=kanban");
