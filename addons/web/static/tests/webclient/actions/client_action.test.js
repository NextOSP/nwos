...
import { animationFrame, runAllTimers } from "@nwos/hoot-mock";
...
test("test reload client action", async () => {
    ...
    await runAllTimers();
    expect.verifySteps([
        "replaceState /nwos?test=42",
        "window_reload",
        "pushState /nwos/action-2",
        "window_reload",
        "pushState /nwos?menu_id=1",
        "window_reload",
        "pushState /nwos/action-1?menu_id=2",
        "window_reload",
    ]);
});
...
test("test display_exception client action", async () => {
    ...
    getService("action").doAction({
        ...
        params: {
            ...
            data: {
                name: `nwos.exceptions.UserError`,
                debug: "traceback",
                arguments: [],
                context: {},
