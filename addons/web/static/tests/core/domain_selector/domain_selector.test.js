import { expect, test, getFixture } from "@nwos/hoot";
import { press, queryAll, queryAllAttributes, queryAllTexts, queryOne } from "@nwos/hoot-dom";
import { animationFrame, mockDate, mockTimeZone, runAllTimers } from "@nwos/hoot-mock";
...
test("show current user name without avatar for uid expressions", async () => {
    ...
    expect(".o_tag:not(.o_avatar)").toHaveText(user.name);
    expect(".o_tag:not(.o_avatar) img").toHaveCount(0);
    await contains(SELECTORS.debugArea).edit(`[("user_id", "=", uid)]`);
    expect(".o_record_selector input").toHaveValue(user.name);
    expect(".o_record_selector img").toHaveCount(0);
});

test("show current user name for uid expressions in readonly descriptions", async () => {
    class Users extends models.Model {
        _name = "res.users";
        name = fields.Char();

        _records = [{ id: user.userId, name: user.name }];
    }
    defineModels([Users]);
    Partner._fields.user_id = fields.Many2one({ relation: "res.users" });
    await makeDomainSelector({
    ...
    expect(".o_tree_editor_condition").toHaveText(`User = ${user.name}`);
});
