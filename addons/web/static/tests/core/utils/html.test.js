...
import { htmlEscape, markup } from "@nwos/owl";
...
test("nwosmark", () => {
    expect(nwosmark("").toString()).toBe("");
    expect(nwosmark("**test**").toString()).toBe("<b>test</b>");
    expect(nwosmark("**test** something else **test**").toString()).toBe(
        "<b>test</b> something else <b>test</b>"
    );
    expect(nwosmark("--test--").toString()).toBe(`<span class="text-muted">test</span>`);
    expect(nwosmark("--test-- something else --test--").toString()).toBe(
        `<span class="text-muted">test</span> something else <span class="text-muted">test</span>`
    );
    expect(nwosmark("`test`").toString()).toBe(
    ...
    expect(nwosmark("test\ttest2").toString()).toBe(
        `test<span style="margin-left: 2em"></span>test2`
    );
    expect(nwosmark("test\ntest2").toString()).toBe("test<br>test2");
    expect(nwosmark("<p>**test**</p>").toString()).toBe("&lt;p&gt;<b>test</b>&lt;/p&gt;");
    expect(nwosmark(markup`<p>**test**</p>`).toString()).toBe("<p><b>test</b></p>");
});
