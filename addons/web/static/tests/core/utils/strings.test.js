...
test("escape", () => {
    ...
    expect(escape(`<a href="https://www.nwos.com">nwos<a>`)).toBe(
        `&lt;a href=&quot;https://www.nwos.com&quot;&gt;nwos&lt;a&gt;`
    );
    expect(escape(`<a href='https://www.nwos.com'>nwos<a>`)).toBe(
        `&lt;a href=&#x27;https://www.nwos.com&#x27;&gt;nwos&lt;a&gt;`
    );
    expect(escape("<a href='https://www.nwos.com'>NWOS`s website<a>")).toBe(
        `&lt;a href=&#x27;https://www.nwos.com&#x27;&gt;NWOS&#x60;s website&lt;a&gt;`
    );
});
...
test("escapeRegExp", () => {
    ...
    expect(escapeRegExp("[wowl]")).toBe("\\[wowl\\]");
    expect(escapeRegExp("[wowl.nwos]")).toBe("\\[wowl\\.nwos\\]");
    expect(escapeRegExp("^nwos.define([.]*)$")).toBe("\\^nwos\\.define\\(\\[\\.\\]\\*\\)\\$");
    expect(escapeRegExp("[.*+?^${}()|[]\\")).toBe("\\[\\.\\*\\+\\?\\^\\$\\{\\}\\(\\)\\|\\[\\]\\\\");
});
...
test("isEmail", () => {
    ...
    expect(isEmail("test@nwos")).toBe(false);
    expect(isEmail("test@nwos@nwos.com")).toBe(false);
    expect(isEmail("te st@nwos.com")).toBe(false);

    expect(isEmail("test@nwos.com")).toBe(true);
});
