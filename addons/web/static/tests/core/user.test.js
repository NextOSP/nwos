import { test, expect, describe } from "@nwos/hoot";
...
test("missing user context defaults to an empty context", () => {
    const userWithoutContext = _makeUser({ uid: 7, user_context: null });

    expect(userWithoutContext.context).toEqual({ uid: 7 });
    expect(userWithoutContext.lang).toBe("en-US");

    userWithoutContext.updateContext({ calendar_filters: { all: true, user: false } });
    expect(userWithoutContext.context).toEqual({
        calendar_filters: { all: true, user: false },
        uid: 7,
    });
});

test("user lang follows context updates", () => {
    const userWithContext = _makeUser({ uid: 7, user_context: { lang: "en_US" } });

    expect(userWithContext.lang).toBe("en-US");

    userWithContext.updateContext({ lang: "vi_VN" });
    expect(userWithContext.lang).toBe("vi-VN");
});
