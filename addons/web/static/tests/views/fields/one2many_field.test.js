...
import { Deferred, animationFrame, mockTimeZone, runAllTimers } from "@nwos/hoot-mock";

import { onWillDestroy, onWillStart, reactive, useState } from "@nwos/owl";
...
test("display correct value after validation error", async () => {
    ...
    function validationHandler(env, error, originalError) {
        if (originalError.data.name === "nwos.exceptions.ValidationError") {
            return true;
        }
    }
...
test("one2many form view with action button", async () => {
    ...
    // See https://github.com/nwos/nwos/issues/24189
    mockService("action", {
        doActionButton(params) {
