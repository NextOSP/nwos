import { expect, getFixture, test } from "@nwos/hoot";
import {
    contains,
    defineModels,
    fieldInput,
...
import { queryAllTexts, queryFirst } from "@nwos/hoot-dom";
...
test("in form view", async () => {
    Contact._records = [{ id: 1, email: "john.doe@nwos.com" }];
    await mountView({
        type: "form",
        resModel: "contact",
        resId: 1,
    ...
    expect(`.o_field_email input[type="email"]`).toHaveValue("john.doe@nwos.com");
    expect(`.o_field_email a`).toHaveCount(1);
    expect(`.o_field_email a`).toHaveAttribute("href", "mailto:john.doe@nwos.com");
    expect(`.o_field_email a`).toHaveAttribute("target", "_blank");
    await fieldInput("email").edit("new@nwos.com");
    expect(`.o_field_email input[type="email"]`).toHaveValue("new@nwos.com");
});

test("in editable list view", async () => {
    Contact._records = [
        { id: 1, email: "john.doe@nwos.com" },
        { id: 2, email: "jane.doe@nwos.com" },
    ];
    await mountView({
        type: "list",
        resModel: "contact",
    ...
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:john.doe@nwos.com");
    ...
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:new@nwos.com");
});
