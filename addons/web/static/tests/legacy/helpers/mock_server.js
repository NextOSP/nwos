/** @nwos-module alias=@web/../tests/helpers/mock_server default=false */
...
export function makeServerError({
    ...
    return makeErrorFromResponse({
        code: code || 0,
        message: message || "NWOS Server Error",
        data: {
            name: errorName || `nwos.exceptions.${type || "UserError"}`,
            debug: "traceback",
            arguments: args || [],
            context: context || {},
            subType,
    ...
    mockLoadAction({ action_id } = {}) {
        ...
        if (!action) {
            throw makeServerError({
                errorName: "nwos.addons.web.controllers.action.MissingActionError",
