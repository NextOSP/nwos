...
export function _makeUser(session) {
    ...
    function updateActiveCompanies(cids, allowedCompanies, defaultCompanyId) {
        activeCompanies = [];
        cids.forEach((cid) => {
            const company = allowedCompanies.find((c) => c.id === cid);
            if (company) {
                activeCompanies.push(company);
            }
        });
        if (activeCompanies.length === 0 && defaultCompanyId) {
    ...
    if (userCompanies) {
        ...
        defaultCompany =
            allowedCompanies.find((c) => c.id === userCompanies.current_company) ||
            allowedCompanies[0]; // TODO: change the name in the session current_company to default_company
        if (defaultCompany) {
    ...
    return {
        ...
        get lang() {
            return pyToJsLocale(context?.lang) || "en-US";
        },
        get tz() {
        ...
        async activateCompanies(
            ...
            const fallbackCompany = this.activeCompany;
