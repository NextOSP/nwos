...
import { Component, useRef, useState, markup } from "@nwos/owl";
...
export const nwosExceptionTitleMap = new Map(
    Object.entries({
        "nwos.addons.base.models.ir_mail_server.MailDeliveryException": _t("MailDeliveryException"),
        "nwos.exceptions.AccessDenied": _t("Access Denied"),
        "nwos.exceptions.MissingError": _t("Missing Record"),
        "nwos.addons.web.controllers.action.MissingActionError": _t("Missing Action"),
        "nwos.addons.base.models.ir_actions.ServerActionWithWarningsError": _t("Invalid Operation"),
        "nwos.exceptions.UserError": _t("Invalid Operation"),
        "nwos.exceptions.ValidationError": _t("Validation Error"),
        "nwos.exceptions.AccessError": _t("Access Error"),
        "nwos.exceptions.Warning": _t("Warning"),
    })
);
...
ClientErrorDialog.title = _t("NWOS Client Error");

// -----------------------------------------------------------------------------
// Network Error Dialog
// -----------------------------------------------------------------------------
export class NetworkErrorDialog extends ErrorDialog {}
NetworkErrorDialog.title = _t("NWOS Network Error");
...
export class RPCErrorDialog extends ErrorDialog {
    ...
    inferTitle() {
        ...
        if (this.props.exceptionName && nwosExceptionTitleMap.has(this.props.exceptionName)) {
            this.title = nwosExceptionTitleMap.get(this.props.exceptionName).toString();
            return;
        }
        ...
        switch (this.props.type) {
            case "server":
                this.title = _t("NWOS Server Error");
                break;
            case "script":
                this.title = _t("NWOS Client Error");
                break;
            case "network":
                this.title = _t("NWOS Network Error");
                break;
        }
    }
...
export class WarningDialog extends Component {
    ...
    inferTitle() {
        if (this.props.exceptionName && nwosExceptionTitleMap.has(this.props.exceptionName)) {
            return nwosExceptionTitleMap.get(this.props.exceptionName).toString();
        }
        return this.props.title || _t("NWOS Warning");
    }
}
...
export class RedirectWarningDialog extends Component {
    ...
    setup() {
        ...
        this.title = capitalize(subType) || _t("NWOS Warning");
        this.message = message;
...
registry
    ...
    .add("nwos.exceptions.AccessDenied", WarningDialog)
    .add("nwos.exceptions.AccessError", WarningDialog)
    .add("nwos.exceptions.MissingError", WarningDialog)
    .add("nwos.addons.web.controllers.action.MissingActionError", WarningDialog)
    .add("nwos.addons.base.models.ir_actions.ServerActionWithWarningsError", WarningDialog)
    .add("nwos.exceptions.UserError", WarningDialog)
    .add("nwos.exceptions.ValidationError", WarningDialog)
    .add("nwos.exceptions.RedirectWarning", RedirectWarningDialog)
    .add("nwos.http.SessionExpiredException", SessionExpiredDialog)
