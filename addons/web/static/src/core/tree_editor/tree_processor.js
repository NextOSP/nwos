...
function isCurrentUserExpression(val, fieldDef) {
    return (
        getResModel(fieldDef) === "res.users" &&
        (val instanceof Expression ? val.toString() : val) === "uid"
    );
}
...
function formatValue(val, disambiguate, fieldDef, displayNames) {
    if (isCurrentUserExpression(val, fieldDef)) {
        return user.name || _t("Current user");
    }
