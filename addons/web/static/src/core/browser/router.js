...
function isScopedApp() {
    return browser.location.href.includes(`/${SCOPED_APP_URL_PREFIX}`) && isDisplayStandalone();
}

function isBackendPath(pathname) {
    return BACKEND_URL_PREFIXES.some(
        (prefix) => pathname === `/${prefix}` || pathname.startsWith(`/${prefix}/`)
    );
}

function isLegacyBackendPath(pathname) {
    return (
        pathname === `/${LEGACY_BACKEND_URL_PREFIX}` ||
        pathname.startsWith(`/${LEGACY_BACKEND_URL_PREFIX}/`)
    );
}
...
export function startUrl() {
    return isScopedApp() ? SCOPED_APP_URL_PREFIX : BACKEND_URL_PREFIX;
}
...
function urlToState(urlObj) {
    ...
    if (WEBCLIENT_URL_PREFIXES.includes(prefix)) {
        ...
        if (prefix === SCOPED_APP_URL_PREFIX && !isDisplayStandalone()) {
            // make sure /scoped_app are redirected to the backend webclient when using the browser instead of the PWA
            const url = browser.location.origin + router.stateToUrl(state);
            urlObj.href = url;
        } else if (prefix === LEGACY_BACKEND_URL_PREFIX) {
            const url = browser.location.origin + router.stateToUrl(state);
            urlObj.href = `${url}${hash}`;
        }
    }
    return state;
}
...
export function startRouter() {
    ...
    // ** url-retrocompatibility **
    if (browser.location.pathname === "/web" || isLegacyBackendPath(browser.location.pathname)) {
 ...
 * external website). Unfortunately, NWOS wasn't designed to be compatible with this cache, which
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            isBackendPath(browser.location.pathname) &&
            (url.pathname === "/web" || isBackendPath(url.pathname)) &&
            a.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (isBackendPath(url.pathname) && url.hash) {
