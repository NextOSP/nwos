...
const cacheName = "nwos-sw-cache-v3";
const homepageURL = "/nwos";
        ...
        Promise.all([
            caches
                ...
                .then((keys) =>
                    Promise.all(
                        keys
                            .filter((key) => key.startsWith("nwos-sw-cache") && key !== cacheName)
...
const extractSessionInfo = (htmlContent) => {
    const match = htmlContent.match(/nwos\.__session_info__\s*=\s*({.*?});/s);
...
const storeDataOnCache = async (url, response) => {
    ...
    const currentSessionInfo = extractSessionInfo(htmlBody);
    ...
    return cache.put(
        ...
        new Response(
            currentSessionInfo
                ? htmlBody.replace(currentSessionInfo, "@@@session_info_secret@@@")
                : htmlBody,
            {
...
const readDataOnCache = async (url) => {
    ...
    // if you come from /nwos to project the url is now /nwos/project, but it doesn't exist in cache so use /nwos instead
    if (!response) {
        return readDataOnCache(homepageURL);
    }
    const htmlBody = await getTextFromResponse(response);
    if (htmlBody.includes("@@@session_info_secret@@@") && !sessionInfo) {
        return null;
    }
...
const serveShareTarget = (event) => {
    ...
    event.respondWith(Response.redirect("/nwos?share_target=trigger"));
    event.waitUntil(
        (async () => {
            // The page sends this message to tell the service worker it's ready to receive the file.
            await waitingMessage("nwos_share_target");
            const client = await self.clients.get(event.resultingClientId || event.clientId);
            const data = await event.request.formData();
            client.postMessage({
                shared_files: data.getAll("externalMedia") || [],
                action: "nwos_share_target_ack",
            });
        })()
    );
};
