...
(function (nwos) {
    ...
    class ModuleLoader {
        /** @type {NWOSModuleLoader["bus"]} */
        bus = new EventTarget();
        /** @type {NWOSModuleLoader["checkErrorProm"]} */
        checkErrorProm = null;
        /** @type {NWOSModuleLoader["factories"]} */
        factories = new Map();
        /** @type {NWOSModuleLoader["failed"]} */
        failed = new Set();
        /** @type {NWOSModuleLoader["jobs"]} */
        jobs = new Set();
        /** @type {NWOSModuleLoader["modules"]} */
        modules = new Map();
        ...
        /** @type {NWOSModuleLoader["addJob"]} */
        addJob(name) {
            this.jobs.add(name);
            this.startModules();
        }

        /** @type {NWOSModuleLoader["define"]} */
        define(name, deps, factory, lazy = false) {
            ...
            this.factories.set(name, {
                deps,
                fn: factory,
                ignoreMissingDeps: globalThis.__nwosIgnoreMissingDependencies,
            });
            if (!lazy) {
                this.addJob(name);
        ...
        /** @type {NWOSModuleLoader["findErrors"]} */
        ...
        /** @type {NWOSModuleLoader["reportErrors"]} */
        ...
        /** @type {NWOSModuleLoader["startModules"]} */
        startModules() {
            let job;
        ...
        /** @type {NWOSModuleLoader["startModule"]} */
        startModule(name) {
            /** @type {(dependency: string) => NWOSModule} */
            const require = (dependency) => this.modules.get(dependency);
            this.jobs.delete(name);
            const factory = this.factories.get(name);
            /** @type {NWOSModule | null} */
            let module = null;
            try {
    ...
    nwos.debug ||= "";
    nwos.define = loader.define.bind(loader);
    nwos.loader = loader;

    if (nwos.debug && !loader.debug) {
