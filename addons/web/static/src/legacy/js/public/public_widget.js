...
function NWOSEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
NWOSEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
NWOSEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * NWOS where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new NWOSEvent(this, name, info);
