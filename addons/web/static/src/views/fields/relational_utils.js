...
import { highlightText, nwosmark } from "@web/core/utils/html";
...
export class Many2XAutocomplete extends Component {
    ...
    onQuickCreateError(error, request) {
        if (
            ...
            error.exceptionName === "nwos.exceptions.ValidationError"
        ) {
    ...
    buildRecordSuggestion(request, record) {
        ...
        return {
            ...
            label: label
                ? highlightText(request, nwosmark(label), "text-primary fw-bold")
                : _t("Unnamed"),
