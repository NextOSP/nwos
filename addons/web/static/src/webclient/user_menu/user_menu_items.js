import { Component, markup } from "@nwos/owl";
...
function documentationItem(env) {
    const documentationURL = "https://www.nwos.com/documentation/17.0";
    return {
        type: "item",
        id: "documentation",
...
export function preferencesItem(env) {
    return {
        ...
        callback: async function () {
            ...
            // `action_get` already returns res_id set to the current user; only override
            // it when we actually have a userId, otherwise a falsy value would open the
            // form in creation mode and saving would fail on the required `login` field.
            actionDescription.res_id = user.userId || actionDescription.res_id;
...
export function nwosAccountItem(env) {
    return {
        type: "item",
        id: "account",
        description: _t("My NWOS.com account"),
        callback: () => {
            rpc("/web/session/account")
                ...
                .catch(() => {
                    browser.open("https://accounts.nwos.com/account", "_blank");
                });
        },
        sequence: 60,
    };
...
registry
    ...
    .add("nwos_account", nwosAccountItem)
