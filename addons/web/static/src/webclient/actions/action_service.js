...
function makeActionEvalContext(actionContext = {}) {
    const evalContext = Object.assign({}, user.context, actionContext);
    if (!Array.isArray(evalContext.allowed_company_ids)) {
        evalContext.allowed_company_ids = getActiveCompanyIds();
    }
    return evalContext;
}
...
function isMissingActionError(error) {
    return error.exceptionName === "nwos.addons.web.controllers.action.MissingActionError";
}

function isMissingModelForState(error, state) {
    return (
        error.exceptionName === "werkzeug.exceptions.NotFound" &&
        error.code === 404 &&
        state.model &&
...
export function makeActionManager(env, router = _router) {
        ...
        if (currentController) {
            if (currentController.virtual) {
                try {
                    ...
                    if (
                        error.exceptionName ===
                        "nwos.addons.web.controllers.action.MissingActionError"
                    ) {
                        action = null;
                    } else {
                        throw error;
    ...
    function _preprocessAction(action, context = {}) {
        ...
        action.domain =
            typeof domain === "string"
                ? evaluateExpr(domain, makeActionEvalContext(action.context))
                : domain;
        if (action.help) {
    ...
    async function _executeActWindowAction(action, options) {
        ...
        for (const [, type] of action.views) {
            ...
            const viewInfo = session.view_info || {};
            if (viewInfo[type]) {
                const { icon, display_name, multi_record: multiRecord } = viewInfo[type];
    ...
    async function loadState(state = router.current) {
        ...
        if (actionParams) {
            ...
            try {
                ...
                if (isMissingActionError(error) || isMissingModelForState(error, state)) {
                    browser.sessionStorage.removeItem("current_action");
