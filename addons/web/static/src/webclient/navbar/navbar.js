...
export class NavBar extends Component {
    ...
    get appSections() {
        return getAppSections(this.menuService.getApps());
    }
    ...
    async openAllApps() {
        await this.actionService.doAction("apps_menu", { clearBreadcrumbs: true });
    }

    getMenuItemHref(payload) {
        return `/nwos/${payload.actionPath || "action-" + payload.actionID}`;
    }

    getAppIconClass(app) {
        const [iconClass, iconValue] = (app.webIcon || "").split(",");
        ...
        return /\b(fa|oi)\b/.test(iconClass) ? iconClass : `fa ${iconClass}`;
    }
    ...
    getAppIconStyle(app) {
        const [, color, backgroundColor] = (app.webIcon || "").split(",");
        if (!backgroundColor) {
            return "";
        }
        return `background-color: ${backgroundColor}; color: ${color || "#fff"};`;
    }
