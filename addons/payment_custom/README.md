...
- `16.1`
  - The default payment instructions message of Wire Transfer can be recomputed at any time after
    installation of the module. nwos/nwos#103903
- `16.0`
  - The `custom_mode` field is added to distinguish custom payment modes from other payment
    ...
    nwos/nwos#99400
  - The module is no longer automatically installed with the `payment` module. nwos/nwos#99400
  - The module is renamed from `payment_transfer` to `payment_custom`. nwos/nwos#99400
...
createdb nwos
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -i payment_custom
```
...
```bash
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -u payment_custom
```
