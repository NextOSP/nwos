...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        if it_moves := moves.filtered(lambda m: 'it_edi_send' in moves_data[m]['extra_edis']):
            ...
            # Invite the user to authorize NWOS and start using IT EDI in production mode
            if 'prod' not in it_moves.mapped('l10n_it_edi_proxy_mode'):
                alerts['l10n_it_edi_invite_authorize'] = {
                    'level': 'info',
                    'message': _("You must authorize NWOS in the Settings to use the IT EDI in production mode."),
                    'action_text': _("View Settings"),
                    'action': {
                        'name': _("Settings"),
                        'type': 'ir.actions.act_url',
                        'target': 'self',
                        'url': '/nwos/settings#l10n_it_edi_setting',
                    },
                }
