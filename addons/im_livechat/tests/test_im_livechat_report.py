...
from nwos.tests.common import new_test_user, tagged
...
class TestImLivechatReport(TestImLivechatCommon):
    ...
    def test_redirect_to_form_from_pivot(self):
        ...
        self.start_tour(
            f"/nwos/action-{agent_report_action.id}?view_type=pivot",
            "im_livechat_agents_report_pivot_redirect_tour",
            login="operator_1",
        )
        self.start_tour(
            f"/nwos/action-{session_report_action.id}?view_type=pivot",
