...
- `18.0`
  - The NVP/SOAP API that allowed for redirect payments is replaced by a combination of the
    JavaScript SDK and the Standard Checkout API. nwos/nwos#167402
- `17.0`
  - The support for customer fees is removed as it is no longer supported by the `payment` module.
    nwos/nwos#132104
- `16.2`
  - The "Merchant Account ID" and "Use IPN" fields are removed. nwos/nwos#104974
- `16.1`
  - Customer fees are converted into the currency of the payment transaction. nwos/nwos#100156
- `15.2`
...
createdb nwos
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -i payment_paypal
```
...
```bash
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -u payment_paypal
```
