...
from nwos.addons.link_tracker.tests.common import MockLinkTracker
...
class TestLinkTracker(common.TransactionCase, MockLinkTracker):
    ...
    def setUp(self):
        ...
        self._web_base_url = 'https://test.nwos.com'
    ...
    def test_absolute_url(self):
        ...
        link_tracker.write({'url': "nwos"})
        # Validate the absolute url is the combination of system parameter and link tracker's url
        self.assertEqual(link_tracker.absolute_url, f'{self._web_base_url}/nwos')
    ...
    def test_create(self):
        ...
        self.assertEqual(
            ...
            ['http://nwos.com', 'http://example.com', 'http://test.example.com'],
        )
    ...
    def test_search_or_create(self):
        values_1, values_2, values_3 = [
            {'url': 'https://nwos.com', 'title': 'NWOS'},
            {'url': 'https://nwos.be', 'title': 'NWOS'},
            {'url': 'https://nwos.com', 'title': 'NWOS New', 'label': 'New one!'}  # title is not in unique constraint
        ]
        ...
        values_4 = {'url': 'https://nwos.com', 'label': 'A different one'}
        ...
        values_5 = {'url': 'https://nwos.com', 'label': 'Yet another label'}
        expected_values_5 = {
            'campaign_id': self.env['utm.campaign'],
    ...
    def test_search_short_url(self):
        ...
        computed field"""
        campaign_1 = self.env['utm.campaign'].create({'name': 'campai.gn1...'})
        campaign_2 = self.env['utm.campaign'].create({'name': 'campai.gn2...'})
        source_1 = self.env['utm.source'].create({'name': 'source_1...'})
        source_2 = self.env['utm.source'].create({'name': 'source_2...'})
        medium_1 = self.env['utm.medium'].create({'name': 'medium_1'})
        medium_2 = self.env['utm.medium'].create({'name': 'medium_2'})
        base = f'{self._web_base_url}/r/'
