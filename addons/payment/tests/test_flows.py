...
from nwos.addons.payment.controllers.portal import PaymentPortal
...
class TestFlows(PaymentHttpCommon):
    ...
    def test_transaction_wrong_flow(self):
        ...
        with mute_logger("nwos.http"), self.assertRaises(
            JsonRpcException,
            msg='nwos.exceptions.UserError: The payment should either be direct, with redirection, or made by a token.',
        ):
    ...
    def test_transaction_wrong_token(self):
        ...
        with mute_logger('nwos.http'), self.assertRaises(JsonRpcException, msg='nwos.exceptions.ValidationError: The access token is invalid.'):
    ...
    def test_direct_payment_triggers_no_payment_request(self):
        ...
        self.user = self.portal_user
        with patch(
            'nwos.addons.payment.models.payment_transaction.PaymentTransaction'
            '._charge_with_token'
        ) as patched:
