...
import nwos.exceptions
from nwos.fields import Command, Date, Datetime
from nwos.http import Controller, Response, dispatch_rpc, request, route
from nwos.tools import lazy
from nwos.tools.misc import frozendict
...
# User code must use the exceptions defined in ``nwos.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, nwos.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, nwos.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, nwos.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, nwos.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, nwos.exceptions.MissingError):
    ...
    # InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(nwos.tools.exception_to_unicode(e), formatted_info)

    return dumps(fault)


class NWOSMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, nwos is sending binary as base64 string.
...
def dumps(params: list | tuple | xmlrpc.client.Fault) -> str:
    response = NWOSMarshaller(allow_none=False).dumps(params)
    return f"""\
<?xml version="1.0"?>
<methodResponse>
{response}
