...
from nwos.addons.certificate.tools import CertificateAdapter
from nwos.exceptions import UserError
from nwos.tools import float_repr, float_round, frozendict, zeep
...
def _get_zeep_operation(company, operation):
    ...
    if operation == 'registration':
        ...
        zeep_client = client._Client__obj  # get the "real" zeep client from the nwos specific wrapper
...
class L10nEsEdiVerifactuDocument(models.Model):
    ...
    def _check_record_values(self, vals):
        ...
        # We currently do not support cancelling records that are not registered or were registered outside nwos.
        if vals['cancellation'] and not verifactu_registered:
            errors.append(_("The cancelled record is not Veri*Factu registered (inside NWOS)."))
    ...
    def _render_vals_SistemaInformatico(self, vals):
        ...
        render_vals = {
            'SistemaInformatico': {
                ...
                'NombreSistemaInformatico': 'NWOS',
                'IdSistemaInformatico': '00',  # identifies NWOS the software as product of NWOS the company
