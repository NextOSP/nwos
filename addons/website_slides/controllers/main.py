...
from nwos import fields, http, tools, _
from nwos.addons.base.models.ir_qweb import keep_query
from nwos.addons.portal.controllers.portal_thread import PortalChatter
from nwos.addons.website.controllers.main import QueryURL
from nwos.addons.website.models.ir_http import sitemap_qs2dom
from nwos.addons.website_profile.controllers.main import WebsiteProfile
from nwos.exceptions import AccessError, ValidationError, UserError, MissingError
from nwos.fields import Domain
from nwos.http import request, Response
from nwos.tools import consteq, email_normalize_all
    ...
    def _set_viewed_slide(self, slide, quiz_attempts_inc=False):
        if not slide.channel_id.is_member:
            if not isinstance(request.session.get('viewed_slides'), dict):
                # Compatibility layer with NWOS 15.0,
                # where `viewed_slides` are stored as `list` in sessions.
                # For performance concerns, `viewed_slides` is changed to a dict,
                # but sessions coming from NWOS 15.0 after an upgrade should still be compatible.
                # This compatibility layer regarding `viewed_slides` must remain from NWOS 16.0 and above,
                # as this is possible to do a jump of multiple versions in one go,
                # and carry the sessions with the upgrade.
                # e.g. upgrade from NWOS 15.0 to 18.0.
