...
from nwos.addons.event.tests.common import EventCase
from nwos.addons.event.models.event_mail import EventMail
from nwos.addons.mail.tests.common import MailCase
from nwos.tests import tagged, users, warmup
...
class TestMailSchedule(EventMailCommon):
    ...
    @mute_logger('nwos.addons.event.models.event_mail')
    @users('user_eventmanager')
            ...
            [{
                ...
                'mail_mail_values': {
                    'body': f'<p>Communication for {self.test_event.name} scheduled on {before_scheduler.scheduled_date} failed. '
                            f'This may be linked to template <a href="{before_scheduler.get_base_url()}/nwos/mail.template/{self.template_reminder.id}">'
    ...
    @mute_logger('nwos.addons.event.models.event_mail', 'nwos.addons.mail.models.mail_render_mixin')
    @users('user_eventmanager')
    def test_event_mail_schedule_fail_global_composer_message(self):
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'mail_mail_values': {
                    'body': f'<p>Communication for {self.test_event.name} scheduled on {before_scheduler.scheduled_date} failed. '
                            f'This is due to an error in template <a href="{before_scheduler.get_base_url()}/nwos/mail.template/{self.template_reminder.id}">'
    ...
    @mute_logger(
        'nwos.addons.event.models.event_mail',
        'nwos.addons.event.models.event_mail_registration',
        'nwos.addons.event.models.event_registration',
    )
            ...
            [{
                ...
                'mail_mail_values': {
                    'body': f'<p>Communication for {self.test_event.name} scheduled on {test_dt} failed. '
                            f'This may be linked to template <a href="{onsub_scheduler.get_base_url()}/nwos/mail.template/{onsub_scheduler.template_ref.id}">'
