...
- `16.1`
  - The "Authorize Currency" field is replaced by the generic "Currencies" field of `payment`.
    nwos/nwos#101018
- `16.0`
  - Archiving a token no longer deactivates the related payment method on Authorize. nwos/nwos#93774
- `15.4`
  - The support for full refunds is added. nwos/nwos#92279
- `15.0`
  - Support for ACH payments is added. nwos/nwos#75289
- `14.3`
...
createdb nwos
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -i payment_authorize
```
...
```bash
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -u payment_authorize
```
