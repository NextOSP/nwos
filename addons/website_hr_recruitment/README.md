...
Get a clean and professional look for your job annouces. NWOS's unique *'edit inline'* approach makes website and job descriptions creation surprisingly easy.
...
createdb nwos
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -i website_hr_recruitment
```
...
```bash
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -u website_hr_recruitment
```
