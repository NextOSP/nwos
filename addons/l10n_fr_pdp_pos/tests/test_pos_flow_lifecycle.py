...
class TestPdpPosFlowLifecycle(TestPoSCommon):
    ...
    def setUpClass(cls):
        ...
        cls.startClassPatcher(patch(
            'nwos.addons.l10n_fr_pdp.models.res_company.ResCompany._compute_l10n_fr_f10_enable_reporting',
    ...
    def _patch_pos_date(self, date):
        ...
        with (
            patch('nwos.fields.Date.context_today', return_value=date),
            patch('nwos.fields.Date.today', return_value=date),
            patch('nwos.fields.Datetime.now', return_value=datetime_value),
        ):
            yield
    ...
    def _run_send_cron(self, date, identifier='POS-FLOW-TEST-001'):
        with (
            patch('nwos.fields.Date.today', return_value=fields.Date.to_date(date)),
            patch(
                'nwos.addons.l10n_fr_pdp.models.pdp_flow.PdpFlow._send_to_proxy',
