...
class NWOSEdiProxyAuth(requests.auth.AuthBase):
    ...
    def __call__(self, request):
        ...
        request.headers.update({
            'nwos-edi-client-id': self.id_client,
            'nwos-edi-timestamp': timestamp,
        })
        message = self.__get_payload(request, timestamp)

        if self.auth_type == 'asymmetric' and self.private_key:
            request.headers.update({
                'nwos-edi-signature': self.__sign_with_private_key(message),
                'nwos-edi-signature-type': 'asymmetric'
            })
        elif self.refresh_token:
            request.headers.update({
                'nwos-edi-signature': self.__sign_request_with_token(message),
                'nwos-edi-signature-type': 'hmac'
            })
