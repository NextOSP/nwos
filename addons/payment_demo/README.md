...
- `17.0`
  - The support for express checkout is added. nwos/nwos#113644
  - The support for customer fees is removed as it is no longer supported by the `payment` module.
    nwos/nwos#132104
- `16.3`
  - The support for partial manual capture is added. nwos/nwos#117356
- `16.0`
  - The module is renamed from `payment_test` to `payment_demo`. nwos/nwos#99397
  - The support for manual capture, full and partial refunds, customer fees, and the selection of
    the payment outcome are added. nwos/nwos#78083
...
createdb nwos
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -i payment_demo
```
...
```bash
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -u payment_demo
```
