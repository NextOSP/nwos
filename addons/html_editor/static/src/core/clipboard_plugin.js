...
export class ClipboardPlugin extends Plugin {
    ...
    onPaste(ev) {
        ...
        if (!this.delegateTo("paste_overrides", selection, ev.clipboardData)) {
            this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
                this.handlePasteNWOSEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePasteNWOSEditorHtml(clipboardData) {
        const nwosEditorHtml = clipboardData.getData("application/vnd.nwos.nwos-editor");
    ...
    async onDrop(ev) {
        ...
        const nwosEditorHtml = ev.dataTransfer.getData("application/vnd.nwos.nwos-editor");
        const fileTransferItems = !nwosEditorHtml && getImageFiles(dataTransfer);
        const htmlTransferItem = [...dataTransfer.items].find((item) => item.type === "text/html");
        if (fileTransferItems.length || htmlTransferItem || nwosEditorHtml) {
