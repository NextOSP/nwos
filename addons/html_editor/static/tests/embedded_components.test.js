...
import { beforeEach, describe, expect, getFixture, test } from "@nwos/hoot";
import { click, queryFirst, waitFor } from "@nwos/hoot-dom";
    ...
    test("Can mount and destroy recursive embedded components in any order", async () => {
        ...
        expect(
            [...fixture.querySelectorAll("[data-embedded]")].filter(
                (elem) => !elem.closest(".nwos-editor-editable")
            )
        ).toEqual([]);
    });
