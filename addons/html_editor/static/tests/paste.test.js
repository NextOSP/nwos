...
import { manuallyDispatchProgrammaticEvent as dispatch, press, waitFor } from "@nwos/hoot-dom";
import { animationFrame, tick } from "@nwos/hoot-mock";
...
import { pasteHtml, pasteNWOSEditorHtml, pasteText, undo } from "./_helpers/user_actions";
...
describe("Unwrapping html element", () => {
    ...
    test("should unwrap li elements having no ul/ol (1)", async () => {
        await testEditor({
            contentBefore: "<p>[]<br></p>",
            stepFunction: async (editor) => {
                pasteNWOSEditorHtml(editor, "<li><p>abc</p></li><li><p>def</p></li>");
            },
            contentAfter: "<p>abc</p><p>def[]</p>",
        });
    });
    ...
    test("should unwrap li elements having no ul/ol (2)", async () => {
        await testEditor({
            contentBefore: "<p>[]<br></p>",
            stepFunction: async (editor) => {
                pasteNWOSEditorHtml(editor, "<li><h1>abc</h1></li><li><h1>def</h1></li");
            },
...
describe("Special cases", () => {
    describe("lists", () => {
        ...
        test("should paste a deeply nested list copied outside from nwos", async () => {
            await testEditor({
                contentBefore: "<ul><li>[]<br></li></ul>",
                stepFunction: async (editor) => {
                    pasteHtml(
...
describe("link", () => {
    describe("range collapsed", () => {
        ...
        test("should replace link for new content (url) when pasting in an empty link (collapsed)", async () => {
            ...
            pasteText(editor, "http://nwos.com");
            await animationFrame();
            await expectElementCount(".o-we-powerbox", 0);
            expect(cleanLinkArtifacts(getContent(el))).toBe(
                `<p>xy<a href="http://nwos.com">http://nwos.com</a>[]z</p>`
            );
        });
        ...
        test("should paste and transform plain text content over an empty link (collapsed) (1)", async () => {
            await testEditor({
                ...
                stepFunction: async (editor) => {
                    pasteText(editor, "abc www.nwos.com xyz");
                },
        ...
        test("should paste and transform plain text content over an empty link (collapsed) (2)", async () => {
            await testEditor({
                contentBefore: '<p><a href="http://test.test/">[]\u200B</a></p>',
                stepFunction: async (editor) => {
                    pasteText(editor, "nwos.com\ngoogle.com");
                },
                contentAfter:
                    '<div><a href="http://nwos.com">nwos.com</a></div>' +
        ...
        test("should paste html content over an empty link (collapsed)", async () => {
            await testEditor({
                ...
                stepFunction: async (editor) => {
                    pasteHtml(
                        editor,
                        '<a href="www.nwos.com">nwos.com</a><br><a href="google.com">google.com</a>'
                    );
                },
                contentAfter:
                    '<p><a href="www.nwos.com">nwos.com</a></p><p><a href="https://google.com">google.com[]</a></p>',
            });
        });
        test("should paste html content over an empty link (collapsed) (2)", async () => {
            await testEditor({
                ...
                stepFunction: async (editor) => {
                    pasteHtml(
                        editor,
                        '<a href="www.nwos.com">nwos.com</a><br><a href="www.google.com">google.com</a>'
                    );
                },
                contentAfter:
                    '<p><a href="www.nwos.com">nwos.com</a></p><p><a href="www.google.com">google.com[]</a></p>',
            });
        });
    ...
    describe("range not collapsed", () => {
        ...
        test("should restore selection when pasting plain text followed by UNDO (3) (not collapsed)", async () => {
            await testEditor({
                contentBefore: "<p>[abc]</p>",
                stepFunction: async (editor) => {
                    pasteText(editor, "def www.nwos.com xyz");
                    undo(editor);
                },
                contentAfter: "<p>[abc]</p>",
            });
        ...
        test("should paste and transform plain text content over an image link if all of its contents is selected (not collapsed) (1)", async () => {
            ...
            await expectElementCount(".o-we-powerbox", 1);
            expect(getContent(el)).toBe(
                `<p>abhttps://download.nwoscdn.com/icons/website/static/description/icon.png[]cd</p>`
            );
            await press("Enter");
...
describe("NWOS editor own html", () => {
    test("should paste html as is", async () => {
        await testEditor({
            ...
            stepFunction: async (editor) => {
                pasteNWOSEditorHtml(editor, '<div class="custom-paste oe_unbreakable">b</div>');
            },
    ...
    test("should not paste unsafe content", async () => {
        await testEditor({
            contentBefore: "<p>a[]b</p>",
            stepFunction: async (editor) => {
                pasteNWOSEditorHtml(editor, `<script>console.log('xss attack')</script>`);
            },
...
describe("editable in iframe", () => {
    test("should paste nwos-editor html", async () => {
        const { el, editor } = await setupEditor("<p>[]</p>", { props: { iframe: true } });
        pasteNWOSEditorHtml(editor, `<p>text<b>bold text</b>more text</p>`);
...
describe("paste in contenteditable span", () => {
    test("should unwrap block when pasting inside a contenteditable span", async () => {
        ...
        pasteNWOSEditorHtml(editor, `<h1>text<b>bold text</b>more text</h1>`);
        expect(getContent(el)).toBe(
    ...
    <head>
        ...
        <style>
            ...
            .font12 {
                ...
                font-style: italic;
                text-decoration: none;
                font-family: "NWOS Unicode Support Noto";
    ...
    <table xmlns="http://www.w3.org/1999/xhtml" cellspacing="0" cellpadding="0" dir="ltr" border="1"
        ...
        <tbody>
            <tr style="height:21px;">
                <td style="overflow:hidden;padding:2px 3px 2px 3px;vertical-align:bottom;font-family:NWOS Unicode Support Noto;font-weight:normal;font-style:italic;color:#495057;"
            ...
            <tr style="height:21px;">
                <td style="overflow:hidden;padding:2px 3px 2px 3px;vertical-align:bottom;font-family:NWOS Unicode Support Noto;font-weight:bold;color:#495057;"
            ...
            <tr style="height:21px;">
                <td style="overflow:hidden;padding:2px 3px 2px 3px;vertical-align:bottom;background-color:#ffff00;font-family:NWOS Unicode Support Noto;font-weight:normal;color:#495057;"
...
describe("onDrop", () => {
    ...
    test("should move an image if it originated from the editor", async () => {
        ...
        await animationFrame();
        const imageHTML = dragdata.getData("application/vnd.nwos.nwos-editor");
        ...
        // Simulate the application/vnd.nwos.nwos-editor data that the browser would do.
        dropData.setData("application/vnd.nwos.nwos-editor", imageHTML);
        await dispatch(pElement, "drop", { dataTransfer: dropData });
        await animationFrame();
    ...
    test("should drag and drop banner", async () => {
        ...
        await animationFrame();
        const nwosEditorData = dragdata.getData("application/vnd.nwos.nwos-editor");
        const textHtml = dragdata.getData("text/html");
        const dropData = new DataTransfer();
        dropData.setData("text/html", textHtml);
        // Simulate the application/vnd.nwos.nwos-editor data that the browser would do.
        dropData.setData("application/vnd.nwos.nwos-editor", nwosEditorData);
    ...
    test("should drag and drop icon", async () => {
        ...
        const dropData = new DataTransfer();
        dropData.setData("text/html", textHtml);
        // Simulate the application/vnd.nwos.nwos-editor-node data that the browser would do.
        dropData.setData("application/vnd.nwos.nwos-editor-node", nwosEditorData);
        await dispatch(targetNodeForDrop, "drop", { dataTransfer: dropData });
        await animationFrame();
