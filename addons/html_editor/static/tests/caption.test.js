...
test("add a link to an image with a caption", async () => {
    await testEditor({
        ...
        stepFunction: async () => {
            await addLinkToImage("nwos.com");
            await expectElementCount(".o-we-linkpopover", 1);
            await expectElementCount(".o-we-toolbar", 1);
        },
        contentAfter: unformat(
            `<p>
                <a href="https://nwos.com">
                    <figure>
                        [<img class="img-fluid test-image" src="${base64Img}">]
                        <figcaption>Hello</figcaption>
                    </figure>
...
test("should cut an image and its caption as a single embedded figure", async () => {
    ...
    const cutPayload = clipboard.getData("application/vnd.nwos.nwos-editor");
...
test("should copy an image along with its caption", async () => {
    ...
    const copiedContent = clipboardData.getData("application/vnd.nwos.nwos-editor");
