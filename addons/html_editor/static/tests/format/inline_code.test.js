import { animationFrame, delay, expect, test } from "@nwos/hoot";
...
test("should paste NWOS editor html as plain text inside inline code", async () => {
    await testEditor({
        contentBefore: `<p>ab<code class="o_inline_code">Inline[]Code</code>cd</p>`,
        stepFunction: async (editor) => {
            const clipboardData = new DataTransfer();
            clipboardData.setData("text/plain", "Hello NWOS self.env.cr._enable_logging()");
            clipboardData.setData(
                "application/vnd.nwos.nwos-editor",
                `<p class="o_paragraph">Hello <strong>NWOS </strong><a href="http://self.env.cr">self.env.cr</a>._enable_logging()</p>`
            );
            const pasteEvent = new ClipboardEvent("paste", {
                clipboardData,
                bubbles: true,
        ...
        contentAfter: `<p>ab<code class="o_inline_code">InlineHello NWOS self.env.cr._enable_logging()[]Code</code>cd</p>`,
    });
});
