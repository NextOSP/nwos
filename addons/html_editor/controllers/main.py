...
from nwos import _, http, tools, SUPERUSER_ID
from nwos.addons.html_editor.tools import get_video_url_data
...
from nwos.tools.image import image_process, image_data_uri, binary_to_image, get_webp_size
    ...
    def link_preview_metadata_internal(self, preview_url):
        try:
            ...
            if not (
                ...
                and (
                    parsed_preview_url.path.startswith("/nwos")
            ...
            if (action_name.startswith('m-') or '.' in action_name) and action_name in request.env and not request.env[action_name]._abstract:
                # if path format is `nwos/<model>/<record_id>` so we use `action_name` as model name
