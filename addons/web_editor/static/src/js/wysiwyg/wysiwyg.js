...
import * as NWOSEditorLib from "@web_editor/js/editor/nwos-editor/src/NWOSEditor";
...
import { isSelectionInSelectors, peek } from '@web_editor/js/editor/nwos-editor/src/utils/utils';
...
import { fixInvalidHTML } from "../editor/nwos-editor/src/NWOSEditor";

const NWOSEditor = NWOSEditorLib.NWOSEditor;
const getDeepRange = NWOSEditorLib.getDeepRange;
const getInSelection = NWOSEditorLib.getInSelection;
const isProtected = NWOSEditorLib.isProtected;
const rgbToHex = NWOSEditorLib.rgbToHex;
const preserveCursor = NWOSEditorLib.preserveCursor;
const closestElement = NWOSEditorLib.closestElement;
const setSelection = NWOSEditorLib.setSelection;
const endPos = NWOSEditorLib.endPos;
const hasValidSelection = NWOSEditorLib.hasValidSelection;
const parseHTML = NWOSEditorLib.parseHTML;
const closestBlock = NWOSEditorLib.closestBlock;
const getRangePosition = NWOSEditorLib.getRangePosition;
const fillEmpty = NWOSEditorLib.fillEmpty;
const isVisible = NWOSEditorLib.isVisible;
const getSelectedNodes = NWOSEditorLib.getSelectedNodes;
...
export class Wysiwyg extends Component {
    ...
    setup() {
        ...
        const getColorPickedHandler = (colorType) => {
            return (params) => {
                if (this.hadNonCollapsedSelectionBeforeColorpicker) {
                    this.nwosEditor.historyResetLatestComputedSelection(true);
                }
                // Unstash the mutations now that the color is picked.
                this.nwosEditor.historyUnstash();
                ...
                // hides it).
                if (hasValidSelection(this.nwosEditor.editable)) {
                    this.nwosEditor.deselectTable();
                }
        ...
        const getColorHoverHandler = (colorType) => {
            return (props) => {
                ...
                this.nwosEditor.historyPauseSteps();
                try {
                    this._processAndApplyColor(colorType, props.color, true);
                    this.nwosEditor._computeHistorySelection();
                } finally {
                    this.nwosEditor.historyUnpauseSteps();
                }
            }
        };
        ...
        const colorPaletteCommonOptions = {
            ...
            onColorLeave: () => {
                ...
                this.nwosEditor.withoutRollback(() => this.nwosEditor.historyRevertCurrentStep());
    ...
    async startEdition() {
        ...
        this.nwosEditor = new NWOSEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.nwosEditor.addEventListener('contentChanged', function () {
        ...
        if (this.nwosEditor.document !== document) {
            this.nwosEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.nwosEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.nwosEditor.document.addEventListener("keyup", this._signalOnline, true);
        }

        this._initialValue = this.getValue();
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $wrapwrap[0].addEventListener('scroll', this.nwosEditor.multiselectionRefresh, { passive: true });
        ...
        this._observeNWOSFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.nwosEditor.observerUnactive();
                const $field = $(this);
                ...
                self.nwosEditor.observerActive();
            }
        );
        ...
        this.$editable.on('dblclick', mediaSelector, ev => {
            ...
            if (isEditable) {
                ...
                if (!isProtected(this.nwosEditor.document.getSelection().anchorNode)) {
        ...
        if (options.snippets) {
            $(this.nwosEditor.document.body).addClass('editor_enable');
            if (localization.direction === "rtl") {
                this.nwosEditor.document.body.setAttribute("is-rtl-backend", "true");
            }
        ...
        if (this.options.getContentEditableAreas) {
            $(this.options.getContentEditableAreas(this.nwosEditor)).find('*').off('mousedown mouseup click');
        }
        ...
        $(this.nwosEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.nwosEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.nwosEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.nwosEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        this.setCSSVariables(this.snippetsMenu ? this.snippetsMenu.el : this.toolbarEl);

        this.nwosEditor.addEventListener('preObserverActive', () => {
            ...
            this._onPostSanitize(this.nwosEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.nwosEditor.isMobile) {
                this.nwosEditor.editable.before(this.toolbarEl);
            } else {
    ...
    setupToolbar(toolbarEl) {
        ...
        this.nwosEditor.setupToolbar(toolbarEl);
    ...
    destroy() {
        ...
        if (this.nwosEditor) {
            this.nwosEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.nwosEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.nwosEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.nwosEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            this.nwosEditor.destroy();
        }
        if (this.snippetsMenu) {
        ...
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length && this.nwosEditor) {
            $('#wrapwrap')[0].removeEventListener('scroll', this.nwosEditor.multiselectionRefresh, { passive: true });
        }
        $(this.$root).off('click');
    ...
    _renderElement() {
        ...
        if (this.options.resizable && !isMobileOS()) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper nwos-editor">');
            this.$root = $wrapper;
    ...
    getValue(options) {
        ...
        if (this.nwosEditor) {
            this.nwosEditor.cleanForSave($editable[0]);
    ...
    async saveContent(reload = true, editable = false) {
        ...
        if (!editable) {
            ...
            const editables = "getContentEditableAreas" in this.options ? this.options.getContentEditableAreas(this.nwosEditor) : [];
    ...
    setValue(value, isSavePoint = true) {
        this.nwosEditor.resetContent(value, isSavePoint);
    }
    /**
     * Undo one step of change in the editor.
     */
    undo() {
        this.nwosEditor.historyUndo();
    }
    /**
     * Redo one step of change in the editor.
     */
    redo() {
        this.nwosEditor.historyRedo();
    }
    /**
     ...
     */
    focus() {
        if (this.nwosEditor && !this.nwosEditor.historyResetLatestComputedSelection(true)) {
            ...
            const selection = this.nwosEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    }
    getDeepRange() {
        return getDeepRange(this.nwosEditor.editable);
    }
    closestElement(...args) {
        return closestElement(...args);
    }
    async cleanForSave(editable = this.nwosEditor.editable) {
        if (this.nwosEditor) {
            this.nwosEditor.cleanForSave(editable);
    ...
    isSelectionInEditable() {
        return this.nwosEditor.isSelectionInEditable();
    }
    /**
     * Start or resume the NWOS field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeNWOSFieldChanges() {
        ...
        if (this.nwosFieldObservers) {
            for (let observerData of this.nwosFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const nwosFieldSelector = '[data-oe-model], [data-oe-translation-initial-sha]';
            const $nwosFields = this.$editable.find(nwosFieldSelector);
            const renderingClassesSelector = this.nwosEditor.options.renderingClasses
                .map(className => `.${className}`).join(", ");
            this.nwosFieldObservers = [];

            $nwosFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.nwosEditor.filterMutationRecords(mutations);
                    ...
                    let $nodes = $nwosFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    //  `_pauseNWOSFieldObservers(field)` call.
                    this.__nwosFieldObserversToPause = this.nwosFieldObservers.filter(
                    ...
                    for (const node of fieldNodeClone.querySelectorAll(renderingClassesSelector)) {
                        node.classList.remove(...this.nwosEditor.options.renderingClasses);
                    }
                    ...
                    this.nwosEditor.observerUnactive("undo_redo_header");
                    this.nwosEditor.withoutRollback(() => {
                    ...
                    this.nwosEditor.observerActive("undo_redo_header");
                    this._observeNWOSFieldChanges();
                });
                observer.observe(field, observerOptions);
                this.nwosFieldObservers.push({field: field, observer: observer});
            });
        }
    }
    /**
     ...
     */
    _pauseNWOSFieldObservers() {
        // TODO adapt in master: remove this and directly exclude observers with
        // targets inside the current field (we use `this.nwosFieldObservers`
        // as fallback for compatibility here).
        const fieldObserversData = this.__nwosFieldObserversToPause || this.nwosFieldObservers;
    ...
    openLinkToolsFromSelection() {
        ...
        for (const el of getSelectedNodes(this.nwosEditor.editable)) {
            if (
        ...
        if (targetEl.nodeType === Node.ELEMENT_NODE
            ...
            this.nwosEditor.dispatchEvent(new Event('activate_image_link_tool'));
            return;
        }
        this.toggleLinkTools();
    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        const linkEl = getInSelection(this.nwosEditor.document, 'a');
        ...
        if (this.snippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                if (!this.state.linkToolProps || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkToolsInfos.link)) {
                    const { link } = getOrCreateLink({
                        containerNode: this.nwosEditor.editable,
                    ...
                    const addHintClasses = () => {
                        this.nwosEditor.observerUnactive("hint_classes");
                        link.classList.add('oe_edited_link');
                        $button.addClass('active');
                        this.nwosEditor.observerActive("hint_classes");
                    };
                    ...
                    this.state.linkToolProps = {
                        ...
                        wysiwyg: this,
                        editable: this.nwosEditor.editable,
                        link,
                ...
                const _onClick = ev => {
                    ...
                    if (
                        ...
                        this.destroyLinkTools();
                        this.nwosEditor.document.removeEventListener('click', _onClick, true);
                        document.removeEventListener('click', _onClick, true);
                    }
                };
                this.nwosEditor.document.addEventListener('click', _onClick, true);
                document.addEventListener('click', _onClick, true);
            } else {
                this.destroyLinkTools();
            }
        } else {
            const historyStepIndex = this.nwosEditor.historySize() - 1;
            this.nwosEditor.historyPauseSteps();
            ...
            this.env.services.dialog.add(LinkDialog, {
                ...
                onClose: () => {
                    this.nwosEditor.historyUnpauseSteps();
                    this.nwosEditor.historyRevertUntil(historyStepIndex)
                }
            });
        }
    }
    ...
    openChatGPTDialog(mode = 'prompt') {
        const restore = preserveCursor(this.nwosEditor.document);
        const params = {
            insert: content => {
                this.nwosEditor.historyPauseSteps();
                const insertedNodes = this.nwosEditor.execCommand('insert', content);
                this.nwosEditor.historyUnpauseSteps();
                ...
                if (start && end) {
                    const divContainer = this.nwosEditor.editable.parentElement;
        ...
        if (mode === 'alternatives') {
            params.originalText = this.nwosEditor.document.getSelection().toString() || '';
        }
        this.nwosEditor.document.getSelection().collapseToEnd();
    ...
    removeLink() {
        if (this.snippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.snippetsMenu._mutex.exec(() => {
                // Wait for the editor panel to be fully updated.
                this.nwosEditor.dispatchEvent(new Event('deactivate_image_link_tool'));
            });
        } else {
            this.nwosEditor.execCommand('unlink');
        }
    }
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.nwosEditor.document.getSelection();
        ...
        const restoreSelection = preserveCursor(this.nwosEditor.document);

        const editable = NWOSEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.nwosEditor.editable;
    ...
    // todo: test me
    showEmojiPicker() {
        const targetEl = this.nwosEditor.document.getSelection();
        ...
        this.popover.add(closest, EmojiPicker, {
                onSelect: (str) => {
                    restoreSelection();
                    this.nwosEditor.execCommand('insert', str);
                },
            }, {
    ...
    async _onMediaDialogSave(params, element) {
        ...
        if (params.node) {
            ...
            this.nwosEditor.historyUnpauseSteps();
            this.nwosEditor.unbreakableStepUnactive();
            ...
            if (params.node.matches(".oe_unremovable")) {
                ...
                this.nwosEditor.historyStep();
                this.nwosEditor.observerUnactive("unremovable");
                element.classList.add("oe_unremovable");
                this.nwosEditor.observerActive("unremovable");
            } else {
                this.nwosEditor.historyStep();
            }

            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.nwosEditor.editable.focus();
        } else {
            this.nwosEditor.historyPauseSteps();
            const result = this.nwosEditor.execCommand('insert', element);
            await saveCallback(element);
            this.nwosEditor.historyUnpauseSteps();
            this.nwosEditor.historyStep();
            // Refocus again to save updates when calling `_onWysiwygBlur`
    ...
    getInSelection(selector) {
        return getInSelection(this.nwosEditor.document, selector);
    }
    /**
    ...
    async _createSnippetsMenuInstance(options={}) {
        const snippetsEditor = await nwos.loader.modules.get('@web_editor/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _configureToolbar(options) {
        ...
        const openTools = e => {
            ...
            switch (e.currentTarget.id) {
                ...
                case 'open-chatgpt': {
                    this.openChatGPTDialog(this.nwosEditor.document.getSelection()?.isCollapsed ? 'prompt' : 'alternatives');
                    break;
                }
            }
        };
        ...
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.nwosEditor.document});
            const destroyTransfo = () => {
                $image.transfo('destroy');
                $(this.nwosEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                this.nwosEditor.document.removeEventListener('keydown', keydown);
            }
            const mouseup = () => {
                imgTransformBtn.classList.toggle('active', $image[0].matches('[style*="transform"]'));
            };
            $(this.nwosEditor.document).on('mouseup', mouseup);
            ...
            $(this.nwosEditor.document).on('mousedown', mousedown);
            ...
            this.nwosEditor.document.addEventListener('keydown', keydown);
        });
    ...
    _showImageCrop() {
        ...
        this.nwosEditor.toolbarHide();
        $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.nwosEditor.toolbarShow());
    }
    /**
     * @private
     * @param {jQuery} $
    ...
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        const selectedTds = this.nwosEditor.document.querySelectorAll('td.o_selected_td');
        ...
        let coloredElements = this.nwosEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        // Some nodes returned by applyColor can be removed of the document by the sanitization in historyStep
        coloredElements = coloredElements.filter(element => this.nwosEditor.document.contains(element));
        ...
        if (selectedTds.length === 1 && !previewMode) {
            ...
            sel.addRange(getDeepRange(this.nwosEditor.editable, { range }));
        }

        const hexColor = this._colorToHex(color);
        this.nwosEditor.updateColorpickerLabels({
     ...
     */
    _updateEditorUI(e) {
        let selection = this.nwosEditor.document.getSelection();
        if (!selection) return;
        const anchorNode = selection.anchorNode;
        if (isProtected(anchorNode)) {
            return;
        }
        if (this.nwosEditor.document.querySelector(".transfo-container")) {
            return;
        }

        this.nwosEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.nwosEditor.document.defaultView;
        ...
        if (isInMedia) {
            ...
            if ($target.attr("data-mimetype")) {
                ...
                const selection = this.nwosEditor.document.getSelection();
                const range = this.nwosEditor.document.createRange();
        ...
        this.nwosEditor.updateToolbarPosition();
    ...
    _updateMediaJustifyButton(commandState) {
        ...
        // Change the ID to prevent NWOSEditor from controlling it as this is
    ...
    _getBannerCommand(title, alertClass, iconClass, description, priority) {
        return {
            ...
            isDisabled: () => isSelectionInSelectors('.o_editor_banner') || !this.nwosEditor.isSelectionInBlockRoot(),
            callback: () => {
                const bannerElement = parseHTML(this.nwosEditor.document, `
                ...
                `).childNodes[0];
                this.nwosEditor.execCommand('insert', bannerElement);
                this.nwosEditor.activateContenteditable();
    ...
    _getPowerboxOptions() {
        ...
        const commands = [
            ...this._getBannerCommands(),
            {
                ...
                isDisabled: () => !this.nwosEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.nwosEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.nwosEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.nwosEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (user && user.signature) {
                        this.nwosEditor.execCommand('insert', parseHTML(this.nwosEditor.document, user.signature));
                    }
                },
            },
            {
            ...
            commands.push(
                {
                    ...
                    callback: () => this.nwosEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.nwosEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                        const anchor = this.nwosEditor.document.getSelection().anchorNode;
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.nwosEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.nwosEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.nwosEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.nwosEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.nwosEditor.execCommand('columnize', 0),
                    isDisabled: () => {
                        if (!this.nwosEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
    ...
    async _saveElement($el, context) {
        ...
        // remove ZeroWidthSpace from nwos field value
        // ZeroWidthSpace may be present from NWOSEditor edition process
    ...
    _onSelectionChange() {
        if (this.linkPopover && this.isSelectionInEditable()) {
            const selectionInLink = getInSelection(this.nwosEditor.document, 'a') === this.linkPopover.target;
            const isVisible = this.linkPopover.el.offsetParent;
            if (
                isVisible &&
                (
                    (this.options.autohideToolbar && !this.nwosEditor.document.getSelection().isCollapsed && !this.linkPopover.$target.attr("data-mimetype")) ||
                    !selectionInLink
                )
            ) {
                this.linkPopover.hide();
    ...
    async _signalOnline() {
        ...
        this.ptp.notifyAllClients('oe_history_step', peek(this.nwosEditor.historyGetSteps()), { transport: 'rtc' });
    }
    /**
    ...
    async _processMissingSteps(missingSteps) {
        ...
        this.ptp && this.nwosEditor.onExternalHistorySteps(missingSteps);
        return true;
    }
    _showConflictDialog() {
        if (this._conflictDialogOpened) return;
        const content = markup(this.nwosEditor.editable.cloneNode(true).outerHTML);
    ...
    _getNewPtp() {
        ...
        return new PeerToPeer({
            ...
            onRequest: {
                ...
                get_missing_steps: (params) => this.nwosEditor.historyGetMissingSteps(params.requestPayload),
                get_history_from_snapshot: () => this._getHistorySnapshot(),
                get_collaborative_selection: () => this.nwosEditor.getCurrentCollaborativeSelection(),
                recover_document: (params) => {
                    const { serverDocumentId, fromStepId } = params.requestPayload;
                    if (!this.nwosEditor.historyGetBranchIds().includes(serverDocumentId)) {
                        return;
                    }
                    return {
                        missingSteps: this.nwosEditor.historyGetMissingSteps({ fromStepId }),
            ...
            onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                switch (notificationName) {
                    case 'ptp_remove':
                        this.nwosEditor.multiselectionRemove(notificationPayload);
                        break;
                    case 'ptp_disconnect':
                        this.ptp.removeClient(fromClientId);
                        this.nwosEditor.multiselectionRemove(fromClientId);
                        break;
                    case 'rtc_data_channel_open': {
                        ...
                        const getClientNamePromise = this.requestClient(
                            ...
                            this.nwosEditor.multiselectionRefresh();
                        });
                    ...
                    case 'oe_history_step':
                        if (this._historySyncFinished) {
                            this.nwosEditor.onExternalHistorySteps([notificationPayload]);
                        } else {
                    ...
                    case 'oe_history_set_selection': {
                        ...
                        this.nwosEditor.onExternalMultiselectionUpdate(selection);
                        break;
                    }
                }
            }
    ...
    async _setCollaborativeSelection(fromClientId) {
        ...
        if (remoteSelection) {
            this.nwosEditor.onExternalMultiselectionUpdate(remoteSelection);
        }
    }
    /**
    ...
    _isLastDocumentStale() {
        ...
        return !this.nwosEditor.historyGetBranchIds().includes(this._serverLastStepId);
    }
    /**
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    'recover_document', {
                        ...
                        fromStepId: peek(this.nwosEditor.historyGetBranchIds()),
                    },
                    { transport: 'rtc' }
                ).then((response) => {
                    nbPendingResponses--;
    ...
    async _resetFromServerAndResyncWithClients() {
        ...
        // lastHistoryId will be different if the nwos bus did not have time to
        // notify the user.
    ...
    async _resetFromClient(fromClientId, resetCollabCount) {
        ...
        if (this._historyStepsBuffer.length) {
            this.nwosEditor.onExternalHistorySteps(this._historyStepsBuffer);
            this._historyStepsBuffer = [];
        }
    ...
    async resetValue(value) {
        this.setValue(value);
        this.nwosEditor.historyReset();
        this.nwosEditor.lastSavePoint = this.nwosEditor._historyIds.at(-1);
        this._historyShareId = Math.floor(Math.random() * Math.pow(2,52)).toString();
        this._serverLastStepId = value && this._getLastHistoryStepId(value);
        if (this._serverLastStepId) {
            this.nwosEditor.historySetInitialId(this._serverLastStepId);
        }
    }
    /**
    ...
    async resetEditor(value, options) {
        ...
        this._currentClientId = this._generateClientId();
        this.nwosEditor.collaborationSetClientId(this._currentClientId);
        this.resetValue(value);
    ...
    _getHistorySnapshot() {
        return Object.assign(
            {},
            this.nwosEditor.historyGetSnapshotSteps(),
    ...
    _applySnapshot(snapshot) {
        ...
        this._historySyncAtLeastOnce = true;
        this.nwosEditor.historyResetFromSteps(steps, historyIds);
        this.nwosEditor.historyResetLatestComputedSelection();
        return true;
    }
    /**
    ...
    _onPostSanitize(node) {
        ...
        // solution is merged.
        if (node?.querySelectorAll && this.nwosEditor && !this.nwosEditor._fixLinkMutatedElements) {
    ...
    _attachHistoryIds(editable = this.nwosEditor.editable) {
        if (this.options.collaborative) {
            ...
            const historyIds = this.nwosEditor.historyGetBranchIds().join(',');
    ...
    _beforeAnyCommand() {
        ...
        for (const node of NWOSEditorLib.getTraversedNodes(this.$editable[0])) {
