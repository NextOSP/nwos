...
export function closestElement(node, predicate = "*") {
    ...
    return element?.closest('.nwos-editor-editable') && element;
}
...
export function getNormalizedCursorPosition(node, offset, full = true) {
    const editable = closestElement(node, '.nwos-editor-editable');
...
// TODO rename this function in master: it also handles NWOS icons, not only
...
export function prepareUpdate(...args) {
    ...
    while (positions.length) {
        ...
        if (options.debug) {
            const editable = el && closestElement(el, '.nwos-editor-editable');
