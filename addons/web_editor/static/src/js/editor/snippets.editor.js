...
import { closestElement, isUnremovable } from "@web_editor/js/editor/nwos-editor/src/utils/utils";
...
var SnippetEditor = Widget.extend({
    ...
    clone: async function (recordUndo) {
        ...
        if (recordUndo) {
            this.options.wysiwyg.nwosEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.trigger_up('call_for_each_child_snippet', {
                $snippet: $clone,
    ...
    _outPreviousDropzone(self, currentDropzoneEl) {
        ...
        if (rowEl.classList.contains('o_grid_mode')) {
            ...
            self.options.wysiwyg.nwosEditor.observerActive('dragAndDropMoveSnippet');
            gridUtils._resizeGrid(rowEl);
            self.options.wysiwyg.nwosEditor.observerUnactive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStart({ helper, addStyle }) {
        this.options.wysiwyg.nwosEditor.observerUnactive('dragAndDropMoveSnippet');
        this.trigger_up('drag_and_drop_start');
        this.options.wysiwyg.nwosEditor.automaticStepUnactive();
        var self = this;
        this.dragState = {};
        ...
        if (rowEl.classList.contains('row') && this.options.isWebsite) {
            if (allowGridMode) {
                ...
                if (!rowEl.classList.contains('o_grid_mode')) {
                    this.options.wysiwyg.nwosEditor.observerActive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStop({ x, y }) {
        this.options.wysiwyg.nwosEditor.automaticStepActive();
        this.options.wysiwyg.nwosEditor.automaticStepSkipStack();
        this.options.wysiwyg.nwosEditor.unbreakableStepUnactive();
...
var SnippetsMenu = Widget.extend({
    ...
    async start() {
        ...
        $(this.options.wysiwyg.nwosEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.nwosEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.nwosEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            ...
            if ($undoButton.length) {
                const updateHistoryButtons = () => {
                    $undoButton.attr('disabled', !this.options.wysiwyg.nwosEditor.historyCanUndo());
                    $redoButton.attr('disabled', !this.options.wysiwyg.nwosEditor.historyCanRedo());
                };
                this.options.wysiwyg.nwosEditor.addEventListener('historyStep', updateHistoryButtons);
                this.options.wysiwyg.nwosEditor.addEventListener('observerApply', () => {
                    $(this.options.wysiwyg.nwosEditor.editable).trigger('content_changed');
                });
            }
    ...
    _makeSnippetDraggable() {
        ...
        const dragAndDropOptions = this.options.getDragAndDropOptions({
            ...
            onDragStart: ({ element }) => {
                ...
                const doc = this.options.wysiwyg.nwosEditor.document;
                $(doc.body).addClass('oe_dropzone_active');

                this.options.wysiwyg.nwosEditor.automaticStepUnactive();

                this.$el.find('.oe_snippet_thumbnail').addClass('o_we_already_dragging');
                this.options.wysiwyg.nwosEditor.observerUnactive('dragAndDropCreateSnippet');
            ...
            onDragEnd: async ({ x, y, helper }) => {
                ...
                this.options.wysiwyg.nwosEditor.observerActive('dragAndDropCreateSnippet');
   ...
   _onSnippetDragAndDropStart: function () {
    ...
    _onSnippetClick() {
        ...
        this.options.wysiwyg.nwosEditor.observerUnactive();
        for (const el of snippetEls) {
            el.classList.add("o_catch_attention", "o_we_snippet_area_animation");
        }
        this.options.wysiwyg.nwosEditor.observerActive();
    ...
    _addToolbar(toolbarMode = "text") {
        ...
        this.options.wysiwyg.nwosEditor.bindExecCommand(customizeTableBlock);
    ...
    _checkEditorToolbarVisibility: function (e) {
        ...
        const selection = this.options.wysiwyg.nwosEditor.document.getSelection();
    ...
    _onRequestEditable: function (ev) {
        ev.data.callback($(this.options.wysiwyg.nwosEditor.editable));
    },
    /**
