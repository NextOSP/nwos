...
import { getAdjacentPreviousSiblings, isBlock, rgbToHex, commonParentGet } from '../editor/nwos-editor/src/utils/utils';
...
export async function toInline($editable, cssRules, $iframe) {
    $editable.removeClass('nwos-editor-editable');
    const editable = $editable.get(0);
    ...
    $editable.addClass('nwos-editor-editable');
}
/**
...
function fontToImg($editable) {
    const editable = $editable.get(0);
    const { fonts } = nwos.loader.modules.get("@web_editor/js/wysiwyg/fonts");
