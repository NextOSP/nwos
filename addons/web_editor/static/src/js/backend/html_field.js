...
import {
    ...
    getRangePosition
} from '@web_editor/js/editor/nwos-editor/src/utils/utils';
...
export class HtmlField extends Component {
    ...
    _filterPowerBoxCommands(commands) {
        ...
        if (this.wysiwyg && this.wysiwyg.nwosEditor) {
            const selection = this.wysiwyg.nwosEditor.document.getSelection();
    ...
    async updateValue() {
        ...
        if (
            ...
            if (this.wysiwyg.nwosEditor) {
                this.wysiwyg.nwosEditor.lastSavePoint =
                    this.wysiwyg.nwosEditor._historySteps.at(-1).id;
            }
        }
    }
    async startWysiwyg(wysiwyg) {
        ...
        wysiwyg.$editable[0].classList.add("nwos-editor-qweb");
        ...
        this.wysiwyg.nwosEditor.lastSavePoint = null;
        this.isDirty = false;
        this.wysiwyg.nwosEditor.addEventListener("historyStep", () => {
            if (!this.wysiwyg.nwosEditor.lastSavePoint) {
                this.wysiwyg.nwosEditor.lastSavePoint = this.wysiwyg.nwosEditor._historySteps[0].id;
            }
            const lastStep = this.wysiwyg.nwosEditor._historySteps.at(-1);
            this.isDirty = true;
            if (lastStep.isSavePoint) {
                this.wysiwyg.nwosEditor.lastSavePoint = lastStep.id;
                this.isDirty = false;
            } else if (
                lastStep.restoredStepId &&
                lastStep.restoredStepId === this.wysiwyg.nwosEditor.lastSavePoint
            ) {
                this.wysiwyg.nwosEditor.lastSavePoint = lastStep.id;
                this.isDirty = false;
            }
        ...
        if (this.props.isCollaborative) {
            this.wysiwyg.nwosEditor.addEventListener("onExternalHistorySteps", () =>
                this.props.record.model.bus.trigger("FIELD_IS_DIRTY", this._isDirty())
            );
        }
    ...
    toggleCodeView() {
        ...
        if (this.wysiwyg) {
            this.wysiwyg.nwosEditor.observerUnactive('toggleCodeView');
            if (this.state.showCodeView) {
                this.wysiwyg.$editable.remove();
                this.wysiwyg.nwosEditor.toolbarHide();
                const value = this.wysiwyg.getValue();
                this.props.record.update({ [this.props.name]: value });
            } else {
                this.wysiwyg.nwosEditor.observerActive('toggleCodeView');
            }
        }
        if (!this.state.showCodeView) {
    ...
    onDynamicPlaceholderValidate(chain, defaultValue) {
        if (chain) {
            ...
            this.wysiwyg.nwosEditor.execCommand('insert', t);
            // Ensure the dynamic placeholder <t> element is sanitized.
            this.wysiwyg.nwosEditor.sanitize(t);
        }
    }
    ...
    async commitChanges({ urgent, shouldInline } = {}) {
        ...
        if (this._isDirty() || urgent || (shouldInline && this.props.isInlineStyle)) {
            ...
            if (this.wysiwyg && this.wysiwyg.nwosEditor) {
                ...
                this.wysiwyg.nwosEditor.observerUnactive('commitChanges');
                ...
                this.wysiwyg.nwosEditor.observerActive('commitChanges');
            }
    ...
    async _lazyloadWysiwyg() {
        ...
        let wysiwygModule = await nwos.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
        this.MoveNodePlugin = (await nwos.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin'))?.MoveNodePlugin;
        // Otherwise, load the module.
        if (!wysiwygModule) {
            await loadBundle('web_editor.backend_assets_wysiwyg');
            wysiwygModule = await nwos.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
            this.MoveNodePlugin = (await nwos.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin')).MoveNodePlugin;
        }
    ...
    async _toInline() {
        ...
        this.wysiwyg.nwosEditor.sanitize(this.wysiwyg.nwosEditor.editable);
        const html = this.wysiwyg.getValue();
        const $nwosEditor = $editable.closest('.nwos-editor-editable');
        // Save correct nodes references.
        // Remove temporarily the class so that css editing will not be converted.
        $nwosEditor.removeClass('nwos-editor-editable');
        $editable.html(html);

        await toInline($editable, undefined, this.wysiwyg.$iframe);
        $nwosEditor.addClass('nwos-editor-editable');
...
export const htmlField = {
    ...
    extractProps({ attrs, options }, dynamicInfo) {
        ...
        return {
            codeview: Boolean(nwos.debug && options.codeview),
            placeholder: attrs.placeholder,
