...
from nwos.addons.mail.models.mail_mail import _UNFOLLOW_REGEX
...
class RecipientsNotificationTest(MailCommon):
    ...
    def test_subscribe_post_author(self):
        """ Test author is added in followers, unless it is archived / nwosbot """
        # some automated action post on behalf of author
        test_record = self.env['mail.test.simple'].create({'name': 'Test'})
        self.partner_root.active = True  # edge case, people activating NWOSbot partner (not user)
        ...
        for user, author, exp_followers in [
            ...
            (self.user_2, self.user_root.partner_id, self.env['res.partner']),  # no nwosbot !
        ]:
...
class UnfollowLinkTest(MailCommon, HttpCase):
    ...
    @mute_logger('nwos.addons.base.models', 'nwos.addons.mail.controllers.mail', 'nwos.http', 'nwos.models')
