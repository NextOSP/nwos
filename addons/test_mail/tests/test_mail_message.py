...
from nwos.addons.base.models.ir_mail_server import MailDeliveryException
...
from nwos.tests.common import tagged, users, HttpCase
from nwos.tools import is_html_empty, mute_logger, formataddr
...
class TestMessageHelpersRobustness(MailCommon, HttpCase):
    ...
    def setUp(self):
        ...
        with mute_logger('nwos.addons.mail.models.mail_mail'), self.mock_mail_gateway():
    ...
    def test_load_message_failures(self):
        ...
        with contextlib.suppress(Exception), mute_logger('nwos.http', 'nwos.sql_db'):  # suppress logged error due to readonly route doing an update
    ...
    def test_message_link_by_employee(self):
        ...
        with self.subTest(thread_message=thread_message):
            expected_url = self.base_url() + f'/nwos/{thread_message.model}/{thread_message.res_id}?highlight_message_id={thread_message.id}'
...
class TestMessageValues(MailCommon):
    ...
    @mute_logger('nwos.models.unlink', 'nwos.addons.mail.models.models')
    @users('employee')
    ...
    def test_mail_message_values_fromto_document_alias(self):
        ...
        self.assertIn('-nwos-%d-mail.test' % self.alias_record.id, msg.message_id.split('@')[0])
        reply_to_name = self.user_employee.name
    ...
    def test_mail_message_values_fromto_document_no_alias(self):
        ...
        self.assertIn('-nwos-%d-mail.test.simple' % test_record.id, msg.message_id.split('@')[0])
        reply_to_name = self.user_employee.name
        reply_to_email = '%s@%s' % (self.alias_catchall, self.alias_domain)
        self.assertEqual(msg.reply_to, formataddr((reply_to_name, reply_to_email)))
        self.assertEqual(msg.email_from, formataddr((self.user_employee.name, self.user_employee.email)))
