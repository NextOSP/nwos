...
class TestMailAlias(TestMailAliasCommon):
    ...
    def test_alias_name_unique_copy(self):
        ...
        with mute_logger('nwos.sql_db'), self.assertRaises(psycopg2.errors.UniqueViolation):
...
class TestAliasCompany(TestMailAliasCommon):
    ...
    def test_alias_domain_setup(self):
        ...
        with self.assertRaises(psycopg2.errors.IntegrityError), mute_logger('nwos.sql_db'):
...
class TestMailAliasDomain(TestMailAliasCommon):
    ...
    def test_alias_domain_parameters_validation(self):
        ...
        with self.subTest(failing_name=''):
            with mute_logger('nwos.sql_db'), self.assertRaises(exceptions.ValidationError):
        ...
        with self.subTest(failing_name=' '):
            with mute_logger('nwos.sql_db'), self.assertRaises(psycopg2.errors.NotNullViolation):
                self.env['mail.alias.domain'].create({'name': ' '})
