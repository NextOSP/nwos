...
from nwos.tests import new_test_user, tagged

from .dummy_methods import DummyMethods
from nwos.addons.api_doc.controllers.api_doc import (
    DocController,
    is_public_method,
    parse_signature,
)
...
class TestDoc(HttpCaseWithUserDemo):
    ...
    def test_doc_access(self):
        ...
        for path in ('/doc', '/doc/index.json', '/doc/res.company.json'):
            with self.subTest(path=path):
                ...
                self.assertEqual(capture.output, [f'WARNING:nwos.http:{e}'])
    ...
    def test_doc_cache(self):
        ...
        self.assertRegex(
            ...
            r'nwos-doc-index-\w+-%s\.json' % etag_demo.strip('"'),
        ...
        self.assertRegex(res.headers.get('Content-Disposition', ''), r'\bnwos-doc-index\.json$',
    ...
    def test_private_methods(self):
        FakeCls = type('ModelDummyMethods', (DummyMethods, Model), {
            ...
            '_register': False,
            '__module__': 'nwos.addons.api_doc',
        })
