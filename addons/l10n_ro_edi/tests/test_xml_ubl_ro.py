...
from nwos.tools import file_open

from nwos.addons.l10n_account_edi_ubl_cii_tests.tests.common import (
    TestUBLCommon,
)
from nwos.addons.l10n_ro_edi.models.account_move import HOLDING_DAYS
    ...
    answer_data = {
        ...
        '3030439533': {
            ...
            'invoice': {
                ...
                'attachment_raw': file_open("l10n_ro_edi/tests/test_files/from_nwos/ciusro_in_invoice.xml").read(),
            },
        },
        '3029027563': {
...
class TestUBLRO(TestUBLROCommon):
    ...
    def test_export_invoice(self):
        ...
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_nwos/ciusro_out_invoice.xml')
    ...
    def test_export_credit_note(self):
        ...
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_nwos/ciusro_out_refund.xml')

    def test_export_invoice_different_currency(self):
        invoice = self.create_move("out_invoice")
        attachment = self.get_attachment(invoice)
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_nwos/ciusro_out_invoice_different_currency.xml')
    ...
    def test_export_invoice_without_country_code_prefix_in_vat(self):
        ...
        invoice = self.create_move("out_invoice", currency_id=self.company.currency_id.id)
        attachment = self.get_attachment(invoice)
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_nwos/ciusro_out_invoice_no_prefix_vat.xml')
    ...
    def test_export_no_vat_but_have_company_registry_without_prefix(self):
        ...
        invoice = self.create_move("out_invoice", currency_id=self.company.currency_id.id)
        attachment = self.get_attachment(invoice)
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_nwos/ciusro_out_invoice_no_prefix_company_registry.xml')
    ...
    def test_export_invoice_no_vat_prefix(self):
        ...
        attachment = self.get_attachment(invoice)
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_nwos/ciusro_out_invoice_defaults.xml')
    ...
    def test_export_invoice_characters_limit(self):
        ...
        attachment = self.get_attachment(invoice)
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_nwos/ciusro_out_invoice_characters_limit.xml')

    ####################################################
    # Testing of the bill synchronization with SPV
    ####################################################

    @patch('nwos.addons.l10n_ro_edi.models.account_move._request_ciusro_synchronize_invoices', new=_patch_request_ciusro_synchronize_invoices)
    ...
    @patch('nwos.addons.l10n_ro_edi.models.account_move._request_ciusro_xml_to_pdf', new=_patch_request_ciusro_xml_to_pdf)
    @patch('nwos.addons.l10n_ro_edi.models.account_move._request_ciusro_synchronize_invoices', new=_patch_request_ciusro_synchronize_invoices)
