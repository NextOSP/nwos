...
class MailBot(models.AbstractModel):
    ...
    def _ai_get_local_answer(self, channel, body_text):
        if self._ai_is_clear_command(body_text):
            ...
            return self._ai_t(
                "The pending NextBot command in this conversation was cleared.",
                "Đã xóa lệnh NextBot đang chờ trong cuộc trò chuyện này.",
            )
        ...
        if self._ai_is_sale_quotation_product_search_query(body_text):
            return self._ai_answer_sale_quotation_product_search(body_text)
    ...
    def _ai_prepare_sale_quotation_arguments(self, arguments):
        if 'sale.order' not in self.env:
            return {
                'error': self._ai_t(
                    "Sales is not installed, so I cannot create a quotation.",
                    "Module Sales chưa được cài nên mình không tạo được báo giá.",
                ),
            }
        arguments = arguments if isinstance(arguments, dict) else {}
        missing = []
    ...
    def _ai_continue_pending_partner_source_tag(self, channel, body_text):
        ...
        if not source_name:
            self._ai_clear_pending_action(channel)
            return self._ai_t(
                "I lost the source name to apply. Please ask again.",
                "Mình đã mất tên nguồn cần gắn. Hãy yêu cầu lại.",
            )
        ...
        if not partner:
            ...
            return self._ai_partner_source_clarification_card(
                arguments,
                warning=self._ai_t(
                    "I could not find customer “%s”. Please enter the customer name again.",
                    "Không tìm thấy khách hàng “%s”. Hãy nhập lại tên khách hàng.",
                    partner_name,
                ),
            )
        ...
        for message in messages:
            ...
            if message.author_id == bot_partner:
                if (
                    ...
                    or "I do not know which customer needs this source" in message_text
                    or "Which customer is this source for" in message_text
                ):
        ...
        return Markup(
            ...
            <div class="border rounded p-3 bg-view o_nextbot_source_clarify_card mb-2">
                ...
                <div class="row g-2 small mb-3">
                    <div class="col-5 text-muted">%(source_label)s</div>
            ...
            'question': self._ai_t("Which customer is this source for?", "Nguồn này cho khách hàng nào?"),
            'hint': self._ai_t("Reply with the customer name.", "Trả lời bằng tên khách hàng."),
            'warning': warning_markup,
            'source_label': self._ai_t("Source", "Nguồn"),
    ...
    def _ai_is_sale_quotation_product_search_query(self, body_text):
        return bool(self._ai_extract_sale_quotation_product_search_text(body_text))

    def _ai_extract_sale_quotation_product_search_text(self, body_text):
        text = re.sub(r'\s+', ' ', str(body_text or '').strip())
        normalized = self._ai_normalize_text(text)
        if not re.search(r'\b(bao\s+gia|quotation|quote|sale\s+order)\b', normalized):
            return ''
        if not re.search(r'\b(chua|co|gom|include|includes|including|contain|contains|containing|with)\b', normalized):
            return ''
        text = re.sub(
            r'^(?:tìm|tim|find|search|xem|show|list|liệt\s+kê|liet\s+ke)\s+',
            '',
            text,
            flags=re.IGNORECASE,
        )
        patterns = [
            r'(?:có\s+chứa|co\s+chua|chứa|chua|có|co|gồm|gom)\s+(.+)$',
            r'(?:containing|contains|contain|including|includes|include|with)\s+(.+)$',
        ]
        query = ''
        for pattern in patterns:
            ...
            if match:
                ...
                break
        query = re.sub(
            r'^(?:sản\s+phẩm|san\s+pham|product|mặt\s+hàng|mat\s+hang|item)\s+',
            '',
            query,
            flags=re.IGNORECASE,
        )
    ...
    def _ai_partner_suggestion_buttons(self):
        ...
        return Markup("").join(
            self._ai_suggestion_button(
                ...
                self._ai_t("customer %s", "cho khách hàng %s", partner.display_name),
            )
    ...
    def _ai_product_suggestion_buttons(self, arguments):
        ...
        return Markup("").join(
            self._ai_suggestion_button(
                product.display_name,
                self._ai_t("product %s", "sản phẩm %s", product.display_name),
            )
            for product in products
        )
    ...
    def _ai_sale_quotation_clarification_card(self, arguments):
        ...
        if 'partner' in missing:
            question = self._ai_t("What customer is this quotation for?", "Báo giá này cho khách hàng nào?")
            hint = self._ai_t(
                "Reply with the customer name, for example: customer Anh Hung.",
                "Trả lời bằng tên khách hàng, ví dụ: cho Anh Hưng.",
            )
            suggestions = self._ai_partner_suggestion_buttons()
        elif 'product' in missing:
            question = self._ai_t(
                "Which product should I use for this quotation?",
                "Bạn muốn dùng sản phẩm nào cho báo giá?",
            )
            hint = self._ai_t(
                "Reply with the product name or internal reference.",
                "Trả lời bằng tên sản phẩm hoặc mã sản phẩm.",
            )
            suggestions = self._ai_product_suggestion_buttons(arguments)
        elif 'quantity' in missing:
            question = self._ai_t(
                "What quantity should I use for this quotation?",
                "Số lượng cho báo giá là bao nhiêu?",
            )
            hint = self._ai_t("Reply with a number.", "Trả lời bằng một con số.")
            suggestions = Markup("")
        else:
            question = self._ai_t(
                "What information do you want to add to this quotation?",
                "Bạn muốn bổ sung thông tin nào cho báo giá?",
            )
            hint = self._ai_t(
                "Reply with the missing information so NextBot can prepare the quotation.",
                "Trả lời thông tin còn thiếu để NextBot chuẩn bị báo giá.",
            )
            suggestions = Markup("")

        rows = []
        if arguments.get('partner_name'):
            rows.append((self._ai_t("Customer", "Khách hàng"), arguments['partner_name']))
        for index, line in enumerate(arguments.get('order_lines') or self._ai_sale_quotation_raw_lines(arguments), start=1):
            product_name = line.get('product_name') or line.get('product_query') or self._ai_t("No product selected", "Chưa chọn sản phẩm")
            quantity = line.get('quantity') or self._ai_t("No quantity yet", "Chưa có số lượng")
            rows.append((
                self._ai_t("Line %s", "Dòng %s", index),
                self._ai_t(
                    "%(product)s × %(quantity)s",
                    "%(product)s × %(quantity)s",
                    product=product_name,
                    quantity=self._ai_format_action_value(quantity),
                ),
            ))
        details = Markup("").join(
            Markup(
        ...
        if not details:
            details = Markup("<div class='col-12 text-muted'>%(empty)s</div>") % {
                'empty': self._ai_t("No confirmed details yet.", "Chưa có chi tiết chắc chắn."),
            }
    ...
    def _ai_sales_report_clarification_card(self):
        buttons = Markup("").join([
            self._ai_suggestion_button(
                self._ai_t("My today", "Hôm nay của tôi"),
                self._ai_t("my sales report today", "báo cáo doanh số hôm nay của tôi"),
                primary=True,
            ),
            self._ai_suggestion_button(
                self._ai_t("My week", "Tuần này của tôi"),
                self._ai_t("my sales report this week", "báo cáo doanh số tuần này của tôi"),
            ),
            self._ai_suggestion_button(
                self._ai_t("My month", "Tháng này của tôi"),
                self._ai_t("my sales report this month", "báo cáo doanh số tháng này của tôi"),
            ),
            self._ai_suggestion_button(
                self._ai_t("All this week", "Tất cả tuần này"),
                self._ai_t("sales report this week for all", "báo cáo doanh số tuần này tất cả"),
            ),
            self._ai_suggestion_button(
                self._ai_t("Export today", "Xuất file hôm nay"),
                self._ai_t("export my sales report today", "xuất báo cáo doanh số hôm nay của tôi"),
            ),
        ])
        return Markup(
            """
            ...
            'question': self._ai_t("Which report do you want?", "Bạn muốn báo cáo nào?"),
            'hint': self._ai_t(
                "Choose the report period and scope so NextBot runs the right data.",
    ...
    def _ai_sales_report_date_range(self, arguments):
        ...
        if period == 'yesterday':
            ...
            end_date = today
            label = self._ai_t("yesterday", "hôm qua")
        elif period == 'this_week':
            start_date = today - timedelta(days=today.weekday())
            end_date = today + timedelta(days=1)
            label = self._ai_t("this week", "tuần này")
        elif period == 'this_month':
            start_date = today.replace(day=1)
            end_date = today + timedelta(days=1)
            label = self._ai_t("this month", "tháng này")
            ...
            start_date = today
            end_date = today + timedelta(days=1)
            label = self._ai_t("today", "hôm nay")
    ...
    def _ai_sales_report_data(self, arguments):
        if 'sale.order' not in self.env:
            return {'error': self._ai_t(
                "Sales is not installed, so I cannot view sales revenue.",
                "Module Sales chưa được cài nên mình không xem được doanh số.",
            )}
    ...
    def _ai_answer_sales_report(self, body_text):
        ...
        if not rows:
            rows = Markup("<div class='border-top py-2 text-muted'>%(empty)s</div>") % {
                'empty': self._ai_t(
                    "There are no confirmed sales orders in this period.",
                    "Không có đơn bán đã xác nhận trong kỳ này.",
                ),
            }

        scope = self._ai_t("your orders", "của bạn") if data['own'] else self._ai_t(
            "all orders you can access",
            "tất cả đơn bạn có quyền xem",
        )
        return Markup(
            """
            <div class="border rounded p-3 bg-view o_nextbot_sales_report_card mb-2">
                ...
                <div class="small text-muted mb-2">%(count_label)s</div>
                %(rows)s
            </div>
            """
        ) % {
            'title': self._ai_t(
                "Sales report %(period)s",
                "Báo cáo doanh số %(period)s",
            ...
            'count_label': self._ai_t(
                "%(count)s confirmed sales orders",
                "%(count)s đơn bán đã xác nhận",
    ...
    def _ai_sales_report_text(self, data):
        lines = [
            self._ai_t("Sales report %s", "Báo cáo doanh số %s", data['period_label']),
            self._ai_t(
                "Scope: %s",
                "Phạm vi: %s",
                self._ai_t("your orders", "của bạn") if data['own'] else self._ai_t("all orders you can access", "tất cả đơn bạn có quyền xem"),
            ),
            self._ai_t("Timezone: %s", "Múi giờ: %s", data['timezone']),
            self._ai_t("Total sales: %s", "Tổng doanh số: %s", data['total_formatted']),
            self._ai_t("Confirmed sales orders: %s", "Số đơn bán đã xác nhận: %s", data['order_count']),
            "",
            self._ai_t("Details:", "Chi tiết:"),
        ]
        if not data['orders']:
            lines.append(self._ai_t(
                "- There are no confirmed sales orders in this period.",
                "- Không có đơn bán đã xác nhận trong kỳ này.",
            ))
        for order in data['orders']:
    ...
    def _ai_sale_quotation_customer_summary_data(self, body_text):
        if 'sale.order' not in self.env:
            return {'error': self._ai_t(
                "Sales is not installed, so I cannot view quotations.",
                "Module Sales chưa được cài nên mình không xem được báo giá.",
            )}
        ...
        for order in orders:
            ...
            if key not in grouped:
                grouped[key] = {
                    'partner': partner,
                    'customer': partner.display_name or self._ai_t("No customer", "Không có khách hàng"),
                    'orders': [],
                    'totals': {},
                }
            currency = order.currency_id or self.env.company.currency_id
    ...
    def _ai_answer_sale_quotation_customer_summary(self, body_text):
            ...
            rows.append(Markup(
                ...
                <div class="d-flex justify-content-between gap-3 border-top py-2">
                    <div>
                        ...
                        <div class="text-muted small">%(count_label)s · %(orders)s</div>
                    </div>
        ...
        if not rows:
            rows.append(Markup("<div class='border-top py-2 text-muted'>%(empty)s</div>") % {
                'empty': self._ai_t("There are no quotations in this period.", "Không có báo giá trong kỳ này."),
            })
        ...
        return Markup(
            ...
            <div class="border rounded p-3 bg-view o_nextbot_quotation_summary_card mb-2">
                <div class="d-flex justify-content-between align-items-start gap-3 mb-2">
                    ...
                    <div class="text-end">
                        <div class="fw-bold fs-4">%(customers_label)s</div>
                        <div class="small text-muted">%(quotations_label)s</div>
                    </div>
                </div>
                <div class="small text-muted mb-2">%(total_label)s</div>
                %(rows)s
            </div>
            """
        ) % {
            'title': self._ai_t("Quotations %(period)s", "Báo giá %(period)s", period=data['period_label']),
            'timezone': data['timezone'],
            'customers_label': self._ai_t(
                "%(count)s customers",
                "%(count)s khách",
                count=data['customer_count'],
            ),
            'quotations_label': self._ai_t(
                "%(count)s quotations",
                "%(count)s báo giá",
                count=data['quotation_count'],
            ),
            'total_label': self._ai_t(
                "Total quotation value: %(total)s",
                "Tổng giá trị báo giá: %(total)s",
                total=data['total_formatted'],
            ),
            'rows': Markup("").join(rows),
        }

    def _ai_sale_quotation_product_search_data(self, body_text):
        if 'sale.order' not in self.env:
            return {'error': self._ai_t(
                "Sales is not installed, so I cannot view quotations.",
                "Module Sales chưa được cài nên mình không xem được báo giá.",
        ...
        SaleOrder.check_access('read')
        SaleOrderLine.check_access('read')

        query = self._ai_extract_sale_quotation_product_search_text(body_text)
        if not query:
            return {'query': '', 'orders': []}
    ...
    def _ai_tool_sale_quotation_search(self, arguments):
        if 'sale.order' not in self.env:
            return {'error': 'Sales is not installed.'}
        query = str(arguments.get('product_query') or arguments.get('query') or '').strip()
        if not query:
            return {'error': 'Missing product_query.'}
        data = self._ai_sale_quotation_product_search_data(f"quotation containing {query}")
        if data.get('error'):
        ...
        for order in data['orders']:
            ...
            records.append({
                ...
                'customer': order.partner_id.display_name,
                'state': order.state,
                'state_label': data['state_labels'].get(order.state, order.state),
                ...
                'matching_lines': [{
                    ...
                    and self._ai_normalize_text(query) in self._ai_normalize_text(
                        " ".join([
    ...
    def _ai_answer_sale_quotation_product_search(self, body_text):
        ...
        orders = data.get('orders') or self.env['sale.order'].browse()
        ...
        for order in orders:
            ...
            matching_lines = order.order_line.filtered(
                lambda line: not line.display_type and (
                    self._ai_normalize_text(query) in self._ai_normalize_text(line.product_id.display_name)
                    or self._ai_normalize_text(query) in self._ai_normalize_text(getattr(line.product_id, 'default_code', ''))
                    or self._ai_normalize_text(query) in self._ai_normalize_text(line.name)
                )
            )
            line_text = ", ".join(
                self._ai_t(
                    ...
                    "%(product)s × %(quantity)s",
                    product=line.product_id.display_name or line.name,
                    quantity=self._ai_format_quantity(line.product_uom_qty),
                )
                for line in matching_lines[:3]
            ) or self._ai_t("Matching quotation line", "Dòng báo giá phù hợp")
            rows.append(Markup(
                """
                <div class="d-flex justify-content-between gap-3 border-top py-2">
                    <div>
                        <div class="fw-bold">%(order)s</div>
                        <div class="text-muted small">%(customer)s · %(state)s</div>
                        <div class="small">%(lines)s</div>
                    </div>
                    <div class="fw-bold text-end">%(amount)s</div>
                </div>
                """
            ) % {
                'order': self._ai_record_modal_link('sale.order', order, label=order.name, button=False),
                'customer': order.partner_id.display_name,
                'state': data['state_labels'].get(order.state, order.state),
                'lines': line_text,
                'amount': currency.format(order.amount_total),
            })

        if not rows:
            rows.append(Markup("<div class='border-top py-2 text-muted'>%(empty)s</div>") % {
                'empty': self._ai_t(
                    "No draft/sent quotations contain “%s”.",
                    "Không tìm thấy báo giá nháp/đã gửi nào chứa “%s”.",
                    query,
                ),
            })
        ...
        return Markup(
            """
            <div class="border rounded p-3 bg-view o_nextbot_quotation_search_card mb-2">
                <div class="d-flex justify-content-between align-items-start gap-3 mb-2">
                    <div>
                        <div class="fw-bold">%(title)s</div>
                        <div class="text-muted small">%(query_label)s: %(query)s</div>
                    </div>
                    <div class="fw-bold text-end">%(count_label)s</div>
                </div>
                %(rows)s
            </div>
            """
        ) % {
            'title': self._ai_t("Matching quotations", "Báo giá phù hợp"),
            'query_label': self._ai_t("Contains", "Chứa"),
            'query': query,
            'count_label': self._ai_t("%(count)s found", "%(count)s kết quả", count=len(orders)),
            'rows': Markup("").join(rows),
        }
    ...
    def _ai_answer_sale_quotation_query(self):
        ...
        if not orders:
            return self._ai_t(
                "There are no draft/sent quotations in the records you can access.",
                "Chưa có báo giá nháp/đã gửi nào trong các bản ghi bạn có quyền xem.",
            )
        cards = Markup("").join(self._ai_sale_quotation_card({}, order=order) for order in orders)
        return Markup(
            """
            <div class="o_nextbot_quotation_list">
                <div class="fw-bold mb-2">%(title)s</div>
                %(cards)s
            </div>
            """
        ) % {
            'title': self._ai_t("Latest draft/sent quotations", "Các báo giá nháp/đã gửi gần nhất"),
            'cards': cards,
        }
    ...
    def _ai_get_answer(self, channel, values, body_text, message=None):
        ...
        try:
            ...
            if tool_choice:
                return self._ai_t(
                    "AI did not return enough information to prepare the quotation. Try again with customer, product, and quantity.",
    ...
    def _ai_execute_tool(self, name, arguments):
        ...
        if name == 'sale_quotation_search':
            return self._ai_tool_sale_quotation_search(arguments)
    ...
    def _ai_clean_values(self, values):
        if not isinstance(values, dict):
            raise UserError(self._ai_t("Updated values must be a JSON object.", "Giá trị cập nhật phải là JSON object."))
        clean = {}
        for key, value in values.items():
            ...
            if isinstance(value, (str, int, float, bool)) or value is None:
                ...
                raise UserError(self._ai_t(
                    "Value for field %s is not supported. Use only simple values.",
                    "Giá trị cho trường %s chưa được hỗ trợ. Chỉ dùng giá trị đơn giản.",
                    key,
                ))
        return clean
    ...
    def _ai_prepare_partner_source_tag_arguments(self, arguments):
        ...
        if not partner:
            return {'error': self._ai_t(
                "Could not find the customer to set the source.",
                "Không tìm thấy khách hàng để gắn nguồn.",
            )}
        partner.check_access('write')
        source_name = str(arguments.get('source_name') or '').strip(' "\'`.,:;')
        if not source_name:
            return {'error': self._ai_t("Missing customer source name.", "Thiếu tên nguồn khách hàng.")}
        return {
            'model': 'res.partner',
            'res_id': partner.id,
            'partner_id': partner.id,
    ...
    def _ai_prepare_pending_action(self, channel, tool_name, arguments):
        ...
        if isinstance(summary, Markup):
            return summary + Markup("<div class='small text-muted mt-2'>%(message)s</div>") % {
                'message': Markup(self._ai_t(
                    "Type <b>ok</b>, <b>do it</b>, or <b>confirm</b> within 15 minutes to run it. "
                    "Type <b>cancel</b> or <b>/clear</b> to discard.",
                    "Gõ <b>ok</b>, <b>ok làm đi</b> hoặc <b>confirm</b> trong 15 phút để chạy. "
                    "Gõ <b>hủy</b> hoặc <b>/clear</b> để bỏ.",
                )),
            }
        return self._ai_t(
            "%s\n\nType `confirm` within 15 minutes to run it, or `cancel` to discard.",
            "%s\n\nGõ `confirm` trong 15 phút để chạy, hoặc `cancel` để bỏ.",
            summary,
        )
    ...
    def _ai_model_label(self, model_name):
        labels = {
            'res.partner': self._ai_t("customer", "khách hàng"),
            'product.product': self._ai_t("product", "sản phẩm"),
            'sale.order': self._ai_t("quotation", "báo giá"),
        }
        if model_name in labels:
            return labels[model_name]
        if model_name in self.env:
            return (self.env[model_name]._description or model_name).lower()
        return model_name or self._ai_t("record", "bản ghi")
    ...
    def _ai_format_action_value(self, value):
        if value is True:
            return self._ai_t("Yes", "Có")
        if value is False or value is None:
            return self._ai_t("Empty", "Trống")
    ...
    def _ai_record_action_rows(self, Model, values):
        ...
        return Markup("").join(rows) or Markup(
            "<div class='col-12 text-muted'>%(empty)s</div>"
        ) % {'empty': self._ai_t("No detailed values.", "Không có giá trị chi tiết.")}
    ...
    def _ai_record_modal_link(self, model_name, record, label=None, button=True):
        ...
        return Markup(
            ...
            'model': model_name,
            'record_id': record.id,
            'label': label or self._ai_t("Open record", "Mở bản ghi"),
        }
    ...
    def _ai_record_action_card(self, action, pending=False, record=None, attachment=None):
        ...
        if tool == 'prepare_create_record':
            title = self._ai_t("Prepare creating %s", "Chuẩn bị tạo %s", model_label) if pending else self._ai_t("Created %s", "Đã tạo %s", model_label)
            rows = self._ai_record_action_rows(Model, arguments.get('values') or {})
        elif tool == 'prepare_update_record':
            title = self._ai_t("Prepare updating %s", "Chuẩn bị cập nhật %s", model_label) if pending else self._ai_t("Updated %s", "Đã cập nhật %s", model_label)
            rows = self._ai_record_action_rows(Model, arguments.get('values') or {})
        elif tool == 'prepare_post_comment':
            title = self._ai_t("Prepare posting a note on %s", "Chuẩn bị ghi chú trên %s", model_label) if pending else self._ai_t("Posted a note on %s", "Đã ghi chú trên %s", model_label)
            rows = Markup(
                """
                <div class="col-5 text-muted">%(label)s</div>
                <div class="col-7 o_nextbot_action_value">%(body)s</div>
                """
            ) % {'label': self._ai_t("Content", "Nội dung"), 'body': arguments.get('body') or ''}
        elif tool == 'prepare_text_attachment':
            title = self._ai_t("Prepare attaching file", "Chuẩn bị đính kèm tệp") if pending else self._ai_t("Attached file", "Đã đính kèm tệp")
            rows = Markup(
                """
                <div class="col-5 text-muted">%(label)s</div>
                <div class="col-7 o_nextbot_action_value">%(name)s</div>
                """
            ) % {'label': self._ai_t("File name", "Tên tệp"), 'name': (attachment.name if attachment else arguments.get('name')) or 'nextbot.txt'}
        elif tool == 'prepare_partner_source_tag':
            title = self._ai_t("Prepare setting customer source", "Chuẩn bị gắn nguồn khách hàng") if pending else self._ai_t("Set customer source", "Đã gắn nguồn khách hàng")
            rows = Markup(
                """
                <div class="col-5 text-muted">%(label)s</div>
                <div class="col-7 o_nextbot_action_value">%(source)s</div>
                """
            ) % {'label': self._ai_t("Source", "Nguồn"), 'source': arguments.get('source_name') or ''}
        else:
            title = self._ai_t("Prepare action", "Chuẩn bị thao tác") if pending else self._ai_t("Action completed", "Đã chạy thao tác")
            rows = Markup("<div class='col-12 text-muted'>%(tool)s</div>") % {'tool': tool or ''}

        reference = record.display_name if record else (model_name or self._ai_t("New record", "Bản ghi mới"))
        status = self._ai_t("Waiting for confirmation", "Chờ xác nhận") if pending else self._ai_t("Done", "Hoàn tất")
        badge_class = "text-bg-warning" if pending else "text-bg-success"
        button_label = self._ai_t("Open %s", "Mở %s", model_label)
        ...
        if attachment:
            attachment_link = Markup(
                ...
                <a href="/web/content/%(attachment_id)s?download=true"
                   class="btn btn-secondary btn-sm" target="_blank">%(label)s</a>
                """
            ) % {'attachment_id': attachment.id, 'label': self._ai_t("Download file", "Tải tệp")}
    ...
    def _ai_handle_confirmation(self, channel, body):
        ...
        if pending.get('expired'):
            return self._ai_t(
                "The pending NextBot command has expired. Please ask me to prepare it again.",
                "Lệnh NextBot đang chờ đã hết hạn. Hãy yêu cầu mình chuẩn bị lại.",
            )
        ...
        if body in AI_CANCEL_WORDS:
            self._ai_clear_pending_action(channel)
            return self._ai_t("The pending NextBot command was cancelled.", "Đã hủy lệnh NextBot đang chờ.")
        ...
        try:
            ...
            self._ai_clear_pending_action(channel)
            return self._ai_t("I could not run the pending command: %s", "Mình không chạy được lệnh đang chờ: %s", error)
    ...
    def _ai_execute_pending_action(self, action, channel=None):
        ...
        if tool == 'prepare_post_comment':
            if not hasattr(record, 'message_post'):
                raise UserError(self._ai_t(
                    "This model does not support chatter notes.",
                    "Model này không hỗ trợ ghi chú chatter.",
                ))
            record.check_access('read')
            record.message_post(
        ...
        raise UserError(self._ai_t("Unsupported pending action: %s", "Thao tác đang chờ chưa được hỗ trợ: %s", tool))

    def _ai_execute_create_sale_quotation(self, arguments, channel=None):
        if 'sale.order' not in self.env:
            raise UserError(self._ai_t(
                "Sales is not installed, so I cannot create a quotation.",
                "Module Sales chưa được cài nên không tạo được báo giá.",
            ))
        ...
        if arguments.get('error'):
            raise UserError(arguments['error'])
        if arguments.get('missing'):
            raise UserError(self._ai_t("The quotation is still missing information.", "Báo giá còn thiếu thông tin."))
        ...
        for line in arguments.get('order_lines') or []:
            ...
            if not product:
                raise UserError(self._ai_t(
                    "Could not find the product to create the quotation.",
                    "Không tìm thấy sản phẩm để tạo báo giá.",
                ))
            product.check_access('read')
            quantity = float(line.get('quantity') or 0.0)
            if quantity <= 0:
                raise UserError(self._ai_t("Quantity must be greater than 0.", "Số lượng phải lớn hơn 0."))
            line_values = {
        ...
        if not line_commands:
            raise UserError(self._ai_t(
                "The quotation needs at least one product line.",
                "Báo giá cần ít nhất một dòng sản phẩm.",
            ))
    ...
    def _ai_sale_quotation_card(self, arguments, pending=False, order=None, partner=None, product=None):
        ...
        state_label = self._ai_t("Ready to create", "Sẵn sàng tạo") if pending else dict(order._fields['state'].selection).get(order.state, order.state)
        title = self._ai_t("Prepare quotation", "Chuẩn bị báo giá") if pending else self._ai_t("Created quotation", "Đã tạo báo giá")
        reference = self._ai_t("Draft", "Bản nháp") if pending else order.name
        open_link = ""
        if order:
            ...
            open_link = Markup(
                ...
                'url': url,
                'order_id': order.id,
                'label': self._ai_t("Open quotation", "Mở báo giá"),
            }
        else:
            open_link = Markup("<span class='badge text-bg-warning'>%(label)s</span>") % {
                'label': self._ai_t("Waiting for confirmation", "Chờ xác nhận"),
            }
        ...
        ) or Markup("<div class='border-top py-2 text-muted small'>%(empty)s</div>") % {
            'empty': self._ai_t("No product lines yet.", "Chưa có dòng sản phẩm."),
        }
        ...
        return Markup(
            ...
            <div class="border rounded p-3 bg-view o_nextbot_quotation_card mb-2">
                ...
                <div class="row g-2 small mb-3">
                    <div class="col-5 text-muted">%(customer_label)s</div>
            ...
            'customer_label': self._ai_t("Customer", "Khách hàng"),
