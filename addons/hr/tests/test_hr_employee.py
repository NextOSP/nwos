...
from nwos.tests import Form, users, new_test_user, HttpCase, tagged, TransactionCase
from nwos.addons.hr.tests.common import TestHrCommon
...
class TestHrEmployee(TestHrCommon):
    ...
    def test_employee_timezone(self):
        ...
        with mute_logger('nwos.sql_db'), self.assertRaises(NotNullViolation):
            employee.tz = False
        ...
        user_1, user_2, user_3 = self.env['res.users'].create([
            {
                'name': 'Test User',
                'login': 'test_user',
                'email': 'test_user@nwos.com',
            },
            {
                'name': 'Test User 2',
                'login': 'test_user_2',
                'email': 'test_user_2@nwos.com',
                'create_employee': True,
            },
            {
                'name': 'Test User 3',
                'login': 'test_user_3',
                'email': 'test_user_3@nwos.com',
    ...
    def test_set_user_on_new_employee(self):
        ...
        employee_form.user_id = self.res_users_hr_officer
        with mute_logger('nwos.sql_db'), self.assertRaises(UniqueViolation), self.assertRaises(ValidationError):
            employee_form.save()
    ...
    def test_search_hr_employee_no_access(self):
        ...
        with self.assertNoLogs('nwos.domains'):
            self.assertEqual(
                employee.ids,
...
class TestHrEmployeeLinks(HttpCase):
    def test_shared_private_link_permissions(self):
        ...
        with mute_logger('nwos.http'):  # ignore raised RedirectWarning
            self.start_tour(
                f"/nwos/employees/{employee_sonic.id}",
