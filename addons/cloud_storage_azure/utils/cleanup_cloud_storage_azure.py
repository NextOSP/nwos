...
def get_blobs_to_be_deleted(blob_urls, batch_size=1000):
    common = xmlrpc.client.ServerProxy(f'{nwos_url}/xmlrpc/2/common')
    uid = common.authenticate(nwos_db, nwos_username, nwos_password, {})
    models = xmlrpc.client.ServerProxy(f'{nwos_url}/xmlrpc/2/object')
    for blob_urls_ in split_every(batch_size, blob_urls):
        blob_urls_ = list(blob_urls_)
        attachments = models.execute_kw(nwos_db, uid, nwos_password, 'ir.attachment', 'search_read', [
