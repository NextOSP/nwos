...
from nwos.addons.project.tests.test_project_base import TestProjectCommon
from nwos.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE
from nwos.tests import tagged, users, new_test_user
...
class TestProjectMailFeatures(TestProjectCommon, MailCommon):
    ...
    def setUp(self):
        super().setUp()
        with mute_logger('nwos.addons.mail.models.mail_thread'):
    ...
    def test_task_creation_notifies_author(self):
        ...
        for test_user in (self.user_employee, self.user_portal, False):
            with self.subTest(user_name=test_user.name if test_user else new_partner_email):
                ...
                # task created by nwosbot if not incoming user -> nwosbot author of ack email
