...
from nwos import api, fields, models, tools, SUPERUSER_ID, _
from nwos.fields import Command, Date, Domain
from nwos.addons.rating.models import rating_data
from nwos.addons.html_editor.tools import handle_history_divergence
from nwos.exceptions import UserError, ValidationError
from nwos.tools import format_list, SQL, LazyTranslate, html_sanitize
from nwos.addons.resource.models.utils import filter_domain_leaf
from nwos.addons.project.controllers.project_sharing_chatter import ProjectSharingChatter
...
class ProjectTask(models.Model):
    ...
    # Need this field to check there is no email loops when NWOS reply automatically
    ...
    def _notify_by_email_get_headers(self, headers=None):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-NWOS-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-NWOS-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-NWOS-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
    ...
    def action_redirect_to_project_task_form(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': f"/nwos/{self.project_id.id}/action-project.act_project_project_2_project_task_all/{self.id}?menu_id={menu_id}",
            'target': 'new',
        }
