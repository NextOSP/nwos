...
import { markRaw, reactive } from "@nwos/owl";
...
export class PosData {
    ...
    async checkConnectivity() {
        try {
            ...
            // if the NWOS server is still accessible.
            //
            // This method also makes it possible to run local tests when no connection is
            // available and an NWOS server is running locally.
            //
    ...
    initializeWebsocket() {
        this.onNotified = getOnNotified(this.bus, nwos.access_token);
    }
    ...
    get databaseName() {
        return `point-of-sale-${nwos.pos_config_id}-${nwos.info?.db}`;
    }
        ...
        if (
            ...
            try {
                ...
                const data = await this.orm.call(
                    "pos.session",
                    "load_data",
                    [nwos.pos_session_id, PosData.modelToLoad],
                    {
                        context: {
    ...
    async loadFieldsAndRelations() {
        const key = `pos_data_params_${nwos.pos_config_id}`;
            ...
            try {
                if (["product.product", "product.template"].includes(model)) {
                    ...
                    await this.callRelated(
                        ...
                        [nwos.pos_config_id, [[domain, "in", Array.from(ids)]], 0, 0],
                        {
                            context: {
                                load_archived: true,
                            },
    ...
    async loadServerOrders(domain) {
        ...
        const config = this.models["pos.config"].get(nwos.pos_config_id);
        const session = this.models["pos.session"].get(nwos.pos_session_id);
