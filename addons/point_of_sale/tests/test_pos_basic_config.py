...
from nwos.addons.point_of_sale.tests.common import TestPoSCommon
from nwos.exceptions import ValidationError
from freezegun import freeze_time
from dateutil.relativedelta import relativedelta
from datetime import datetime, timedelta
import unittest.mock
from nwos.http import UserError
...
class TestPoSBasicConfig(TestPoSCommon):
    ...
    def test_cash_register_if_no_order(self):
        ...
        with (
            self.assertLogs('nwos.addons.point_of_sale.models.pos_order') as cm,
            unittest.mock.patch('nwos.addons.point_of_sale.models.pos_order.randrange', return_value=1996)
        ):
            ...
            nwos_order_id = res['pos.order'][0]['id']
            self.assertEqual(len(cm.output), 4)
            self.assertEqual(cm.output[0], f"INFO:nwos.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 started for PoS orders references: [{order_log_str}]")
            self.assertTrue(cm.output[1].startswith(f'INFO:nwos.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 processing order {order_log_str} order full data:'))
            self.assertEqual(cm.output[2], f'INFO:nwos.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 order {order_log_str} created pos.order #{nwos_order_id}')
            self.assertEqual(cm.output[3], 'INFO:nwos.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 finished')
