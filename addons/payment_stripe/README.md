...
- `16.4`
  - The previous Checkout API that allowed for redirect payments is replaced by the Payment Intents
    API that supports direct payments. nwos/nwos#123573
  - The support for eMandates for recurring payments is added. nwos/nwos#123573
  - The responses of webhook notifications are sent with the proper HTTP code. nwos/nwos#117940
- `16.0`
  - Stripe uses the payment methods set up on the account when none are assigned to the payment
    provider in NWOS, instead of only offering the "Card" payment method. nwos/nwos#107647
  - The support for express checkout is added. nwos/nwos#88374
- `15.4`
  - The support for full and partial refunds is added. nwos/nwos#92235
- `15.3`
  - Webhook notifications accept three new events based on the PaymentIntent and SetupIntent objects
    ...
    nwos/nwos#84150
  - The support for manual capture is added. nwos/nwos#69598
- `15.2`
...
- `15.0`
  - A new button is added to create a webhook automatically. nwos/nwos#79621
  - The support for the Stripe Connect onboarding flow is added. nwos/nwos#79621
- `14.3`
  - The previous direct payment flow that was supported by the SetupIntent API is replaced by a
    payment with redirection flow using the Checkout API. nwos/nwos#141661
...
createdb nwos
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -i payment_stripe
```
...
```bash
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -u payment_stripe
```
