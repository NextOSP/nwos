...
from nwos.tools import SQL, mute_logger
...
class PasskeyTest(HttpCaseWithUserDemo):
    ...
    def test_check_identity(self):
        for key in ['test-yubikey', 'test-yubikey-nano', 'test-keepassxc', 'test-android']:
            ...
            with self.patch_start_auth(webauthn_challenge):
                ...
                self.assertEqual(response['error']['data']['name'], 'nwos.exceptions.UserError')
                self.assertEqual(
    ...
    def test_check_user_verification(self):
        ...
        with self.patch_start_auth(webauthn_challenge):
            ...
            # Login successful, redirected to /nwos
            self.assertTrue(response.url.endswith('/nwos'))
...
class PasskeyTestTours(PasskeyTest):
    ...
    def test_passkey_backend(self):
        ...
        with self.patch_start_registration(self.passkeys['test-yubikey']['registration']['challenge']):
            self.start_tour("/nwos?debug=tests", 'passkeys_tour_registration', login="admin")
        with self.patch_start_auth(self.passkeys['test-yubikey']['auth']['challenge']):
            self.start_tour("/nwos?debug=tests", 'passkeys_tour_verify', login="admin")
        self.start_tour("/nwos?debug=tests", 'passkeys_tour_delete', login="admin")
