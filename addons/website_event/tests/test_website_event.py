...
from nwos import fields, http
from nwos.addons.base.tests.common import HttpCaseWithUserDemo, HttpCaseWithUserPortal
...
from nwos.addons.website_event.tests.common import TestEventOnlineCommon, OnlineEventCase
...
class TestEventRegisterUTM(HttpCase, TestEventOnlineCommon):
    def test_event_registration_utm_values(self):
        ...
        self.opener.cookies.update({
            'nwos_utm_campaign': event_campaign.name,
            'nwos_utm_source': self.env.ref('utm.utm_source_newsletter').name,
            'nwos_utm_medium': self.env.ref('utm.utm_medium_email').name
        })
