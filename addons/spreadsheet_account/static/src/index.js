...
featurePluginRegistry.add("nwosAccountingAggregates", AccountingPlugin);
...
cellMenuRegistry.add("move_lines_see_records", {
    ...
    async execute(env, newWindow) {
        ...
        if ( func.functionName === "NWOS.PARTNER.BALANCE" ) {
            [partner_ids, codes, date_range, offset, companyId, includeUnposted] = parsed_args;
        } else if ( func.functionName === "NWOS.BALANCE.TAG" ) {
        ...
        if ( date_range?.value && !isEvaluationError(date_range.value) ) {
            ...
            if ( ["NWOS.PARTNER.BALANCE", "NWOS.RESIDUAL", "NWOS.BALANCE.TAG"].includes(func.functionName) ) {
        ...
        if ( func.functionName === "NWOS.BALANCE.TAG" ) {
        ...
        } else if ( func.functionName === "NWOS.PARTNER.BALANCE" ) {
