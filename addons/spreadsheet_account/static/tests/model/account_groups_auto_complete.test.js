...
test("NWOS.ACCOUNT.GROUP type", async function () {
    const { store: composer } = await makeStore(CellComposerStore);

    composer.startEdition("=NWOS.ACCOUNT.GROUP(");
    ...
    await animationFrame();
    expect(composer.currentContent).toBe('=NWOS.ACCOUNT.GROUP("asset_receivable"');
