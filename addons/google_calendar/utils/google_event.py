...
class GoogleEvent(abc.Set):
    ...
    Inspired by NWOS recordset, one instance can be a single Google event or a
    ...
    def nwos_id(self, env):
        self.nwos_ids(env)  # load ids
        return self._nwos_id

    def _meta_nwos_id(self, dbname):
        """Returns the NWOS id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_nwos_id' % dbname)
        if o_id:
            return int(o_id)

    def nwos_ids(self, env):
        ids = tuple(e._nwos_id for e in self if e._nwos_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_nwos_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_nwos_ids_from_metadata(env, model)

        return tuple(e._nwos_id for e in self)

    def _load_nwos_ids_from_metadata(self, env, model):
        unsure_nwos_ids = tuple(e._meta_nwos_id(env.cr.dbname) for e in self)
        nwos_events = model.browse(_id for _id in unsure_nwos_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same NWOS id.
        # Therefore, we only consider NWOS records without google id when trying
        # to match events.
        o_ids = nwos_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            nwos_id = e._meta_nwos_id(env.cr.dbname)
            if nwos_id in o_ids:
                e._events[e.id]['_nwos_id'] = nwos_id

    def _load_nwos_ids_from_db(self, env, model):
        nwos_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in nwos_events}  # {google_id: nwos_id}
        existing_google_ids = nwos_events.mapped('google_id')
        for e in self:
            nwos_id = mapping.get(e.id)
            if nwos_id:
                e._events[e.id]['_nwos_id'] = nwos_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and NWOS.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in NWOS (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in NWOS) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in NWOS several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_nwos_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.nwos_ids(env)
        events.nwos_ids(env)

        return self.filter(lambda e: e._nwos_id)
    ...
    def get_nwos_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.nwos_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.nwos_id(self.env)).base_event_id
