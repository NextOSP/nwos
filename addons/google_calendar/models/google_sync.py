...
from nwos.tools import email_normalize
from nwos.sql_db import BaseCursor

from nwos.addons.google_calendar.utils.google_event import GoogleEvent
from nwos.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from nwos.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the NWOS database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in NWOS.
...
class GoogleCalendarSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on NWOS DB (_sync_google2nwos).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_nwos2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2nwos(self, google_events: GoogleEvent, write_dates=None, default_reminders=()):
        """Synchronize Google recurrences in NWOS. Creates new recurrences, updates
        existing ones.

        :param google_events: Google recurrences to synchronize in NWOS
        :param write_dates: A dictionary mapping NWOS record IDs to their write dates.
        :param default_reminders:
        :return: synchronized nwos recurrences
        """
        ...
        nwos_values = [
            dict(self._nwos_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_nwos = self.with_context(dont_notify=True, skip_contact_description=True)._create_from_google(new, nwos_values)
        cancelled = existing.cancelled()
        cancelled_nwos = self.browse(cancelled.nwos_ids(self.env))

        # Check if it is a recurring event that has been rescheduled.
        # We have to check if an event already exists in NWOS.
        # Explanation:
        ...
        if rescheduled_events:
            ...
            cancelled_nwos += self.env['calendar.event'].search([('google_id', 'in', google_ids_to_remove)])

        cancelled_nwos.exists()._cancel()
        synced_records = new_nwos + cancelled_nwos
        pending = existing - cancelled
        pending_nwos = self.browse(pending.nwos_ids(self.env)).exists()
        for gevent in pending:
            nwos_record = self.browse(gevent.nwos_id(self.env))
            if nwos_record not in pending_nwos:
                # The record must have been deleted in the mean time; nothing left to sync
                continue
            # Last updated wins.
            # This could be dangerous if google server time and nwos server time are different
            updated = parse(gevent.updated)
            # Use the record's write_date to apply Google updates only if they are newer than NWOS's write_date.
            nwos_record_write_date = write_dates.get(nwos_record.id, nwos_record.write_date)
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not nwos_record_write_date or updated >= pytz.utc.localize(nwos_record_write_date):
                vals = dict(self._nwos_values(gevent, default_reminders), need_sync=False)
                nwos_record.with_context(dont_notify=True)._write_from_google(gevent, vals)
                synced_records |= nwos_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from NWOS to Google
    ...
    def _check_any_records_to_sync(self):
        """ Returns True if there are pending records to be synchronized from NWOS to Google, False otherwise. """
        is_active_clause = Domain(self._active_name, '=', True) if self._active_name else Domain.TRUE
        domain = self._get_sync_domain()
        domain &= (Domain('google_id', '=', False) & is_active_clause) | Domain('need_sync', '=', True)
    ...
    @api.model
    def _nwos_values(self, google_event: GoogleEvent, default_reminders=()):
    ...
    def _is_google_insertion_blocked(self, sender_user):
        ...
        This is a necessary step for ensuring data match between NWOS and Google,
