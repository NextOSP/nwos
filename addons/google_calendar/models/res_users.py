...
from nwos import api, fields, models, Command
from nwos.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
from nwos.addons.google_calendar.models.google_sync import google_calendar_token
from nwos.addons.google_account.models import google_service
from nwos.exceptions import LockError
from nwos.loglevels import exception_to_unicode
from nwos.tools import str2bool
...
class ResUsers(models.Model):
    ...
    def _check_pending_nwos_records(self):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        if not events and not self._check_pending_nwos_records():
            return False
        # Google -> NWOS
        ...
        # We apply Google updates only if their write date is later than the write date in NWOS.
        # It's possible that multiple updates affect the same record, maybe not directly.
        # To handle this, we preserve the write dates in NWOS before applying any updates,
        # and use these dates instead of the current live dates.
        nwos_events = self.env['calendar.event'].browse((events - recurrences).nwos_ids(self.env))
        nwos_recurrences = self.env['calendar.recurrence'].browse(recurrences.nwos_ids(self.env))
        recurrences_write_dates = {r.id: r.write_date for r in nwos_recurrences}
        events_write_dates = {e.id: e.write_date for e in nwos_events}
        synced_recurrences = self.env['calendar.recurrence']._sync_google2nwos(recurrences, recurrences_write_dates)
        synced_events = self.env['calendar.event']._sync_google2nwos(events - recurrences, events_write_dates, default_reminders=default_reminders)

        # NWOS -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_nwos2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_nwos2google(calendar_service)
    ...
    def _sync_google_calendar_filter_remote_events(self, google_events):
        """Filter out events coming from google which should not be synced into nwos."""
    ...
    def _sync_single_event(self, calendar_service: GoogleCalendarService, nwos_event, event_id):
        self.ensure_one()
        ...
        synced_events = self.env['calendar.event']._sync_google2nwos(event, default_reminders=default_reminders)
        # NWOS -> Google
        nwos_event.with_context(send_updates=send_updates)._sync_nwos2google(calendar_service)
        return bool(nwos_event | synced_events)
