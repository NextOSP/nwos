...
from nwos.tests import JsonRpcException, tagged
from nwos.tools import mute_logger

from nwos.addons.account_payment.tests.common import AccountPaymentCommon
...
from nwos.addons.sale.controllers.portal import CustomerPortal
...
class TestSalePayment(AccountPaymentCommon, MailCase, PaymentHttpCommon, SaleCommon):
    ...
    @mute_logger('nwos.http', 'werkzeug')
    ...
    def test_auto_confirm_and_auto_invoice(self):
        ...
        tx = self._create_transaction(flow='redirect', sale_order_ids=[self.sale_order.id], state='done')
        with (
            mute_logger('nwos.addons.sale.models.payment_transaction'),
    ...
    def test_auto_done_and_auto_invoice(self):
        ...
        self.amount = self.sale_order.amount_total
        tx = self._create_transaction(flow='redirect', sale_order_ids=[self.sale_order.id], state='done')
        with mute_logger('nwos.addons.sale.models.payment_transaction'):
            tx._post_process()
    ...
    def test_invoice_is_final(self):
        ...
        with mute_logger('nwos.addons.sale.models.payment_transaction'), patch(
            'nwos.addons.sale.models.sale_order.SaleOrder._create_invoices',
    ...
    def test_transaction_route_rejects_unexpected_kwarg(self):
        ...
        with self.assertRaises(JsonRpcException, msg='nwos.exceptions.ValidationError'):
    ...
    def test_partial_payment_confirm_order(self):
        ...
        with patch(
            'nwos.addons.sale.models.sale_order.SaleOrder._send_order_notification_mail',
