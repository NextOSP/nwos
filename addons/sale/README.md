...
From quotes to invoices, in just a few clicks with the NWOS <a href="https://www.nwos.com/app/crm">Sales Management</a>.
...
price; NWOS supports all possible methods.
...
createdb nwos
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -i sale
```
...
```bash
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -u sale
```
