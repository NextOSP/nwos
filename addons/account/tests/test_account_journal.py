...
from nwos.tools import hash_sign
from nwos.addons.account.tests.common import AccountTestInvoicingCommon
from nwos.addons.account.models.account_payment_method import AccountPaymentMethod
from nwos.addons.mail.tests.common import MailCommon
from nwos.tests import Form, tagged, HttpCase, new_test_user
from nwos.addons.test_mail.data.test_mail_data import MAIL_EML_ATTACHMENT
from nwos.exceptions import UserError, ValidationError
from nwos import fields, Command
...
class TestAccountJournal(AccountTestInvoicingCommon, HttpCase):
    ...
    def test_legacy_odoo_payment_reference_generation(self):
        """Existing databases can still contain the former 'odoo' reference model."""
        journal = self.company_data['default_journal_sale']
        self.env.cr.execute(
            "UPDATE account_journal SET invoice_reference_model = %s WHERE id = %s",
            ('odoo', journal.id),
        )
        journal.invalidate_recordset(['invoice_reference_model'])
    ...
    def test_payment_reference_error_names_unsupported_settings(self):
        journal = self.company_data['default_journal_sale']
        self.env.cr.execute(
            "UPDATE account_journal SET invoice_reference_model = %s WHERE id = %s",
            ('unsupported', journal.id),
        )
        journal.invalidate_recordset(['invoice_reference_model'])

        invoice = self.init_invoice("out_invoice", products=self.product_a)
        invoice.journal_id = journal
        with self.assertRaisesRegex(UserError, 'cannot generate a payment reference') as error:
            invoice._get_invoice_computed_reference()

        message = str(error.exception)
        self.assertIn(journal.display_name, message)
        self.assertIn('Communication Standard: unsupported', message)
        self.assertIn('Communication Type: Based on Invoice', message)
...
class TestAccountJournalAlias(AccountTestInvoicingCommon, MailCommon):
    ...
    def test_alias_name_creation(self):
        ...
        characters removal. See nwos/nwos@339cdffb68f91eb1455d447d1bdd7133c68723bd """
    ...
    def test_alias_from_type(self):
        ...
        well as defaults update, see nwos/nwos@400b6860271a11b9914166ff7e42939c4c6192dc """
        journal = self.company_data['default_journal_purchase']
