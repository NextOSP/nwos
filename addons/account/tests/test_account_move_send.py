...
from nwos.addons.mail.tests.common import MailCommon
from nwos.exceptions import UserError
from nwos.tests import users, warmup, tagged, Form
from nwos.tools import formataddr, mute_logger
...
class TestAccountComposerPerformance(AccountTestInvoicingCommon, MailCommon):
    ...
    @users('user_account')
    @warmup
    @mute_logger('nwos.addons.mail.models.mail_mail', 'nwos.models.unlink')
...
class TestAccountMoveSend(TestAccountMoveSendCommon):
    ...
    def test_invoice_single(self):
        ...
        wizard = self.create_send_and_print(invoice, sending_methods=['email', 'manual'])
        with patch('nwos.addons.account.models.account_move_send.AccountMoveSend._hook_invoice_document_after_pdf_report_render') as mocked_method:
            results = wizard.action_send_and_print()
    ...
    def test_invoice_multi_with_edi(self):
        ...
        with (
            patch('nwos.addons.account.models.account_move_send.AccountMoveSend._get_default_extra_edis', get_default_extra_edis),
            patch('nwos.addons.account.models.account_move_send.AccountMoveSend._get_all_extra_edis', get_all_extra_edis)
        ):
            wizard = self.create_send_and_print(invoice1 + invoice2)
            self.assertEqual(wizard.summary_data, {
    ...
    def test_invoice_web_service_after_pdf_rendering(self):
        ...
        with patch(
                'nwos.addons.account.models.account_move_send.AccountMoveSend._call_web_service_after_invoice_pdf_render',
    ...
    def test_proforma_pdf(self):
        ...
        # Process.
        with patch(
                'nwos.addons.account.models.account_move_send.AccountMoveSend._hook_invoice_document_before_pdf_report_render',
    ...
    def test_link_pdf_webservice_fails_before(self):
        ...
        # Process.
        with patch(
                'nwos.addons.account.models.account_move_send.AccountMoveSend._call_web_service_before_invoice_pdf_render',
    ...
    def test_cron_notifications(self):
        ...
        sp_partner_2 = self.env['res.partner'].create({'name': 'Partner 2', 'email': 'test@test.nwos.com'})
    ...
    def test_get_sending_settings(self):
        ...
        expected_results = {
            ...
            'mail_subject': 'company_1_data - Hóa đơn INV/2019/00001',
    ...
    def test_ai_write_email(self):
        ...
        with patch('nwos.addons.account.wizard.account_move_send_wizard.requests.post') as mock_post:
            mock_post.return_value.raise_for_status.return_value = None
            mock_post.return_value.json.return_value = {
                'choices': [{
                    'message': {
                        'content': json.dumps({
                            'subject': 'Email hóa đơn mới',
                            'body_html': '<p>Nội dung tốt hơn.</p>',
                        }),
                    },
                }],
            }
            wizard.action_ai_write_email()

        self.assertEqual(wizard.subject, 'Email hóa đơn mới')
        self.assertIn('Nội dung tốt hơn.', wizard.body)
        self.assertEqual(mock_post.call_args.args[0], 'https://api.openai.example/v1/chat/completions')
    ...
    def test_journal_follower_gets_pdf_on_send_and_print(self):
        ...
        with patch('nwos.addons.mail.models.mail_template.MailTemplate.send_mail') as patched_send_mail:
            wizard.action_send_and_print()
