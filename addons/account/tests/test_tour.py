...
from nwos.addons.account.tests.common import AccountTestInvoicingHttpCommon


@nwos.tests.tagged('post_install_l10n', 'post_install', '-at_install')
class TestUi(AccountTestInvoicingHttpCommon):
    ...
    def test_01_account_tour(self):
        ...
        self.start_tour("/nwos", 'account_tour', login="admin")
    ...
    def test_01_account_tax_groups_tour(self):
        ...
        self.start_tour("/nwos", 'account_tax_group', login="admin")
    ...
    def test_use_product_catalog_on_invoice(self):
        ...
        self.start_tour("/nwos/customer-invoices/new", 'test_use_product_catalog_on_invoice', login="admin")
    ...
    def test_deductible_amount_column(self):
        ...
        partner = self.env['res.partner'].create({'name': "Test Partner", 'email': "test@test.nwos.com"})
        ...
        self.start_tour("/nwos/vendor-bills/new", 'deductible_amount_column', login=self.env.user.login)

    def test_add_section_from_product_catalog_on_invoice_tour(self):
        self.product.write({'is_favorite': True})
        self.start_tour(
            '/nwos/customer-invoices/new',
