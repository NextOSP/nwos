...
from nwos import api, fields, models, _, SUPERUSER_ID, modules
from nwos.tools.sql import column_exists, create_column
from nwos.addons.account.tools import format_structured_reference_iso
from nwos.exceptions import UserError, ValidationError, AccessError, RedirectWarning
from nwos.fields import Command, Domain
...
from nwos.tools.mail import email_re, email_split, is_html_empty, generate_tracking_message_id
from nwos.tools.misc import StackMap
...
class AccountMove(models.Model):
    ...
    quick_edit_total_amount = fields.Monetary(
        ...
        help='Use this field to encode the total amount of the invoice.\n'
             'NWOS will automatically create one invoice line with default values to match it.',
    )
    ...
    def _get_invoice_reference_number_invoice(self):
        ...
        """
        ref = self._get_invoice_reference_nwos_invoice() or ''
    ...
    def _get_invoice_reference_number_partner(self):
        ...
        """
        ref = self._get_invoice_reference_nwos_partner()
        return ''.join(char for char in ref if char.isdigit())

    def _get_invoice_reference_nwos_invoice(self):
        """ This computes the reference based on the NWOS format.
    ...
    def _get_invoice_reference_nwos_partner(self):
        """ This computes the reference based on the NWOS format.
            The data used is the reference set on the partner or its database
            id otherwise. For instance if the reference of the customer is
    ...
    def _get_invoice_computed_reference(self):
        ...
        reference_model = journal.invoice_reference_model
        if reference_model == 'odoo':
            # Legacy databases can still contain the old technical value.
            reference_model = 'nwos'
        reference_type = journal.invoice_reference_type
        ref_function = getattr(self, f'_get_invoice_reference_{reference_model}_{reference_type}', None)
        if ref_function is None:
            reference_models = dict(journal._fields['invoice_reference_model']._description_selection(self.env))
            reference_types = dict(journal._fields['invoice_reference_type']._description_selection(self.env))
            raise UserError(_(
                "The journal \"%(journal)s\" cannot generate a payment reference with these settings:\n"
                "- Communication Standard: %(reference_model)s\n"
                "- Communication Type: %(reference_type)s\n\n"
                "Open the journal settings and choose a supported communication standard/type.",
                journal=journal.display_name,
                reference_model=reference_models.get(journal.invoice_reference_model, journal.invoice_reference_model or _("Not set")),
                reference_type=reference_types.get(reference_type, reference_type or _("Not set")),
            ))
        return ref_function()
    ...
    def _get_fields_to_copy_recurring_entries(self, values):
        ...
        values.update({
            ...
            'invoice_user_id': self.invoice_user_id.id,  # otherwise user would be NWOSBot
        })
    ...
    def _message_post_after_hook(self, new_message, message_values):
        ...
        if self.env.context.get('from_alias'):
            ...
            # Only trigger decoding if the message was sent by an active internal user (note NWOSBot is always inactive).
