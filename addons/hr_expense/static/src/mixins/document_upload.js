...
import { onWillStart, useRef, useEffect, useState } from "@nwos/owl";
...
export const ExpenseDocumentUpload = (T) => class ExpenseDocumentUpload extends T {
    ...
    async onChangeFileInput() {
        try {
            const uploaded = await this._onChangeFileInput([...this.fileInput.el.files]);
            if (uploaded && this.uploadsProcessing === 1) {
            ...
            this.notification.add(
                error.message || _t("An error occurred during the upload"),
                { type: "danger" }
            );
        } finally {
            if (this.fileInput.el) {
                this.fileInput.el.value = null;
            }
            this.uploadsProcessing--;
        }
    }
    ...
    async _onChangeFileInput(files) {
        ...
        let attachments;
        try {
            attachments = JSON.parse(fileData);
        } catch {
            throw new Error(_t("The upload could not be completed. Please refresh the page and try again."));
        }
        const uploadError = Array.isArray(attachments)
        ...
        if (!Array.isArray(attachments)) {
            throw new Error(_t("The upload returned an invalid response. Please try again."));
        }
