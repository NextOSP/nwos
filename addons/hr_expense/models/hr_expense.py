...
from nwos import api, fields, Command, models, _
from nwos.exceptions import AccessError, RedirectWarning, UserError, ValidationError
from nwos.fields import Domain
from nwos.tools import clean_context, email_normalize, float_repr, float_round, format_date, is_html_empty, parse_version
...
class HrExpense(models.Model):
    ...
    def _compute_currency_rate(self):
        """
            We want the default nwos rate when the following change:
        ...
        for company, expenses_submitted_per_company in self.grouped('company_id').items():
            ...
            for manager, expenses_submitted in expenses_submitted_per_company.grouped('manager_id').items():
                ...
                body = self.env['ir.qweb']._render(
                    ...
                    values={'manager_name': manager.name, 'url': '/nwos/expenses-to-process', 'company': company},
                    lang=mail_lang,
                )
                new_mails.append({
                    'author_id': self.env.user.partner_id.id,
    ...
    def _get_base_account(self):
        ...
        if not account:
            raise UserError(self.env._(
                "NWOS had a look at your expense, its product, your company and the journal but came back with empty hands.\n"
                "Give NWOS a hand to find an account by setting up an expense account.\n"
