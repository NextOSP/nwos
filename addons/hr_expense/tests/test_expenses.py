...
from nwos.tests import Form, tagged

from nwos.addons.hr_expense.tests.common import TestExpenseCommon
...
class TestExpenses(TestExpenseCommon):
    ...
    def test_own_account_expense_payment_without_method_account_creates_move(self):
        """Expense payments must not silently become no-entry registered payments."""
        bank_journal = self.company_data['default_journal_bank']
        outbound_manual_line = bank_journal.outbound_payment_method_line_ids.filtered(lambda line: line.code == 'manual')[:1]
        outbound_manual_line.payment_account_id = False

        expense = self.create_expenses({
            'name': 'Lunch',
            'payment_mode': 'own_account',
            'total_amount_currency': 50.00,
        })
        expense.action_submit()
        expense.action_approve()
        self.post_expenses_with_wizard(expense)
        ...
        payment = self.env['account.payment.register'].with_context(
            ...
            active_ids=expense.account_move_id.ids,
        ).create({
            'journal_id': bank_journal.id,
            'payment_method_line_id': outbound_manual_line.id,
        })._create_payments()

        self.assertTrue(payment.move_id)
        self.assertEqual(payment.outstanding_account_id, bank_journal.default_account_id)
        self.assertEqual(outbound_manual_line.payment_account_id, bank_journal.default_account_id)
        self.assertEqual(payment.state, 'paid')
        self.assertEqual(expense.account_move_id.payment_state, 'paid')
