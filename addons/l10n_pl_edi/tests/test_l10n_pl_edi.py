...
from nwos import Command, fields, tools
from nwos.exceptions import UserError
from nwos.tests import freeze_time, patch, tagged
...
from nwos.addons.l10n_pl_edi.models.account_move import AccountMove
from nwos.addons.l10n_pl_edi.tools.ksef_api_service import KsefApiService
...
class TestL10nPlEdi(AccountTestInvoicingCommon, CronMixinCase):
    ...
    def test_ksef_fa3_standard_vat(self):
        ...
        This test verifies that a regular NWOS invoice (not a down payment or correction)
    ...
    def l10n_pl_edi_generate_attachments(self, invoices, from_cron=False):
        ...
        with patch('nwos.addons.l10n_pl_edi.models.account_move_send.AccountMoveSend._call_web_service_before_invoice_pdf_render'):
    ...
    def test_cron_creates_empty_move_on_parsing_error(self):
        ...
        with (
            patch.object(KsefApiService, 'query_invoice_metadata', side_effect=query_invoice_metadata),
            patch.object(KsefApiService, 'get_invoice_by_ksef_number', side_effect=get_invoice_by_ksef_number),
            patch('nwos.addons.l10n_pl_edi.models.account_move.AccountMove.l10n_pl_edi_get_ksef_bill_vals_from_xml', mock_parse)
        ):
            self.env['account.move'].with_company(self.company)._l10n_pl_edi_download_bills_from_ksef()
