...
- `15.0`
  - The support for webhook notifications is added. nwos/nwos#84069
...
createdb nwos
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -i payment_payulatam
```
...
```bash
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -u payment_payulatam
```
