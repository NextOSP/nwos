...
from nwos import SUPERUSER_ID, api, fields, models, modules
from nwos.exceptions import UserError
from nwos.fields import Domain
from nwos.tools import config, date_utils, split_every
from nwos.tools.image import image_data_uri

from nwos.addons.account.tools import dict_to_xml
...
class MyInvoisDocument(models.Model):
    ...
    In NWOS, a document represent either an invoice, or a group of PoS order.
    """
    ...
    def _myinvois_map_error(self, error):
        ...
        error_map = {
            ...
            "internal_server_error": self.env._(
                "Server error; If the problem persists, please contact the NWOS support."
            ),
            # The proxy user credentials are either incorrect, or NWOS does not have the permission to invoice on their behalf.
            "invalid_tin": self.env._(
                "Please make sure that your company TIN is correct, and that you gave NWOS sufficient permissions on the MyInvois platform."
            ),
            ...
            'action_forbidden': self.env._('Permission to do this action has not been granted. Please ensure that NWOS has sufficient permissions on the MyInvois platform.'),
        }
    ...
    def _myinvois_update_document(self, status, reason):
        """
        This method will try to update the status of a document on the platform, and if needed also the status in NWOS.
    ...
    def _myinvois_set_state(self, state, message=None):
        ...
        # In NWOS, the best way to represent that is by cancelling the invoice.
    ...
    def _myinvois_set_validation_fields(self, validation_result):
        ...
        # NWOS expect a timezone unaware datetime in UTC, so we can safely remove the info without any more work needed.
