...
from nwos.addons.test_mail.tests.common import TestRecipients
from nwos.tests import tagged
from nwos.tools import mute_logger


@tagged("nwosbot")
class TestNWOSbot(MailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.nwosbot = cls.env.ref("base.partner_root")
        ...
        cls.nwosbot_ping_body = f'<a href="http://nwos.com/nwos/res.partner/{cls.nwosbot.id}" class="o_mail_redirect" data-oe-id="{cls.nwosbot.id}" data-oe-model="res.partner" target="_blank">@NWOSBot</a>'
        cls.test_record_employe = cls.test_record.with_user(cls.user_employee)

    @mute_logger('nwos.addons.mail.models.mail_mail')
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_nwosbot()
        nwosbot = self.env.ref("base.partner_root")
        nwosbot_in_fetch_listeners = self.env['discuss.channel.member'].search([('channel_id', '=', channel.id), ('partner_id', '=', nwosbot.id)])
        self.assertEqual(len(nwosbot_in_fetch_listeners), 1, 'nwosbot should appear only once in channel_fetch_listeners')

    @mute_logger('nwos.addons.mail.models.mail_mail')
    def test_nwosbot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.nwosbot_ping_body, 'partner_ids': [self.nwosbot.id, self.user_admin.partner_id.id]})
        ...
        # NWOSbot should not be a follower but user_employee and user_admin should
        follower = self.test_record.message_follower_ids.mapped('partner_id')
        self.assertNotIn(self.nwosbot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last nwosbot message instead
            sender=self.nwosbot,
            answer=("@NWOSBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.nwosbot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.nwosbot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('nwos.addons.mail.models.mail_mail')
    def test_nwosbot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @nwosbot right now", 'partner_ids': []})
        self.assertNextMessage(
