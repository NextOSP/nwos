...
current_repo="$(git remote get-url $current_remote | sed 's/.*github.com[\/:]//' | sed 's/\/nwos.git//')"
read -p "Enter repo [${current_repo}]: " REPO
REPO="https://github.com/${REPO:-$current_repo}/nwos.git"
echo "Using repo: ${REPO}"

CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/nwos"
if [ ! -d "$CLONE_DIR" ]; then
    ...
    git remote set-url origin "https://github.com/nwos/nwos.git" # ensure remote is the original repo
fi
cd "${__dir}"
