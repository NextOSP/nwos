...
if [[ "${1:-}" == "-c" || "${1:-}" == "--cleanup" ]]; then
    ...
    rm -rf overwrite_before_init/home/pi/nwos
    rm -rfv iotbox.img
...
# Download and extract Raspberry Pi OS, ngrok and clone NWOS repository
...
mkdir -pv ./.ssh
echo "y" | ssh-keygen -t ed25519 -f "./.ssh/iotbox_ca_${VERSION_IOTBOX}" -N "" -C "NWOS SSH CA ${VERSION_IOTBOX}"
...
mkdir -pv "${MOUNT_POINT}/var/nwos/"
echo "${VERSION_IOTBOX}" > "${MOUNT_POINT}/var/nwos/iotbox_version"
