...
- `17.0`
  - The Web Drop-in SDK is replaced by the Web Components SDK (version 5.39.0); the Checkout and
    Recurring APIs are updated to versions 70 and 68. nwos/nwos#120446
  - The 'Checkout API URL' and 'Recurring API URL' fields are replaced by the 'API URL Prefix'
    field. nwos/nwos#126831
- `16.4`
  - The responses of webhook notifications are sent with the proper HTTP code. nwos/nwos#117940
- `16.2`
  - The support for partial manual capture is added. nwos/nwos#87251
- `16.0`
  - Archiving a token no longer deactivates the related payment method on Adyen. nwos/nwos#93774
- `15.3`
  - The support for manual capture is added. nwos/nwos#70591
- `15.2`
...
- `15.0`
  - The support for both full and partial refunds is added. nwos/nwos#70881
  - The Web Drop-in SDK is migrated to version 4.7.3 and the Checkout API to version 67 to switch
    from relying on origin keys and use client keys instead. nwos/nwos#74827
- `14.3`
  - The previous Hosted Payment Pages API that allowed for redirect payments is replaced by a 
    ...
    (version 49) APIs. nwos/nwos#141661
...
createdb nwos
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -i payment_adyen
```
...
```bash
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -u payment_adyen
```
