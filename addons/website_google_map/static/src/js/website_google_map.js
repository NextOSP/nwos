...
function initialize_map() {
    ...
    var map = new google.maps.Map(document.getElementById('nwos-google-map'), {
        zoom: 1,
    ...
    async function initializeMarkers(nwosPartnerData) {
        // Create the markers and cluster them on the map
        if (nwosPartnerData) { /* nwos_partner_data special variable should have been defined in google_map.xml */
            const markerPromises = [];
            for (var i = 0; i < nwos_partner_data.counter; i++) {
                const prom = set_marker(nwos_partner_data.partners[i])
    ...
    initializeMarkers(nwos_partner_data);
