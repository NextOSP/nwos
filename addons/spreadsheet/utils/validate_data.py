...
from nwos.tools.view_validation import get_domain_value_names


markdown_link_regex = r"^\[([^\[]+)\]\((.+)\)$"

xml_id_url_prefix = "nwos://ir_menu_xml_id/"

nwos_view_link_prefix = "nwos://view/"


def nwos_charts(data):
    """returns all nwos chart definitions in the spreadsheet"""
    figures = []
    for sheet in data.get("sheets", []):
        for figure in sheet.get("figures", []):
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("nwos_"):
                figures.append(dict(figure["data"], id=figure["id"]))
            elif figure["tag"] == "carousel":
                figures.extend(get_nwos_charts_from_carousel(figure["data"]))
    return figures


def get_nwos_charts_from_carousel(carousel):
    charts = []
    for chart_id, chart in carousel["chartDefinitions"].items():
        if chart["type"].startswith("nwos_"):
...
def nwos_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like nwos://view/{... view data...}
    """
    return [
        json.loads(url[len(nwos_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(nwos_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = nwos_charts(data)
    if "nwosVersion" in data and data["nwosVersion"] < 5:
...
def fields_in_spreadsheet(data):
    ...
    nwos_pivots = (pivot for pivot in data.get("pivots", dict()).values() if pivot.get("type", "NWOS") == "NWOS")
    all_fields = chain(
        extract_fields(list_fields, data.get("lists", dict()).values()).items(),
        extract_fields(pivot_fields, nwos_pivots).items(),
        extract_fields(chart_fields, nwos_charts(data)).items(),
        extract_fields(nwos_view_fields, nwos_view_links(data)).items(),
...
def menus_xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartNWOSMenusReferences", {}).values()) | {
