...
import { EvaluationError, helpers } from "@nwos/o-spreadsheet";

const { isDateOrDatetimeField } = helpers;

/**
 * @typedef {import("@nwos/o-spreadsheet").Token} Token
 * @typedef {import("@nwos/o-spreadsheet").Granularity} Granularity
 * */
...
export const NWOS_AGGREGATORS = {};

for (const type in AGGREGATORS_BY_FIELD_TYPE) {
    NWOS_AGGREGATORS[type] = {};
    for (const aggregator of AGGREGATORS_BY_FIELD_TYPE[type]) {
        NWOS_AGGREGATORS[type][aggregator] = AGGREGATOR_NAMES[aggregator];
    }
}
...
/**
 * @typedef {import("@spreadsheet").NWOSField} NWOSField
 */

/**
 * Parses the positional char (#), the field and operator string of pivot group.
 * e.g. "create_date:month"
 * @param {Record<string, NWOSField | undefined>} allFields
 * @param {string} groupFieldString
 * @returns {{field: NWOSField, granularity: Granularity, isPositional: boolean, dimensionWithGranularity: string}}
 */
