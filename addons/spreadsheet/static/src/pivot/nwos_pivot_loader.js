...
import { EvaluationError, CellErrorType } from "@nwos/o-spreadsheet";
...
export class NWOSPivotLoader {
    /**
     * @param {NWOSDataProvider} nwosDataProvider
     * @param {Function} load Function to fetch data
     */
    constructor(nwosDataProvider, load) {
        /** @private @type {NWOSDataProvider} */
        this.nwosDataProvider = nwosDataProvider;
        /** @protected */
        this.loadFn = load;
    ...
    async load(options) {
        if (options && options.reload) {
            this.nwosDataProvider.cancelPromise(this.loadPromise);
            this.loadPromise = undefined;
        }
        if (!this.loadPromise) {
            this.isFullyLoaded = false;
            ...
            await this.nwosDataProvider.notifyWhenPromiseResolves(this.loadPromise);
        }
     ...
     * @returns {Promise<NWOSFields>} Fields of the model
     */
    async getFields(model) {
        return getFields(this.nwosDataProvider.fieldService, model);
    }
    /**
     * @param {string} model Technical name of the model
    ...
    async getModelLabel(model) {
        const result = await this.nwosDataProvider.orm
            .cache({ type: "disk" })
