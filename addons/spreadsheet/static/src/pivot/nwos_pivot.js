...
import { NO_RECORD_AT_THIS_POSITION, NWOSPivotModel } from "./pivot_model";
import { EvaluationError, PivotRuntimeDefinition, registries, helpers } from "@nwos/o-spreadsheet";
import { LOADING_ERROR } from "@spreadsheet/data_sources/data_source";
import { omit } from "@web/core/utils/objects";
import { NWOSPivotLoader } from "./nwos_pivot_loader";
...
/**
 * @typedef {import("@nwos/o-spreadsheet").FunctionResultObject} FunctionResultObject
 * @typedef {import("@nwos/o-spreadsheet").PivotMeasure} PivotMeasure
 * @typedef {import("@nwos/o-spreadsheet").PivotDomain} PivotDomain
 * @typedef {import("@nwos/o-spreadsheet").PivotDimension} PivotDimension
 * @typedef {import("@nwos/o-spreadsheet").PivotCoreMeasure} PivotCoreMeasure
 * @typedef {import("@spreadsheet").WebPivotModelParams} WebPivotModelParams
 * @typedef {import("@spreadsheet").NWOSPivot<NWOSPivotRuntimeDefinition>} IPivot
 * @typedef {import("@spreadsheet").NWOSFields} NWOSFields
 * @typedef {import("@spreadsheet").NWOSPivotCoreDefinition} NWOSPivotCoreDefinition
 * @typedef {import("@spreadsheet").NWOSGetters} NWOSGetters
 * @typedef {import("@spreadsheet/data_sources/nwos_data_provider").NWOSDataProvider} NWOSDataProvider
 */

/**
 * @implements {IPivot}
 */
export class NWOSPivot {
    /**
     *
     * @override
     * @param {Object} services Services (see DataSource)
     * @param {Object} params
     * @param {NWOSPivotCoreDefinition} params.definition
     * @param {NWOSGetters} params.getters
     */
    constructor(services, { definition, getters }) {
        /** @type {"NWOS"} */
        this.type = "NWOS";

        /** @type {NWOSPivotCoreDefinition} @protected */
        this.coreDefinition = definition;

        this.needsReevaluation = false;

        /** @type {NWOSPivotRuntimeDefinition | undefined} @protected */
        this.runtimeDefinition = undefined;

        /** @type {NWOSPivotModel | undefined} @protected */
        this.model = undefined;

        /** @type {NWOSGetters} @protected */
        this.getters = getters;

        /** @protected */
        this.loader = new NWOSPivotLoader(services.nwosDataProvider, this._load.bind(this));

        /** @type {NWOSFields | undefined} @protected */
        this._fields = undefined;

        /** @protected @type {NWOSDataProvider}*/
        this.nwosDataProvider = services.nwosDataProvider;
    ...
    /**
     * @param {NWOSPivotCoreDefinition} nextDefinition
     */
    onDefinitionChange(nextDefinition) {
        ...
        if (
            ...
            if (
                ...
                this.coreDefinition = nextDefinition;
                const runtimeDefinition = new NWOSPivotRuntimeDefinition(
                    this.coreDefinition,
                    this.getFields()
                );
    ...
    async createModelAndDefinition() {
        ...
        const definition = new NWOSPivotRuntimeDefinition(this.coreDefinition, this.getFields());
        const model = new NWOSPivotModel(
            { _t },
            {
            ...
            {
                orm: this.nwosDataProvider.orm,
                serverData: this.nwosDataProvider.serverData,
    ...
    /**
     * @param {import("@nwos/o-spreadsheet").Maybe<FunctionResultObject>[]} args
     *
     * @returns {PivotDomain}
     */
        ...
        await Promise.all(
            related.map((dimension) =>
                getRelationalFieldDefinition(
                    ...
                    dimension.fieldName,
                    this.nwosDataProvider.fieldService
    ...
    async _loadPropertiesDefinitions() {
        ...
        const orm = this.nwosDataProvider.orm;
...
export class NWOSPivotRuntimeDefinition extends PivotRuntimeDefinition {
    /**
     * @param {NWOSPivotCoreDefinition} definition
     * @param {NWOSFields} fields
     */
...
pivotRegistry.add("NWOS", nwosPivotRegistryItem);
supportedPivotPositionalFormulaRegistry.add("NWOS", true);
