...
export class PivotCoreViewGlobalFilterPlugin extends NWOSCoreViewPlugin {
     ...
     */
    _addDomain(pivotId) {
        if (this.getters.getPivotCoreDefinition(pivotId).type !== "NWOS") {
            return;
        }
        const domainList = [];
    ...
    _addDomains() {
        for (const pivotId of this.getters
            .getPivotIds()
            .filter((pivotId) => this.getters.getPivot(pivotId).type === "NWOS")) {
