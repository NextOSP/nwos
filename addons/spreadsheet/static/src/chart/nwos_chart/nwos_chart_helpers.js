...
export function onNWOSChartItemClick(getters, chart) {
    return navigateInNWOSMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onWaterfallNWOSChartItemClick(getters, chart) {
    return navigateInNWOSMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onGeoNWOSChartItemClick(getters, chart) {
    return navigateInNWOSMenuOnClick(getters, chart, (chartJsItem) => {
...
export function onSunburstNWOSChartItemClick(getters, chart) {
    return navigateInNWOSMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
...
export function onTreemapNWOSChartItemClick(getters, chart) {
    return navigateInNWOSMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
        const rawItem = chartJSChart.data.datasets[datasetIndex].data[index];
...
function navigateInNWOSMenuOnClick(getters, chart, getDomainFromChartItem) {
    return async (event, items, chartJSChart) => {
        const env = getters.getNWOSEnv();
        const { datasets, labels } = chart.dataSource.getData();
...
export function onNWOSChartItemHover() {
    return (event, items) => {
        if (items.length > 0) {
...
export function onGeoNWOSChartItemHover() {
    return (event, items) => {
        if (!items.length) {
            event.native.target.style.cursor = "";
            return;
...
export async function navigateToNWOSMenu(menu, actionService, notificationService, newWindow) {
    if (!menu) {
