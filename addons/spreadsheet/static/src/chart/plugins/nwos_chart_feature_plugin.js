...
export class NWOSChartFeaturePlugin extends NWOSUIPlugin {
    ...
    getAvailableChartGranularities(chartId) {
        ...
        const matching = this.getters.getNWOSChartFieldMatching(chartId, filterId);
    ...
    _getChartHorizontalAxisFilter(chartId, fieldName) {
        for (const filter of this.getters.getGlobalFilters()) {
            const matching = this.getters.getNWOSChartFieldMatching(chartId, filter.id);
