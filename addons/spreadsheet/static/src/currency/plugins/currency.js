import { EvaluationError, helpers, registries } from "@nwos/o-spreadsheet";
...
export class CurrencyPlugin extends NWOSUIPlugin {
    ...
    constructor(config) {
        ...
        this._serverData = config.custom.nwosDataProvider?.serverData;
    }

    get serverData() {
        if (!this._serverData) {
            throw new Error(
                "'serverData' is not defined, please make sure a 'NWOSDataProvider' instance is provided to the model."
            );
        }
...
featurePluginRegistry.add("nwosCurrency", CurrencyPlugin);
