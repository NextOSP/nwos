...
import { LEGACY_NWOS_CHART_TYPE_ALIASES } from "../chart/nwos_chart/chart_type";
...
const MAP_V1 = {
    PIVOT: "NWOS.PIVOT",
    "PIVOT.HEADER": "NWOS.PIVOT.HEADER",
    "PIVOT.POSITION": "NWOS.PIVOT.POSITION",
    "FILTER.VALUE": "NWOS.FILTER.VALUE",
    LIST: "NWOS.LIST",
    "LIST.HEADER": "NWOS.LIST.HEADER",
};

const MAP_FN_NAMES_V10 = {
    "NWOS.PIVOT": "PIVOT.VALUE",
    "NWOS.PIVOT.HEADER": "PIVOT.HEADER",
    "NWOS.PIVOT.TABLE": "PIVOT",
};

const MAP_NWOS_FN_NAMES = {
    "NWOS.ACCOUNT.GROUP": "NWOS.ACCOUNT.GROUP",
    "NWOS.BALANCE": "NWOS.BALANCE",
    "NWOS.BALANCE.TAG": "NWOS.BALANCE.TAG",
    "NWOS.CREDIT": "NWOS.CREDIT",
    "NWOS.CURRENCY.RATE": "NWOS.CURRENCY.RATE",
    "NWOS.DEBIT": "NWOS.DEBIT",
    "NWOS.FILTER.LABEL": "NWOS.FILTER.LABEL",
    "NWOS.FILTER.VALUE": "NWOS.FILTER.LABEL",
    "NWOS.FILTER.VALUE.V18": "NWOS.FILTER.VALUE.V18",
    "NWOS.FISCALYEAR.END": "NWOS.FISCALYEAR.END",
    "NWOS.FISCALYEAR.START": "NWOS.FISCALYEAR.START",
    "NWOS.LIST": "NWOS.LIST",
    "NWOS.LIST.HEADER": "NWOS.LIST.HEADER",
    "NWOS.PARTNER.BALANCE": "NWOS.PARTNER.BALANCE",
    "NWOS.PIVOT": "PIVOT.VALUE",
    "NWOS.PIVOT.HEADER": "PIVOT.HEADER",
    "NWOS.PIVOT.POSITION": "NWOS.PIVOT.POSITION",
    "NWOS.PIVOT.TABLE": "PIVOT",
    "NWOS.RESIDUAL": "NWOS.RESIDUAL",
};

const MAP_LEGACY_NWOS_FN_NAMES = Object.fromEntries(
    Object.entries(MAP_NWOS_FN_NAMES).map(([fnName, replacement]) => [
        fnName.replace(/^NWOS/, "ODOO"),
        replacement,
    ])
);
...
migrationStepRegistry.add("17.3.1", {
    migrate(data) {
        return migrateNWOSData(data);
    },
});
migrationStepRegistry.add("18.1.2", {
    migrate(data) {
        const version = data.nwosVersion || 0;
        if (version < 13) {
    ...
    migrate(data) {
        for (const sheet of data.sheets || []) {
            for (const figure of sheet.figures || []) {
                if (
                    figure.tag === "chart" &&
                    ["nwos_bar", "nwos_line", "nwos_pie"].includes(figure.data.type) &&
...
migrationStepRegistry.add("18.5.10", {
    migrate(data) {
        ...
        const re = /NWOS\.FILTER\.VALUE/gi;
        for (const sheet of data.sheets || []) {
            for (const xc in sheet.cells || {}) {
                const content = sheet.cells[xc];
                if (re.test(content)) {
                    sheet.cells[xc] = content.replaceAll(re, "NWOS.FILTER.LABEL");
                }
            }
        }
        return data;
...
// NWOS dashboards created from NWOS spreadsheet data can still store NWOS
// pivot definitions. Normalize them to the NWOS pivot runtime.
migrationStepRegistry.add("18.5.11", {
    migrate(data) {
        ...
        return renameFunctions(data, MAP_NWOS_FN_NAMES);
    },
});

migrationStepRegistry.add("18.5.12", {
    migrate(data) {
        normalizeLegacyNWOSChartTypes(data);
        normalizeLegacyNWOSPivotTypes(data);
        return renameFunctions(data, MAP_LEGACY_NWOS_FN_NAMES);
    },
});

function migrateNWOSData(data) {
    const version = data.nwosVersion || 0;
    if (version < 1) {
...
function normalizeLegacyNWOSChartTypes(data) {
    for (const sheet of data.sheets || []) {
        for (const figure of sheet.figures || []) {
            if (figure.tag === "chart") {
                normalizeLegacyNWOSChartDefinition(figure.data);
            } else if (figure.tag === "carousel") {
                for (const definition of Object.values(figure.data?.chartDefinitions || {})) {
                    normalizeLegacyNWOSChartDefinition(definition);
                }
            }
        }
    }
...
function normalizeLegacyNWOSChartDefinition(definition) {
    if (definition?.type in LEGACY_NWOS_CHART_TYPE_ALIASES) {
        definition.type = LEGACY_NWOS_CHART_TYPE_ALIASES[definition.type];
    }
}

function normalizeLegacyNWOSPivotTypes(data) {
    for (const pivot of Object.values(data.pivots || {})) {
        if (typeof pivot.type === "string" && pivot.type.toUpperCase() === "ODOO") {
            pivot.type = "NWOS";
        }
    }
}
...
function migratePivotDaysParameters(formulaString) {
    ...
    const convertedAst = convertAstNodes(ast, "FUNCALL", (ast) => {
        if (["NWOS.PIVOT", "NWOS.PIVOT.HEADER"].includes(ast.value.toUpperCase())) {
...
/**
 * Migrate the pivot data to add the type, by default "NWOS". And replace the
...
function migrate11to12(data) {
    // remove the calls to NWOS.PIVOT.POSITION and replace
    ...
    for (const sheet of data.sheets || []) {
        for (const xc in sheet.cells || []) {
            ...
            if (
                ...
                cell.content.includes("NWOS.PIVOT.POSITION")
            ) {
                const tokens = tokenize(cell.content);
                /* given that nwos.pivot.position is automatically set, we know that:
                1) it is always on the form of NWOS.PIVOT.POSITION(1, ...)
                2) it is always preceded by a dimension of a pivot or header, inside another pivot formula
                3) there is only one nwos.pivot.position per cell
                4) nwos.pivot.position can only exist after the 3rd token and needs at least 7 tokens to be valid*/
                for (let i = 2; i < tokens.length - 7; i++) {
                    const token = tokens[i];
                    if (
                        token.type === "SYMBOL" &&
                        token.value.toUpperCase() === "NWOS.PIVOT.POSITION"
                    ) {
                        const order = tokens[i + 6];
                        tokens[i - 2].value = '"#' + tokens[i - 2].value.slice(1); // "dimension" becomes "#dimension"
                        tokens.splice(i, 7); // remove "NWOS.PIVOT.POSITION", "(", "1", ",", "dimension", ", ", order
                        // tokens[i-1] is the comma before nwos.pivot.position
                        tokens[i] = order;
                        cell.content = tokensToString(tokens);
                    }
                }
...
/**
 * Change pivot sortedColumn of the nwos's pivot model to the sortedColumn of o-spreadsheet
 */
