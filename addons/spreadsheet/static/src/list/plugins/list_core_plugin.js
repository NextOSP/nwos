...
export class ListCorePlugin extends NWOSCorePlugin {
    ...
    _insertHeaders(sheetId, anchor, id, columns) {
        ...
        for (const column of columns) {
            const content = column.string
                ? `=NWOS.LIST.HEADER(${id},"${column.name}","${column.string}")`
                : `=NWOS.LIST.HEADER(${id},"${column.name}")`;
        ...
        for (let i = 1; i <= linesNumber; i++) {
            ...
            for (const column of columns) {
                this.dispatch("UPDATE_CELL", {
                    ...
                    col,
                    row,
                    content: `=NWOS.LIST(${id},${i},"${column.name}")`,
                });
                col++;
            }
            row++;
