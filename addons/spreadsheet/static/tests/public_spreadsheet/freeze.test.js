import { animationFrame, describe, expect, test } from "@nwos/hoot";
import { Model, registries } from "@nwos/o-spreadsheet";
...
import { freezeNWOSData, waitForDataLoaded } from "@spreadsheet/helpers/model";
import { NWOSPivot, NWOSPivotRuntimeDefinition } from "@spreadsheet/pivot/nwos_pivot";
...
test("nwos pivot functions are replaced with their value", async function () {
    ...
    const data = await freezeNWOSData(model);
...
test("Pivot with a type different of NWOS is not converted", async function () {
    // Add a pivot with a type different of NWOS
...
test("nwos pivot functions detection is not case sensitive", async function () {
    const { model } = await createSpreadsheetWithPivot();
    setCellContent(model, "A1", '=pivot.value(1,"probability:avg")');
    const data = await freezeNWOSData(model);
...
test("nwos charts are replaced with an image", async function () {
...
test("Carousels figure with nwos data is converted to an image", async function () {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Empty NWOS.LIST result is frozen to an empty string", async function () {
    const { model } = await createSpreadsheetWithList();
    setCellContent(model, "A1", '=NWOS.LIST(1, 9999,"probability")'); // has no record
    await waitForDataLoaded(model);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    const frozenData = await freezeNWOSData(model);
    expect(frozenData.sheets[0].cells.A1).toBe('=""');
});

test("nwos links are replaced with their label", async function () {
    const view = {
        name: "an nwos view",
        viewType: "list",
        action: {
    ...
    const data = {
        sheets: [
            {
                cells: {
                    A1: "[menu_xml](nwos://ir_menu_xml_id/test_menu)",
                    A2: "[menu_id](nwos://ir_menu_id/12)",
                    A3: `[nwos_view](nwos://view/${JSON.stringify(view)})`,
                    A4: "[external_link](https://nwos.com)",
    ...
    expect(frozenData.sheets[0].cells.A3).toBe("[nwos_view](neutralized:link)");
    expect(frozenData.sheets[0].cells.A4).toBe("[external_link](https://nwos.com)");
