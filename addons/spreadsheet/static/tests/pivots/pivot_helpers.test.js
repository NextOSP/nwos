...
import { constants, tokenize, helpers } from "@nwos/o-spreadsheet";
...
test("Basic formula extractor", async function () {
    const formula = `=PIVOT.VALUE("1", "test") + NWOS.LIST("2", "hello", "bla")`;
    ...
    expect(functionName).toBe("NWOS.LIST");
...
test("Number of formulas", async function () {
    const formula = `=PIVOT.VALUE("1", "test") + PIVOT.VALUE("2", "hello", "bla") + NWOS.LIST("1", "bla")`;
