import { describe, expect, test, Deferred } from "@nwos/hoot";
...
test("evaluates only once when two pivots are loading", async function () {
    ...
    model.config.custom.nwosDataProvider.addEventListener("data-source-updated", () =>
...
test("can edit pivot domain with UPDATE_NWOS_PIVOT_DOMAIN", async () => {
    const { model } = await createSpreadsheetWithPivot();
    const [pivotId] = model.getters.getPivotIds();
