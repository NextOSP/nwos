...
import { load } from "@nwos/o-spreadsheet";
...
test("NWOS formulas are migrated", () => {
    const data = {
        version: 16,
        sheets: [
            {
    ...
    expect(migratedData.sheets[0].cells.A3).toBe(`=NWOS.FILTER.LABEL("1")`);
    expect(migratedData.sheets[0].cells.A4).toBe(`=NWOS.LIST("1")`);
    expect(migratedData.sheets[0].cells.A5).toBe(`=NWOS.LIST.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A6).toBe(`=NWOS.PIVOT.POSITION("1")`);
...
test("Pivot 'day' arguments are migrated", () => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=NWOS.PIVOT("1","21/07/2022")` },
                    A2: { content: `=NWOS.PIVOT.HEADER("1","11/12/2022")` },
                    A3: { content: `=nwos.pivot("1","21/07/2021")` },
                    A4: { content: `=NWOS.PIVOT("1","test")` },
                    A5: { content: `=nwos.pivot("1","21/07/2021")+"21/07/2021"` },
...
test("NWOS pivot types are migrated to NWOS pivot types", () => {
    const data = {
        version: "18.5.10",
        sheets: [
            {
                cells: {
                    A1: '=NWOS.LIST.HEADER(1, "name", _t("Reference"))',
                    A2: '=NWOS.LIST(1,1,"name")',
                    A3: '=NWOS.BALANCE(NWOS.ACCOUNT.GROUP("income"),$B$4)',
                    A4: '=NWOS.FILTER.VALUE("Period")',
                    A5: { content: '=nwos.currency.rate("EUR","USD")' },
                    A6: '=NWOS.PIVOT("1")',
                },
            },
        ],
        pivots: {
            1: {
                ...
                name: "Name",
                model: "res.model",
                measures: [{ id: "probability", fieldName: "probability" }],
                columns: [],
                rows: [],
                formulaId: "1",
            },
    ...
    const migratedData = load(data);
    expect(migratedData.pivots["1"].type).toBe("NWOS");
    expect(migratedData.sheets[0].cells.A1).toBe(
        '=NWOS.LIST.HEADER(1, "name", _t("Reference"))'
    );
    expect(migratedData.sheets[0].cells.A2).toBe('=NWOS.LIST(1,1,"name")');
    expect(migratedData.sheets[0].cells.A3).toBe(
        '=NWOS.BALANCE(NWOS.ACCOUNT.GROUP("income"),$B$4)'
    );
    expect(migratedData.sheets[0].cells.A4).toBe('=NWOS.FILTER.LABEL("Period")');
    expect(migratedData.sheets[0].cells.A5).toBe('=NWOS.CURRENCY.RATE("EUR","USD")');
    expect(migratedData.sheets[0].cells.A6).toBe('=PIVOT.VALUE("1")');
});

test("Legacy NWOS spreadsheet names are migrated", () => {
    const data = {
        version: "18.5.11",
        sheets: [
            {
                cells: {
                    A1: '=ODOO.FILTER.VALUE("Period")',
                    A2: { content: '=odoo.currency.rate("EUR","USD")' },
                    A3: '=ODOO.PIVOT("1")',
                },
                figures: [
                    {
                        id: "chart1",
                        tag: "chart",
                        data: { type: "odoo_line", chartId: "chart1" },
                    },
                    {
                        id: "chart2",
                        tag: "chart",
                        data: { type: "flectra_line", chartId: "chart2" },
                    },
                    {
                        id: "carousel1",
                        tag: "carousel",
                        data: {
                            chartDefinitions: {
                                chart3: { type: "odoo_bar" },
                                chart4: { type: "flectra_bar" },
                            },
                        },
                    },
                ],
    ...
    const migratedData = load(data);
    expect(migratedData.sheets[0].figures[0].data.type).toBe("nwos_line");
    expect(migratedData.sheets[0].figures[1].data.type).toBe("nwos_line");
    expect(migratedData.sheets[0].figures[2].data.chartDefinitions.chart3.type).toBe(
        "nwos_bar"
    );
    expect(migratedData.sheets[0].figures[2].data.chartDefinitions.chart4.type).toBe(
        "nwos_bar"
    );
    expect(migratedData.pivots["1"].type).toBe("NWOS");
    expect(migratedData.sheets[0].cells.A1).toBe('=NWOS.FILTER.LABEL("Period")');
    expect(migratedData.sheets[0].cells.A2).toBe('=NWOS.CURRENCY.RATE("EUR","USD")');
    expect(migratedData.sheets[0].cells.A3).toBe('=PIVOT.VALUE("1")');
});
...
test("Pivot formulas are migrated from 9 to 10", () => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=NWOS.PIVOT("1")` },
                    A2: { content: `=NWOS.PIVOT.HEADER("1")` },
                    A3: { content: `=NWOS.PIVOT.POSITION("1")` },
                    A4: { content: `=NWOS.PIVOT.TABLE("1")` },
                    A5: { content: `=nwos.pivot("1")` },
                },
            },
        ],
    };
    ...
    expect(migratedData.sheets[0].cells.A1).toBe(`=PIVOT.VALUE("1")`);
    expect(migratedData.sheets[0].cells.A2).toBe(`=PIVOT.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A3).toBe(`=NWOS.PIVOT.POSITION("1")`);
...
test("Pivot formulas using pivot positions are migrated (11 to 12)", () => {
        ...
        sheets: [
            {
                cells: {
                    A1: {
                        content: `=-PIVOT.VALUE("1","balance","account_id",NWOS.PIVOT.POSITION("1","account_id",12),"date:quarter","4/"&NWOS.FILTER.VALUE("Year"))`,
                    },
                    A2: {
                        content: `=PIVOT.HEADER("1","account_id",NWOS.PIVOT.POSITION("1","account_id",14))`,
                    },
                    A3: { content: `=NWOS.PIVOT.POSITION("1","account_id",14)` },
                    A4: { content: `=NWOS.PIVOT.POSITION("1",14)` },
                },
            },
        ],
    };
    const migratedData = load(data);
    expect(migratedData.sheets[0].cells.A1).toBe(
        `=-PIVOT.VALUE("1","balance","#account_id",12,"date:quarter","4/"&NWOS.FILTER.LABEL("Year"))`
    );
    expect(migratedData.sheets[0].cells.A2).toBe(`=PIVOT.HEADER("1","#account_id",14)`);
    expect(migratedData.sheets[0].cells.A3).toBe(`=NWOS.PIVOT.POSITION("1","account_id",14)`);
    expect(migratedData.sheets[0].cells.A4).toBe(`=NWOS.PIVOT.POSITION("1",14)`);
});
...
test("18.5.10: NWOS.FILTER.VALUE to NWOS.FILTER.LABEL in cells", () => {
    const data = {
        version: "18.4.14",
        sheets: [
            {
                cells: {
                    A1: '=NWOS.FILTER.VALUE("MyFilter")+nwos.filter.value("AnotherFilter")',
                },
            },
        ],
    };
    const migratedData = load(data);
    expect(migratedData.sheets[0].cells.A1).toBe(
        `=NWOS.FILTER.LABEL("MyFilter")+NWOS.FILTER.LABEL("AnotherFilter")`
    );
});
