...
import { animationFrame, mockDate, mockTimeZone } from "@nwos/hoot-mock";

import { DispatchResult, Model, helpers, tokenize, constants } from "@nwos/o-spreadsheet";
...
test("Adding new DataSource will set its fieldMatching according to other ones with the same model", async function () {
    ...
    const chartId = model.getters.getNWOSChartIds()[0];
    fieldMatching = model.getters.getNWOSChartFieldMatching(chartId, filterId);
    expect(fieldMatching.chain).toBe("created_on");
    expect(fieldMatching.type).toBe("date");
    expect(fieldMatching.offset).toBe(undefined);
});
...
test("NWOS.FILTER.VALUE text filter", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=NWOS.FILTER.VALUE("Text Filter")`);
...
test("NWOS.FILTER.VALUE numeric filter", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=NWOS.FILTER.VALUE("Numeric Filter")`);
    await animationFrame();
    expect(getCellValue(model, "A10")).toBe("#ERROR");
    await addGlobalFilter(model, {
        id: "42",
...
test("NWOS.FILTER.VALUE date filter", async function () {
    mockDate("2022-03-10 00:00:00");
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=NWOS.FILTER.VALUE("Date Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
...
test("NWOS.FILTER.VALUE date from/to without values", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(model, "A1", `=NWOS.FILTER.VALUE("Date Filter")`);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    expect(getEvaluatedCell(model, "B1").value).toBe("");
});

test("NWOS.FILTER.VALUE date from/to with only from defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=NWOS.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("NWOS.FILTER.VALUE date from/to with only to defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=NWOS.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("NWOS.FILTER.VALUE date from/to with from and to defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=NWOS.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("NWOS.FILTER.VALUE relation filter", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=NWOS.FILTER.VALUE("Relation Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "relation",
...
test("NWOS.FILTER.VALUE with escaped quotes in the filter label", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "text",
    ...
    setCellContent(model, "A1", '=NWOS.FILTER.VALUE("my \\"special\\" filter")');
    expect(getCellValue(model, "A1")).toBe("Jean-Jacques");
});

test("NWOS.FILTER.VALUE formulas are updated when filter label is changed", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(
        model,
        "A10",
        `=NWOS.FILTER.VALUE("Cuillère") & NWOS.FILTER.VALUE( "Cuillère" )`
    );
    const [filter] = model.getters.getGlobalFilters();
    const newFilter = {
        ...filter,
    ...
    expect(getCellFormula(model, "A10")).toBe(
        `=NWOS.FILTER.VALUE("Interprete") & NWOS.FILTER.VALUE("Interprete")`
    );
});
...
test("field matching is removed when list is deleted", async function () {
    ...
    model.dispatch("REMOVE_NWOS_LIST", { listId });
...
test("field matching is removed when an NWOS chart is deleted", async function () {
    ...
    expect(model.getters.getNWOSChartFieldMatching(chartId, filter.id)).toEqual(matching);
    model.dispatch("DELETE_FIGURE", {
        figureId: model.getters.getFigureIdFromChartId(chartId),
        sheetId,
    });
    expect(() => model.getters.getNWOSChartFieldMatching(chartId, filter.id)).toThrow(undefined, {
...
test("Updating the pivot domain should keep the global filter domain", async () => {
    ...
    model.dispatch("UPDATE_NWOS_PIVOT_DOMAIN", {
        pivotId,
        domain: [["foo", "in", [55]]],
    });
...
test("Updating a non-nwos pivot should not crash on global filter", async () => {
    const grid = {
        A1: "Customer",
        B1: "Price",
        C1: `=PIVOT(1)`,
