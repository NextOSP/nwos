...
import { animationFrame, mockDate } from "@nwos/hoot-mock";
...
test("NWOS.FILTER.LABEL text filter", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=NWOS.FILTER.LABEL("Text Filter")`);
...
test("NWOS.FILTER.LABEL empty date filter does't spill", async function () {
    mockDate("2022-03-10 00:00:00");
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=NWOS.FILTER.LABEL("Date Filter")`);
...
test("NWOS.FILTER.LABEL date filter", async function () {
    mockDate("2022-03-10 00:00:00");
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=NWOS.FILTER.LABEL("Date Filter")`);
    await animationFrame();
...
test("NWOS.FILTER.LABEL date from/to without values", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(model, "A1", `=NWOS.FILTER.LABEL("Date Filter")`);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    expect(getEvaluatedCell(model, "B1").value).toBe(null);
});

test("NWOS.FILTER.LABEL date from/to with only from defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=NWOS.FILTER.LABEL("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("NWOS.FILTER.LABEL date from/to with only to defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=NWOS.FILTER.LABEL("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("NWOS.FILTER.LABEL date from/to with from and to defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=NWOS.FILTER.LABEL("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("NWOS.FILTER.LABEL relation filter", async function () {
    const { model } = await createModelWithDataSource({
        mockRPC: function (route, { method, args }) {
            if (method === "web_search_read") {
                ...
                return {
                    records: resIds.map((resId) => ({ id: resId, display_name: names[resId] })),
                };
            }
        },
    });
    setCellContent(model, "A10", `=NWOS.FILTER.LABEL("Relation Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "relation",
...
test("NWOS.FILTER.LABEL with escaped quotes in the filter label", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "text",
    ...
    setCellContent(model, "A1", '=NWOS.FILTER.LABEL("my \\"special\\" filter")');
