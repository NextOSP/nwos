...
test("List export", async () => {
    ...
    expect(getCellFormula(model, "A1")).toBe(`=NWOS.LIST.HEADER(1,"foo")`);
    expect(getCellFormula(model, "B1")).toBe(`=NWOS.LIST.HEADER(1,"bar")`);
    expect(getCellFormula(model, "C1")).toBe(`=NWOS.LIST.HEADER(1,"date")`);
    expect(getCellFormula(model, "D1")).toBe(`=NWOS.LIST.HEADER(1,"product_id")`);
    expect(getCellFormula(model, "A2")).toBe(`=NWOS.LIST(1,1,"foo")`);
    expect(getCellFormula(model, "B2")).toBe(`=NWOS.LIST(1,1,"bar")`);
    expect(getCellFormula(model, "C2")).toBe(`=NWOS.LIST(1,1,"date")`);
    expect(getCellFormula(model, "D2")).toBe(`=NWOS.LIST(1,1,"product_id")`);
    expect(getCellFormula(model, "A3")).toBe(`=NWOS.LIST(1,2,"foo")`);
    expect(getCellFormula(model, "A11")).toBe(`=NWOS.LIST(1,10,"foo")`);
    expect(getCellFormula(model, "A12")).toBe("");
});

test("List field name should not be empty", async () => {
    const { model } = await createSpreadsheetWithList();
    setCellContent(model, "A1", `=NWOS.LIST(1,1,"")`);
    expect(getCellValue(model, "A1")).toBe("#ERROR");
    expect(getEvaluatedCell(model, "A1").message).toBe("The field name should not be empty.");
});

test("NWOS.LIST.HEADER with a custom header string", async () => {
    const { model } = await createSpreadsheetWithList();
    setCellContent(model, "A1", '=NWOS.LIST.HEADER(1,"foo","My custom header")');
...
test("Can display a field which is not in the columns", async function () {
    const { model } = await createSpreadsheetWithList();
    setCellContent(model, "A1", `=NWOS.LIST(1,1,"active")`);
...
test("Can remove a list with undo after editing a cell", async function () {
    const { model } = await createSpreadsheetWithList();
    expect(getCellContent(model, "B1").startsWith("=NWOS.LIST.HEADER")).toBe(true);
...
test("Json fields are not supported in list formulas", async function () {
    ...
    setCellContent(model, "A1", `=NWOS.LIST(1,1,"foo")`);
    setCellContent(model, "A2", `=NWOS.LIST(1,1,"jsonField")`);
    await waitForDataLoaded(model);
...
test("can get a listId from cell formula with other numerical values", async function () {
    const { model } = await createSpreadsheetWithList();
    setCellContent(model, "A1", `=3*NWOS.LIST("1","1","foo")`);
    const sheetId = model.getters.getActiveSheetId();
    const listId = model.getters.getListIdFromPosition({ sheetId, col: 0, row: 0 });
    expect(listId).toBe("1");
});
...
test("can get a listId from cell formula within a formula", async function () {
    const { model } = await createSpreadsheetWithList();
    setCellContent(model, "A1", `=SUM(NWOS.LIST("1","1","foo"),1)`);
    const sheetId = model.getters.getActiveSheetId();
    const listId = model.getters.getListIdFromPosition({ sheetId, col: 0, row: 0 });
    expect(listId).toBe("1");
});
...
test("can get a listId from cell formula where the id is a reference", async function () {
    const { model } = await createSpreadsheetWithList();
    setCellContent(model, "A1", `=NWOS.LIST(G10,"1","foo")`);
...
test("Referencing non-existing fields does not crash", async function () {
    ...
    await animationFrame();
    setCellContent(model, "A1", `=NWOS.LIST.HEADER("1", "${forbiddenFieldName}")`);
    setCellContent(model, "A2", `=NWOS.LIST("1","1","${forbiddenFieldName}")`);
...
test("rename list with empty name is refused", async () => {
    const { model } = await createSpreadsheetWithList();
    const result = model.dispatch("RENAME_NWOS_LIST", {
        listId: "1",
        name: "",
    });
...
test("Undo/Redo for RENAME_NWOS_LIST", async function () {
    const { model } = await createSpreadsheetWithList();
    expect(model.getters.getListName("1")).toBe("List");
    model.dispatch("RENAME_NWOS_LIST", { listId: "1", name: "test" });
...
test("Can delete list", async function () {
    const { model } = await createSpreadsheetWithList();
    model.dispatch("REMOVE_NWOS_LIST", { listId: "1" });
    expect(model.getters.getListIds().length).toBe(0);
...
test("can update a list", async () => {
    ...
    model.dispatch("UPDATE_NWOS_LIST", {
        listId,
        list: newListDef,
    });
    await waitForDataLoaded(model);
...
test("can edit list domain", async () => {
    ...
    model.dispatch("UPDATE_NWOS_LIST_DOMAIN", {
        listId,
        domain: [["foo", "in", [55]]],
    });
...
test("editing the sorting of a list of that does not exist should throw an error", async () => {
    const { model } = await createSpreadsheetWithList();
    const result = model.dispatch("UPDATE_NWOS_LIST", {
        listId: "invalid",
        list: undefined,
    });
    expect(result.reasons).toEqual([CommandResult.ListIdNotFound]);
...
test("edited domain is exported", async () => {
    ...
    const result = model.dispatch("UPDATE_NWOS_LIST_DOMAIN", {
        listId: "invalid",
        domain: [],
    });
    expect(result.reasons).toEqual([CommandResult.ListIdNotFound]);
...
test("Can see record with link to list cell", async function () {
    ...
    setCellContent(model, "A1", '=NWOS.LIST(1, 1, "foo")');
    setCellContent(model, "A2", '=NWOS.LIST(1, 2, "foo")');
...
test("Can see record on vectorized list index", async function () {
    ...
    setCellContent(model, "A1", '=NWOS.LIST(1, C1:D2, "foo")');
    const seeRecordAction = cellMenuRegistry.getAll().find((item) => item.id === "list_see_record");
...
test("add currency field after the list has been loaded", async function () {
    ...
    setCellContent(model, "A1", '=NWOS.LIST(1, 1, "pognon")');
    await waitForDataLoaded(model);
    setCellContent(model, "A2", '=NWOS.LIST(1, 1, "currency_id")');
    await waitForDataLoaded(model);
        ...
        sheets: [
            {
                id: "sheet1",
                cells: {
                    A1: '=NWOS.LIST(1, 1, "foo")', // in the definition
                    A2: '=NWOS.LIST(1, 1, "product_id")', // not in the definition
                    A3: '=NWOS.LIST(1, 1, "invalid_field")',
                    A4: '=NWOS.LIST.HEADER(1, "bar")',
                },
            },
        ],
        lists: {
        ...
        sheets: [
            {
                id: "sheet1",
                cells: {
                    A1: '=NWOS.LIST(1, 11, "foo")',
                    A2: '=NWOS.LIST(1, A3, "foo")',
                    A3: "111",
                },
            },
        ],
...
test("List record limit is computed during the import and UPDATE_CELL", async function () {
    ...
    setCellContent(model, "A1", `=NWOS.LIST("1", "42", "foo")`);
...
test("Spec of web_search_read is minimal", async function () {
    ...
    setCellContent(model, "A1", '=NWOS.LIST(1, 1, "pognon")');
    setCellContent(model, "A2", '=NWOS.LIST(1, 1, "currency_id")');
    setCellContent(model, "A3", '=NWOS.LIST(1, 1, "foo")');
    await waitForDataLoaded(model);
    expect.verifySteps(["web_search_read"]);
});
...
test("Can duplicate a list", async () => {
    ...
    model.dispatch("DUPLICATE_NWOS_LIST", { listId, newListId: "2" });
...
test("Cannot duplicate unknown list", async () => {
    const { model } = await createSpreadsheetWithList();
    const result = model.dispatch("DUPLICATE_NWOS_LIST", {
        listId: "hello",
...
test("isListUnused getter", async () => {
    ...
    setCellContent(model, "A1", '=NWOS.LIST.HEADER(1, "foo")');
    expect(model.getters.isListUnused("1")).toBe(false);

    setCellContent(model, "A1", '=NWOS.LIST.HEADER(A2, "foo")');
    expect(model.getters.isListUnused("1")).toBe(true);
...
test("INSERT_NWOS_LIST_WITH_TABLE adds a table that maches the list dimension", async function () {
    const { model } = await createSpreadsheetWithList({
        linesNumber: 4,
    });
    const sheetId = model.getters.getActiveSheetId();
    ...
    model.dispatch("INSERT_NWOS_LIST_WITH_TABLE", {
        sheetId,
        col,
        row,
...
test("Support field chaining in list", async function () {
    const { model } = await createSpreadsheetWithList();
    const listId = model.getters.getListIds()[0];
    setCellContent(model, "A1", `=NWOS.LIST(${listId}, 1, "product_id.id")`);
    await animationFrame();
...
test("Support many2many field chaining in list", async function () {
    ...
    const { model } = await createSpreadsheetWithList();
    const listId = model.getters.getListIds()[0];
    setCellContent(model, "A1", `=NWOS.LIST(${listId}, 1, "user_ids.id")`);
    setCellContent(model, "A2", `=NWOS.LIST(${listId}, 1, "user_ids.group_ids.id")`);
    await animationFrame();
...
test("Invalid field chaining in list should be marked as such", async function () {
    const { model } = await createSpreadsheetWithList();
    const listId = model.getters.getListIds()[0];
    setCellContent(model, "A1", `=NWOS.LIST(${listId}, 1, "product_id.id.id")`);
    await animationFrame();
    expect(getCellValue(model, "A1")).toBe("#ERROR");
...
test("Field chaining can be more than 1 deep", async function () {
    const { model } = await createSpreadsheetWithList();
    const listId = model.getters.getListIds()[0];
    setCellContent(model, "A1", `=NWOS.LIST(${listId}, 2, "product_id.template_id.name")`);
    await animationFrame();
...
test("Chaining fields are fetched with the same web_search_read", async function () {
    ...
    const listId = model.getters.getListIds()[0];
    setCellContent(model, "A1", `=NWOS.LIST(${listId}, 1, "product_id.template_id.name")`);
    initialLoad = false;
    await animationFrame();
    expect.verifySteps(["web_search_read"]);
});
...
test("Chaining monetary fields includes the currency field", async function () {
    ...
    const listId = model.getters.getListIds()[0];
    setCellContent(model, "A1", `=NWOS.LIST(${listId}, 1, "product_id.pognon")`);
    initialLoad = false;
    await animationFrame();
...
test("List header labels are loaded even if there are no corresponding list values", async function () {
    ...
    const listId = model.getters.getListIds()[0];
    setCellContent(model, "A1", `=NWOS.LIST.HEADER(${listId}, "currency_id")`);
    setCellContent(model, "B1", `=NWOS.LIST.HEADER(${listId}, "product_id.template_id.name")`);
    await animationFrame();
...
test("List Headers with invalid field names do not trigger infinite loops", async function () {
    const { model } = await createSpreadsheetWithList();
    const listId = model.getters.getListIds()[0];
    setCellContent(model, "A1", `=NWOS.LIST.HEADER(${listId}, "")`);
    setCellContent(model, "A2", `=NWOS.LIST.HEADER(${listId}, #REF)`);
    setCellContent(model, "A3", `=NWOS.LIST.HEADER(${listId}, Z4)`); // Z4 is empty
    setCellContent(model, "A4", `=NWOS.LIST.HEADER(${listId}, "notAField")`);
    await animationFrame();
    expect(getCellValue(model, "A1")).toBe("#ERROR");
...
test("List with invalid field names do not trigger infinite loops", async function () {
    const { model } = await createSpreadsheetWithList();
    const listId = model.getters.getListIds()[0];
    setCellContent(model, "A1", `=NWOS.LIST(${listId}, 1, "")`);
    setCellContent(model, "A2", `=NWOS.LIST(${listId}, 1, #REF)`);
    setCellContent(model, "A3", `=NWOS.LIST(${listId}, 1, Z4)`); // Z4 is empty
    setCellContent(model, "A4", `=NWOS.LIST(${listId}, 1, "notAField")`);
    await animationFrame();
    expect(getCellValue(model, "A1")).toBe("#ERROR");
    expect(getCellValue(model, "A2")).toBe("#REF");
    expect(getCellValue(model, "A3")).toBe("#ERROR");
