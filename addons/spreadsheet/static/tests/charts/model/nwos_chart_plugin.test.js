...
import { NWOSBarChart } from "@spreadsheet/chart/nwos_chart/nwos_bar_chart";
import { NWOSChart } from "@spreadsheet/chart/nwos_chart/nwos_chart";
import { NWOSLineChart } from "@spreadsheet/chart/nwos_chart/nwos_line_chart";
...
test("Can add an NWOS Bar chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_bar" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof NWOSBarChart).toBe(true);
...
test("Can add an NWOS Line chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_line" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof NWOSLineChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
    expect(model.getters.getChartRuntime(chartId).chartJsConfig.type).toBe("line");
});

test("Can load legacy NWOS Line chart types", () => {
    for (const legacyType of ["odoo_line", "flectra_line"]) {
        const builder = chartRegistry.getAll().find((builder) => builder.match(legacyType));
        expect(builder).not.toBe(undefined);

        const chart = builder.createChart(
            { ...cumulativeChartDefinition, type: legacyType },
            "Sheet1",
            {}
        );
        expect(chart instanceof NWOSLineChart).toBe(true);
        expect(chart.getDefinition().type).toBe("nwos_line");
    }
});

test("Can add an NWOS Pie chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_pie" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof NWOSChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
...
test("NWOS bar chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "nwos_bar",
...
test("NWOS pie chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "nwos_pie",
        mockRPC: async function (route, args) {
            if (args.method === "formatted_read_group") {
...
test("NWOS line chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "nwos_line",
        mockRPC: async function (route, args) {
            if (args.method === "formatted_read_group") {
...
test("Data reloaded upon domain update for charts other than pie/bar/line", async () => {
    ...
    updateChart(model, chartId, { type: "nwos_pie" });
    await waitForDataLoaded(model);
...
test("Can import/export an NWOS chart", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "nwos_line");
    const data = model.exportData();
    const figures = data.sheets[0].figures;
    expect(figures.length).toBe(1);
    const figure = figures[0];
    expect(figure.tag).toBe("chart");
    expect(figure.data.type).toBe("nwos_line");
...
test("Can undo/redo an NWOS chart creation", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "nwos_line");
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("charts with no legend", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_pie" });
    insertChartInSpreadsheet(model, "nwos_bar");
    insertChartInSpreadsheet(model, "nwos_line");
    const sheetId = model.getters.getActiveSheetId();
...
test("Can copy/paste NWOS chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[1]) instanceof NWOSChart).toBe(true);
...
test("Can cut/paste NWOS chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[0]) instanceof NWOSChart).toBe(true);
    expect(JSON.stringify(model.getters.getChartRuntime(chartIds[0]))).toBe(
        JSON.stringify(chartRuntime)
    );
});

test("Duplicating a sheet correctly duplicates NWOS chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Load nwos chart spreadsheet with models that cannot be accessed", async function () {
...
test("Can insert nwos chart from a different model", async () => {
    const { model } = await createModelWithDataSource();
...
test("NWOS chart legend color changes with background color update", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
...
test("Remove nwos chart when sheet is deleted", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_line" });
    const sheetId = model.getters.getActiveSheetId();
    ...
    expect(model.getters.getNWOSChartIds().length).toBe(1);
    model.dispatch("DELETE_SHEET", { sheetId });
    expect(model.getters.getNWOSChartIds().length).toBe(0);
});

test("NWOS chart datasource display name has a default when the chart title is empty", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
    expect(model.getters.getNWOSChartDisplayName(chartId)).toBe("(#1) Partners");
    model.dispatch("UPDATE_CHART", {
        definition: {
            ...definition,
            title: { text: "" },
    ...
    expect(model.getters.getNWOSChartDisplayName(chartId)).toBe("(#1) NWOS Line Chart");
});

test("Every NWOS chart type has a default title", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const figureId = model.getters.getFigureIdFromChartId(chartId);
    const chartTypes = chartRegistry.getKeys().filter((type) => type.startsWith("nwos_"));
    model.dispatch("UPDATE_CHART", {
    ...
    for (const chartType of chartTypes) {
        ...
        expect(model.getters.getNWOSChartDisplayName(chartId)).toBe(`(#1) NWOS ${chartName} Chart`);
    }
});
...
test("NWOS line and bar charts display only horizontal grid lines", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "nwos_line",
    });
    const sheetId = model.getters.getActiveSheetId();
...
test("Can configure the chart datasets", async () => {
    ...
    const { model } = await createSpreadsheetWithChart({
        type: "nwos_bar",
        serverData,
        definition: { type: "nwos_bar", metaData, searchParams, id: "42" },
    });
    await waitForDataLoaded(model);
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("Non-web chart types are using data source in 'bar' mode", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "nwos_radar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
