...
import { expect, test, beforeEach } from "@nwos/hoot";
...
test("icon external link isn't on the chart when its not linked to an nwos menu", async function () {
    ...
    await animationFrame();
    const nwosMenu = model.getters.getChartNWOSMenu(chartId);
    expect(nwosMenu).toBe(undefined, { message: "No menu linked with the chart" });

    const externalRefIcon = fixture.querySelector(".o-chart-external-link");
    expect(externalRefIcon).toBe(null);
});

test("icon external link is on the chart when its linked to an nwos menu", async function () {
    const { model } = await createModelWithDataSource({
        serverData,
    });
    await mountSpreadsheet(model);
    createBasicChart(model, chartId);
    model.dispatch("LINK_NWOS_MENU_TO_CHART", {
        chartId,
        nwosMenuId: 1,
    });

    const chartMenu = model.getters.getChartNWOSMenu(chartId);
    expect(chartMenu.id).toBe(1, { message: "NWOS menu is linked to chart" });
    await animationFrame();
    expect(".o-chart-external-link").toHaveCount(1);
});

test("icon external link is not on the chart when its linked to a wrong nwos menu", async function () {
    const { model } = await createModelWithDataSource({
        serverData,
    });
    await mountSpreadsheet(model);
    ...
    model.dispatch("LINK_NWOS_MENU_TO_CHART", {
        chartId,
        nwosMenuId: "menu which does not exist",
    });
    const chartMenu = model.getters.getChartNWOSMenu(chartId);
...
test("click on icon external link on chart redirect to the nwos menu", async function () {
    ...
    const chartMenu = model.getters.getChartNWOSMenu(chartId);
    expect(chartMenu.id).toBe(2, { message: "NWOS menu is linked to chart" });
    await animationFrame();
...
test("can use menus xmlIds instead of menu ids", async function () {
    ...
    model.dispatch("LINK_NWOS_MENU_TO_CHART", {
        chartId,
        nwosMenuId: "spreadsheet.test.menu2",
    });
    await animationFrame();
...
test("Trying to open a menu without an action sends a notification to the user", async function () {
    ...
    model.dispatch("LINK_NWOS_MENU_TO_CHART", {
        chartId,
        nwosMenuId: "spreadsheet.test.menu_without_action",
    });
    await animationFrame();
