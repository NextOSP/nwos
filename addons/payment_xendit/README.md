...
- For `Card` payments, it renders a self-hosted payment form with regular (non-iframe) inputs and 
  ...
  token is saved in NWOS. Other communications with Xendit are performed via server-to-server API
  calls.
...
- `17.4`
  - The support for tokenization via `Card` is added. nwos/nwos#158445
- `17.0`
  - The first version of the module is merged. nwos/nwos#141661
...
createdb nwos
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -i payment_xendit
```
...
```bash
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -u payment_xendit
```
