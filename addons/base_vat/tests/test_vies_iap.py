...
from nwos.tests.common import HttpCase, tagged
...
class TestViesIAP(HttpCase):
    ...
    def test_vies_iap_valid_vat(self):
        ...
        with (
            self.assertLogs('nwos.addons.base_vat.models.res_partner', logging.INFO) as log_catcher,
        ):
    ...
    def test_vies_iap_controller(self):
        ...
        Upon having a new lookup status, it will call the NWOS client db with the updated status
        """
        self.mock_return_status = "pending"
        self.partner.vat = self.RANDOM_VAT
        self.partner.flush_recordset()  # trigger computes
        ...
        # At this point, the NWOS db is passively waiting for an update coming from IAP via the webhook
        ...
        with (
            self.assertLogs('nwos.addons.base_vat.controllers.webhook', level="WARNING") as log_catcher,
        ):
            self.url_open(
    ...
    def test_vies_iap_cron(self):
        """
        Same as previous test, but for cases where the NWOS client db is unreachable (invalid
