...
from nwos.addons.iot_drivers.main import iot_devices, manager
from nwos.addons.iot_drivers.tools import helpers, upgrade, wifi
from nwos.addons.iot_drivers.tools.system import IS_RPI, IS_TEST
...
class ConnectionManager(Thread):
    ...
    def _register_iot_box(self):
        """ This method is called to register the IoT Box on nwos.com and get a pairing code"""
    ...
    def _get_next_polling_interval(self):
        # To avoid spamming nwos.com with requests we gradually space out the requests
    ...
    def _should_poll_to_connect_database(self):
        return (
            not helpers.get_nwos_server_url() and
            helpers.get_ip() and
    ...
    def _call_iot_proxy(self):
        ...
        try:
            req = requests.post(
                'https://iot-proxy.nwos.com/nwos-enterprise/iot/connect-box',
                json=data,
                timeout=5,
            )
            ...
            _logger.exception('Could not reach iot-proxy.nwos.com')
            return {}
