...
from nwos.addons.iot_drivers.tools.system import IS_RPI, IS_TEST, IS_WINDOWS
...
def ensure_validity():
    ...
    """
    inform_database(get_certificate_end_date() or download_nwos_certificate())
...
def get_certificate_end_date():
    ...
    if (
        common_name == 'NWOSTempIoTBoxCertificate'
...
def download_nwos_certificate(retry=0):
    """Send a request to NWOS with customer db_uuid and enterprise_code
    ...
    try:
        response = requests.post(
            'https://www.nwos.com/nwos-enterprise/iot/x509',
        ...
        _logger.warning("An error occurred while trying to reach nwos.com to get a new certificate: %s", e)
        if retry < 5:
            return download_nwos_certificate(retry=retry + 1)
        return _logger.exception("Maximum attempt to download the nwos.com certificate reached")

    server_error = response_body.get('error')
    if server_error:
        _logger.error("Server error received from nwos.com while trying to get the certificate: %s", server_error)
        return None

    result = response_body.get('result', {})
    certificate_error = result.get('error')
    if certificate_error:
        _logger.warning("Error received from nwos.com while trying to get the certificate: %s", certificate_error)
        return None
    ...
    if not certificate or not private_key:  # ensure not empty strings
        _logger.error("The certificate received from nwos.com is not valid.")
        return None
...
def inform_database(ssl_certificate_end_date, server_url=None):
    ...
    :param str server_url: URL of the NWOS server (provided by decorator).
    """
