...
from nwos.addons.iot_drivers.tools.system import (
    IOT_CHAR,
    IOT_RPI_CHAR,
    IOT_WINDOWS_CHAR,
    IS_RPI,
...
from nwos.tools.func import reset_cached_properties
...
def require_db(function):
    ...
    def wrapper(*args, **kwargs):
        fname = f"<function {function.__module__}.{function.__qualname__}>"
        server_url = get_nwos_server_url()
        iot_box_ip = get_ip()
...
def check_image():
    ...
    """
    try:
        response = requests.get('https://nightly.nwos.com/master/iotbox/SHA1SUMS.txt', timeout=5)
...
def delete_iot_handlers():
    ...
    try:
        ...
        filenames = [
            f"nwos/addons/iot_drivers/iot_handlers/{file.relative_to(iot_handlers)}"
...
def download_iot_handlers(auto=True, server_url=None):
    ...
    :param server_url: The URL of the connected NWOS database (provided by decorator).
    """
    ...
    delete_iot_handlers()
    path = path_file('nwos', 'addons', 'iot_drivers', 'iot_handlers')
    zip_file.extractall(path)


def compute_iot_handlers_addon_name(handler_kind, handler_file_name):
    return "nwos.addons.iot_drivers.iot_handlers.{handler_kind}.{handler_name}".\
        format(handler_kind=handler_kind, handler_name=handler_file_name.removesuffix('.py'))


def load_iot_handlers():
    """
    This method loads local files: 'nwos/addons/iot_drivers/iot_handlers/drivers' and
    'nwos/addons/iot_drivers/iot_handlers/interfaces'
...
nwos_start_time = time.monotonic()
system_start_time = nwos_start_time - _get_system_uptime()
...
def toggle_remote_connection(token=""):
    ...
    if p.returncode == 0:
        subprocess.run(
            ['sudo', 'systemctl', 'restart' if token else "stop", 'nwos-ngrok.service'],
