"""Module to manage nwos code upgrades using git"""
...
import requests
import subprocess
from nwos.addons.iot_drivers.tools.helpers import (
    nwos_restart,
    path_file,
    require_db,
    toggleable,
...
from nwos.addons.iot_drivers.tools.system import rpi_only, IS_RPI, IS_TEST
...
def git(*args):
    ...
    command = [git_executable, f'--work-tree={path_file("nwos")}', f'--git-dir={path_file("nwos", ".git")}', *args]
...
def get_db_branch(server_url):
    ...
    :param server_url: The URL of the connected NWOS database.
    ...
    try:
        ...
        _logger.exception('Could not reach configured server to get the NWOS version')
        return None
    try:
...
def check_version_upgrades(local_branch, db_branch):
    ...
    try:
        ...
        subprocess.run(
            ['/home/pi/nwos/addons/iot_drivers/tools/upgrade_scripts/upgrade_trixie/upgrade_trixie.sh'], check=True,
        )
...
def check_git_branch(server_url=None):
    """Check if the local branch is the same as the connected NWOS DB and
    ...
    try:
        ...
        _logger.info("IoT Box git branch: %s / Associated NWOS db's git branch: %s", local_branch, db_branch)

        if db_branch != local_branch:
            # Repository updates
            unlink_file("nwos/.git/shallow.lock")  # In case of previous crash/power-off, clean old lockfile
...
def _ensure_production_remote():
    ...
    (https://github.com/nwos/nwos.git).
    """
    production_remote = "https://github.com/nwos/nwos.git"
...
def update_requirements():
    ...
    """
    requirements_file = path_file('nwos', 'addons', 'iot_box_image', 'configuration', 'requirements.txt')
...
def update_packages():
    ...
    """
    packages_file = path_file('nwos', 'addons', 'iot_box_image', 'configuration', 'packages.txt')
...
def misc_migration_updates():
    ...
    if path_file('nwos', 'addons', 'point_of_sale').exists():
    ...
    if path_file('nwos', 'addons', 'hw_drivers').exists():
        # TODO: remove this when v18.4 is deprecated (hw_drivers/,hw_posbox_homepage/ -> iot_drivers/)
        subprocess.run(
            ['sed', '-i', 's|iot_drivers|hw_drivers,hw_posbox_homepage|g', '/home/pi/nwos.conf'], check=False
        )
