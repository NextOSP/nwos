...
from nwos.addons.iot_drivers.tools import helpers
from nwos.addons.iot_drivers.tools.system import IS_TEST
from nwos.netsvc import ColoredFormatter

_logger = logging.getLogger(__name__)

IOT_LOG_TO_SERVER_CONFIG_NAME = 'iot_log_to_server'  # config name in nwos.conf
...
class AsyncHTTPHandler(logging.Handler):
    ...
    def __init__(self, nwos_server_url, active):
        """
        :param nwos_server_url: NWOS Server URL
        """
        super().__init__()
        self._nwos_server_url = nwos_server_url
    ...
    def toggle_active(self, is_active):
        ...
        if self._active and self._nwos_server_url:
    ...
    def _periodic_flush(self):
        nwos_session = requests.Session()
        while self._nwos_server_url and self._active:  # allow to exit the loop on thread.join
            time.sleep(self._FLUSH_INTERVAL)
            self._flush_logs(nwos_session)

    def _flush_logs(self, nwos_session):
        ...
        if not self._nwos_server_url or queue_size == 0:
            return
        try:
            nwos_session.post(
                self._nwos_server_url + '/iot/log',
                data=empty_queue(),
                headers={'X-NWOS-Database': self._db_name},
...
def get_nwos_config_log_to_server_option():
...
def check_and_update_nwos_config_log_to_server_option(new_state):
    """
    :return: wherever the config file need to be updated or not
    """
    if get_nwos_config_log_to_server_option() != new_state:
...
_server_log_sender_handler = AsyncHTTPHandler(helpers.get_nwos_server_url(), get_nwos_config_log_to_server_option())
if not IS_TEST:
    ...
    # Set it in the 'root' logger, on which every logger (including nwos) is a child
