...
from nwos.addons.iot_drivers import main
from nwos.addons.iot_drivers.tools import helpers, upgrade
from nwos.addons.iot_drivers.server_logger import close_server_log_sender_handler
from nwos.addons.iot_drivers.webrtc_client import webrtc_client
...
def send_to_controller(params, method="send_websocket", server_url=None):
    """Confirm the operation's completion by sending a response back to the NWOS server

    :param params: the parameters to send back to the server
    :param method: method to call on the IoT box controller
    :param server_url: URL of the NWOS server (provided by decorator).
    """
...
class WebsocketClient(Thread):
        ...
        for message in json.loads(messages):
            ...
            match message['message']['type']:
                ...
                case 'server_update':
                    ...
                    helpers.get_nwos_server_url.cache_clear()
                case 'restart_nwos':
                    send_to_controller({
    ...
    def run(self):
        ...
        self.ws = websocket.WebSocketApp(self.websocket_url,
            header={"User-Agent": "NWOSIoTBox/1.0", "Cookie": f"session_id={self.session_id}"},
