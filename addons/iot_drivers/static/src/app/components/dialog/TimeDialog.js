...
export class TimeDialog extends Component {
    ...
    setup() {
        this.store = useStore();
        this.state = useState({
            nwosUptimeSeconds: this.store.base.nwos_uptime_seconds,
            systemUptimeSeconds: this.store.base.system_uptime_seconds,
        });
        setInterval(() => {
            this.state.nwosUptimeSeconds += 1;
    ...
    startDateFromSeconds(uptimeInSeconds) {
        ...
        const nwosUptimeMs = uptimeInSeconds * 1000;
        return new Date(currentTimeMs - nwosUptimeMs).toUTCString();
    }
        ...
        <BootstrapDialog identifier="'time-dialog'" btnName="'View Uptime'">
            ...
            <t t-set-slot="body">
                <div class="d-flex flex-column gap-4">
                  <div>
                    <h5>NWOS Service</h5>
                    <div>Running for <b t-out="secondsToHumanReadable(state.nwosUptimeSeconds)"/></div>
                    <div class="text-secondary">Started at <b t-out="startDateFromSeconds(state.nwosUptimeSeconds)"/></div>
                  </div>
