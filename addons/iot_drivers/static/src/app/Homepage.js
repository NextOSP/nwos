...
export class Homepage extends Component {
    ...
    async restartNWOSService() {
        try {
            await this.store.rpc({
                url: "/iot_drivers/restart_nwos_service",
            });

            this.state.waitRestart = true;
        } catch {
            console.warn("Error while restarting NWOS Service");
        }
    }
        ...
        <div t-if="!this.state.loading" class="w-100 d-flex flex-column align-items-center justify-content-center background">
            <div class="bg-white p-4 rounded overflow-auto position-relative w-100 main-container">
                <div class="position-absolute end-0 top-0 mt-3 me-4 d-flex gap-1">
                    ...
                    <IconButton onClick.bind="restartNWOSService" icon="'fa-power-off'" />
                </div>
                ...
                <SingleData t-if="!store.base.is_access_point_up" name="'NWOS database connected'" value="state.data.server_status" icon="'fa-link'">
                    <t t-set-slot="button">
                        <ServerDialog />
                    </t>
                </SingleData>
                <SingleData t-if="state.data.pairing_code and !this.store.base.is_access_point_up and !state.data.pairing_code_expired" name="'Pairing Code'" value="state.data.pairing_code + ' - Enter this code in the IoT app in your NWOS database'" icon="'fa-code'"/>
                ...
                <div class="d-flex justify-content-center gap-2 mt-2" t-if="!store.base.is_access_point_up">
                    <a href="https://www.nwos.com/fr_FR/help" target="_blank" class="link-primary">Help</a>
                    <a href="https://www.nwos.com/documentation/latest/applications/general/iot.html" target="_blank" class="link-primary">Documentation</a>
                </div>
            </div>
        </div>
