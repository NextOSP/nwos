...
from nwos.addons.iot_drivers.controllers.proxy import proxy_drivers
from nwos.addons.iot_drivers.iot_handlers.drivers.printer_driver_base import PrinterDriverBase
from nwos.addons.iot_drivers.main import iot_devices
from nwos.addons.iot_drivers.tools import helpers, wifi, route
...
class PrinterDriver(PrinterDriverBase):
    ...
    def print_raw(self, data, action_unique_id=None):
        ...
        try:
            with self.cups_lock:
                job_id = self.conn.createJob(self.device_identifier, 'NWOS print job', {'document-format': CUPS_FORMAT_AUTO})
                self.conn.startDocument(self.device_identifier, job_id, 'NWOS print job', CUPS_FORMAT_AUTO, 1)
    ...
    def print_status_zpl(self):
        ...
        title = "IoT Box Connected" if helpers.get_nwos_server_url() else "IoT Box Status"
    ...
    def _printer_status_content(self):
        ...
        wan_quality = helpers.check_network("www.nwos.com")
        ...
        title = b'IoT Box Connected' if helpers.get_nwos_server_url() else b'IoT Box Status'
