...
from nwos.tools.config import config
from nwos.addons.iot_drivers.tools import route
from nwos.addons.iot_drivers.tools.system import IOT_SYSTEM, IS_RPI, IS_WINDOWS
...
class EtaUsbController(http.Controller):
    ...
    def eta_certificate(self, pin, access_token):
        """Gets the certificate from the token and returns it to the main nwos instance so that we can prepare the
        cades-bes object on the main nwos instance rather than this middleware

        :param pin: pin of the token
        :param access_token: token shared with the main nwos instance
