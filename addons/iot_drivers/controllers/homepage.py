...
from nwos.addons.iot_drivers.tools import certificate, helpers, route, upgrade, wifi
from nwos.addons.iot_drivers.tools.system import IOT_SYSTEM, IS_RPI
from nwos.addons.iot_drivers.main import iot_devices, unsupported_devices
from nwos.addons.iot_drivers.connection_manager import connection_manager
from nwos.tools.misc import file_path
from nwos.addons.iot_drivers.server_logger import (
    check_and_update_nwos_config_log_to_server_option,
    get_nwos_config_log_to_server_option,
...
class IotBoxOwlHomePage(http.Controller):
    ...
    # ---------------------------------------------------------- #
    @route.iot_route('/iot_drivers/restart_nwos_service', type='http', cors='*')
    def nwos_service_restart(self):
        helpers.nwos_restart(0)
        return json.dumps({
            'status': 'success',
            'message': 'NWOS service restarted',
        })

    @route.iot_route('/iot_drivers/iot_logs', type='http', cors='*')
    def get_iot_logs(self):
        logs_path = "/var/log/nwos/nwos-server.log" if IS_RPI else Path().absolute().parent.joinpath('nwos.log')
    ...
    def get_homepage_data(self):
        ...
        nwos_server_url = helpers.get_nwos_server_url() or ''
        nwos_uptime_seconds = time.monotonic() - helpers.nwos_start_time
        ...
        return json.dumps({
            ...
            'server_status': nwos_server_url,
            'pairing_code': connection_manager.pairing_code,
            'new_database_url': connection_manager.new_database_url,
            'pairing_code_expired': connection_manager.pairing_code_expired and not nwos_server_url,
            'six_terminal': six_terminal,
            'is_access_point_up': IS_RPI and wifi.is_access_point(),
            'network_interfaces': network_interfaces,
            'version': helpers.get_version(),
            'system': IOT_SYSTEM,
            'nwos_uptime_seconds': nwos_uptime_seconds,
    ...
    def get_version_info(self):
        ...
        return json.dumps({
            ...
            'nwosIsUpToDate': current_commit == last_available_commit or not bool(helpers.get_nwos_server_url()),
    ...
    def log_levels(self):
        ...
        return json.dumps({
            'title': "NWOS's IoT Box - Handlers list",
            'breadcrumb': 'Handlers list',
            'drivers_list': drivers_list,
            'interfaces_list': interfaces_list,
            'server': helpers.get_nwos_server_url(),
            'is_log_to_server_activated': get_nwos_config_log_to_server_option(),
            'root_logger_log_level': self._get_logger_effective_level_str(logging.getLogger()),
            'nwos_current_log_level': self._get_logger_effective_level_str(logging.getLogger('nwos')),
    ...
    def connect_to_nwos_server(self, token):
        if token:
            try:
    ...
    def update_log_level(self, name, value):
        ...
        if name == 'log-to-server':
            check_and_update_nwos_config_log_to_server_option(value)

        name = name[len(IOT_LOGGING_PREFIX):]
        if name == 'root':
            self._update_logger_level('', value, AVAILABLE_LOG_LEVELS)
        elif name == 'nwos':
            self._update_logger_level('nwos', value, AVAILABLE_LOG_LEVELS)
    ...
    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        """Update (if necessary) NWOS's configuration and logger to the given logger_name to the given level.
        ...
        NWOS_TOOL_CONFIG_HANDLER_NAME = 'log_handler'
        LOG_HANDLERS = (helpers.get_conf(NWOS_TOOL_CONFIG_HANDLER_NAME, section='options') or []).split(',')
        ...
        if not IS_NEW_LEVEL_PARENT:
            ...
            _logger.debug('Adding to nwos config log_handler: %s', new_entry)
        conf[NWOS_TOOL_CONFIG_HANDLER_NAME] = ','.join(log_handlers_without_logger)
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        """
        Get NWOS Iot logger given an IoT handler name
        ...
        """
        nwos_addon_handler_path = helpers.compute_iot_handlers_addon_name(handler_folder_name, handler_name)
        return nwos_addon_handler_path in logging.Logger.manager.loggerDict and \
               logging.getLogger(nwos_addon_handler_path)
