...
"""Pinguin module for nwos REST Api.
...
from nwos.addons.base_api.lib.pinguin import (
    error_response,
    get_dict_from_record,
    get_dictlist_from_model,
    get_model_for_read,
)
from nwos.addons.web.controllers.main import ReportController
...
def setup_db(httprequest, db_name):
    ...
    if httprequest.session.db:
        return
    if db_name not in nwos.service.db.list_dbs(force=True):
        raise werkzeug.exceptions.HTTPException(
...
def create_log_record(**kwargs):
    ...
    # transaction)
    if not test_mode:
        with nwos.registry(request.session.db).cursor() as cr:
            # use new to save data even in case of an error in the old cursor
            env = nwos.api.Environment(cr, request.session.uid, {})
...
def get_model_openapi_access(namespace, model):
    ...
    :returns: The error response object if namespace validation failed.
        A dictionary containing the model API configuration for this namespace.
            ...
            ```python
            {'context':                 (Dict)      nwos context (default values through context),
...
def wrap__resource__create_one(modelname, context, data, success_code, out_fields):
    ...
    try:
        ...
        if not test_mode:
            ...
            # Probably, nwos (10.0 at least) uses different cursors
...
def get_OAS_definitions_part(
    ...
    :param nwos.models.Model model_obj: The model object.
    ...
    for field, child_fields in export_fields_dict.items():
        ...
        if child_fields:
            ...
            # definition, for that reason we cannot blindly use nwos's
            # attributed readonly and required.
            #
            # 1) For nwos-required, but NOT nwos-related field, we do NOT use
            # openapi-readonly
            #
            ...
            #     False)]}, required=True, ...)
            #
            # 2) For nwos-required and nwos-related field, we DO use
