...
from nwos.http import Controller, route
from nwos.modules import Manifest
from nwos.modules import module as nwos_modules_module
from nwos.release import series as release_series
from nwos.sql_db import db_connect
from nwos.tools import SQL

from nwos.addons.base.models.res_users import _check_apikey_credentials
        ...
        for kpi_provider in manifest.get('kpi_providers', []):
            ...
            try:
                if module_path:
                    ...
                    module = import_module(f'.{module_path}', package=f'nwos.addons.{manifest.name}')
                else:
                    # Support ":function" for root-level function
                    module = import_module(f'nwos.addons.{manifest.name}')
                if not function:
...
def _db_kpi_summary(database, api_key):
    ...
    ensures the database version matches the current NWOS release, and
    ...
    with cursor as cr:
        ...
        for addon, kpi_provider, get_kpi_summary in _get_kpi_providers():
            try:
                ...
                if not nwos_modules_module.current_test:
                    cr.rollback()
...
class KpiController(Controller):
    ...
    def kpi_summary(self, credentials):
        ...
        The result of this call will only include the databases:
            ...
            - that are on the same NWOS version as the current NWOS
