...
import { Deferred, manuallyDispatchProgrammaticEvent, runAllTimers, waitFor } from "@nwos/hoot-dom";
import { mockWebSocket } from "@nwos/hoot-mock";
import {
    asyncStep,
    contains,
    getService,
...
test("websocket disconnects when user logs out", async () => {
    ...
    patchWithCleanup(session, { user_id: null, db: "nwos" });
