...
export const presenceService = {
    start(env) {
        ...
        function onFocusChange(isFocused) {
            ...
            isNWOSFocused = isFocused;
            browser.localStorage.setItem(`${LOCAL_STORAGE_PREFIX}.focus`, isNWOSFocused);
            if (isNWOSFocused) {
                lastPresenceTime = luxon.DateTime.now().ts;
                env.bus.trigger("window_focus", isNWOSFocused);
            }
        }

        function onStorage({ key, newValue }) {
            if (key === `${LOCAL_STORAGE_PREFIX}.focus`) {
                isNWOSFocused = JSON.parse(newValue);
