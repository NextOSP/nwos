import { beforeEach, expect, test } from "@nwos/hoot";
...
test("Click on chart in dashboard mode redirect to the nwos menu", async function () {
    ...
    await click(fixture.querySelector(".o-chart-container canvas"));
    await animationFrame();
    // Clicking on a chart while on dashboard mode redirect to the nwos menu
...
test("Click on chart element in dashboard mode do not redirect twice", async function () {
    ...
    const { model } = await createModelWithDataSource({ serverData });
    const fixture = await mountSpreadsheet(model);
    const chartId = insertChartInSpreadsheet(model, "nwos_pie");
    model.dispatch("LINK_NWOS_MENU_TO_CHART", { chartId, nwosMenuId: 2 });
    await animationFrame();
    model.updateMode("dashboard");
    await animationFrame();
...
test("Clicking on a scorecard or gauge redirects to the linked menu id", async function () {
    ...
    model.dispatch("LINK_NWOS_MENU_TO_CHART", { chartId: "scorecardId", nwosMenuId: 2 });
    model.dispatch("LINK_NWOS_MENU_TO_CHART", { chartId: "gaugeId", nwosMenuId: 2 });
    await animationFrame();
...
test.tags("desktop");
test("Middle-click on chart in dashboard mode open the nwos menu in a new tab", async function () {
    const { model } = await createModelWithDataSource({ serverData });
    await mountSpreadsheet(model);
