...
from nwos import http, release, service
from nwos.tools.func import lazy_property
from nwos.tools.misc import file_path
...
class IoTRestart(Thread):
    """
    Thread to restart nwos server in IoT Box when we must return a answer before
    """
...
def check_certificate():
    ...
    if cn == 'NWOSTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_version_upgrades(local_branch, db_branch):
    ...
    :param db_branch: The git branch of the connected NWOS database (Ex: "saas-19.1" / "master" etc.)
    """
    ...
    try:
        ...
        with writable():
            ...
            subprocess.run(
                ['/home/pi/nwos/addons/hw_drivers/tools/upgrade_scripts/upgrade_trixie/upgrade_trixie.sh'], check=True,
            )
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected NWOS DB and
    ...
    try:
        ...
        if response.status == 200:
            git = ['git', '--work-tree=/home/pi/nwos/', '--git-dir=/home/pi/nwos/.git']
            ...
            _logger.info(
                "Current IoT Box local git branch: %s / Associated NWOS database's git branch: %s",
                local_branch,
                db_branch,
            )
            ...
            if db_branch != local_branch:
                try:
                    ...
                    with writable():
                        ...
                        _logger.info("Updating nwos folder to the branch %s", db_branch)
                        subprocess.run(
                            ['/home/pi/nwos/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh'], check=True
                        )
                except subprocess.CalledProcessError:
...
def check_image():
    ...
    """
    url = 'https://nightly.nwos.com/master/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def save_conf_server(url, token, db_uuid, enterprise_code, db_name=None):
    """
    Save server configurations in nwos.conf
...
@cache
def get_nwos_server_url():
    """Get the URL of the linked NWOS database.
    If the IoT Box is in access point mode, it will return ``None`` to avoid
    connecting to the server.

    :return: The URL of the linked NWOS database.
    :rtype: str or None
    """
...
def get_commit_hash():
    return subprocess.run(
        ['git', '--work-tree=/home/pi/nwos/', '--git-dir=/home/pi/nwos/.git', 'rev-parse', '--short', 'HEAD'],
...
def get_version(detailed_version=False):
    if platform.system() == 'Linux':
        image_version = read_file_first_line('/var/nwos/iotbox_version')
...
def load_certificate():
    """
    Send a request to NWOS with customer db_uuid and enterprise_code to get a true certificate
    """
    ...
    url = 'https://www.nwos.com/nwos-enterprise/iot/x509'
    data = {
        'params': {
            'db_uuid': db_uuid,
    ...
    try:
        ...
        _logger.exception("An error occurred while trying to reach nwos.com servers.")
    ...
    if server_error:
        _logger.error("A server error received from nwos.com while trying to get the certificate: %s", server_error)
        return "ERR_IOT_HTTPS_LOAD_REQUEST_NO_RESULT"

    result = response_body.get('result', {})
    certificate_error = result.get('error')
    if certificate_error:
        _logger.error("An error received from nwos.com while trying to get the certificate: %s", certificate_error)
        return "ERR_IOT_HTTPS_LOAD_REQUEST_NO_RESULT"
...
def delete_iot_handlers():
    ...
    try:
        ...
        filenames = [
            f"nwos/addons/hw_drivers/iot_handlers/{file.relative_to(iot_handlers)}"
...
def download_iot_handlers(auto=True):
    """
    Get the drivers from the configured NWOS server
    """
    server = get_nwos_server_url()
    if server:
        urllib3.disable_warnings()
        ...
        try:
            ...
            if resp.data:
                ...
                delete_iot_handlers()
                path = path_file('nwos', 'addons', 'hw_drivers', 'iot_handlers')
                with writable():
...
def compute_iot_handlers_addon_name(handler_kind, handler_file_name):
    return "nwos.addons.hw_drivers.iot_handlers.{handler_kind}.{handler_name}".\
        format(handler_kind=handler_kind, handler_name=handler_file_name.removesuffix('.py'))

def load_iot_handlers():
    """
    This method loads local files: 'nwos/addons/hw_drivers/iot_handlers/drivers' and
    'nwos/addons/hw_drivers/iot_handlers/interfaces'
...
def nwos_restart(delay=0):
    """
    Restart NWOS service
...
def path_file(*args):
    """Return the path to the file from IoT Box root or Windows NWOS
    server folder
    :return: The path to the file
    """
    platform_os = platform.system()
    if platform_os == 'Linux':
        return Path("~pi", *args).expanduser() # Path.home() returns nwos user's home instead of pi's
    elif platform_os == 'Windows':
...
def download_from_url(download_url, path_to_filename):
    ...
    (Example: 'C:\\Program Files\\NWOS\\downloaded_file.zip')
    """
    try:
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    Example: unzip_file('C:\\Program Files\\NWOS\\downloaded_file.zip', 'C:\\Program Files\\NWOS\\new_folder'))
...
def update_conf(values, section='iot.box'):
    """
    Update nwos.conf with the given key and value.
    ...
    """
    with writable():
        _logger.debug("Updating nwos.conf with values: %s", values)
        conf = get_conf()
        get_conf.cache_clear()  # Clear the cache to get the updated config

        if not conf.has_section(section):
            _logger.debug("Creating new section '%s' in nwos.conf", section)
            conf.add_section(section)

        for key, value in values.items():
            conf.set(section, key, value) if value else conf.remove_option(section, key)

        with open(path_file("nwos.conf"), "w", encoding='utf-8') as f:
            conf.write(f)
...
def get_conf(key=None, section='iot.box'):
    """
    Get the value of the given key from nwos.conf, or the full config if no key is provided.
...
def disconnect_from_server():
    ...
    get_nwos_server_url.cache_clear()
    update_conf({
        'remote_server': '',
        'token': '',
        'db_uuid': '',
...
def migrate_old_config_files_to_new_config_file():
    """Migrate old config files to the new nwos.conf"""
    if not get_conf().has_section('iot.box'):
        _logger.info('Migrating old config files to the new nwos.conf')
        iotbox_version = read_file_first_line('/var/nwos/iotbox_version')
        db_uuid = read_file_first_line('nwos-db-uuid.conf')
        enterprise_code = read_file_first_line('nwos-enterprise-code.conf')
        remote_server = read_file_first_line('nwos-remote-server.conf')
        token = read_file_first_line('token')
        subject = read_file_first_line('nwos-subject.conf')
        ...
        unlink_file('nwos-db-uuid.conf')
        unlink_file('nwos-enterprise-code.conf')
        unlink_file('nwos-remote-server.conf')
        unlink_file('token')
        unlink_file('nwos-subject.conf')
