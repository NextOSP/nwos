...
# Fix sparse-checkout
echo setup/iot_box_builder/configuration | tee -a /home/pi/nwos/.git/info/sparse-checkout
echo setup/iot_box_builder/overwrite_after_init/etc | tee -a /home/pi/nwos/.git/info/sparse-checkout

# Fix services
sed -i 's|After=.*|After=network-online.target time-sync.target cups.socket NetworkManager.service rc-local.service|g' /etc/systemd/system/nwos.service
sed -i 's|Wants=.*|Wants=network-online.target time-sync.target|g' /etc/systemd/system/nwos.service
sed -i '/Environment="LIBCAMERA_LOG_LEVELS=3"/a Environment="NWOS_PY_COLORS=True"' /etc/systemd/system/nwos.service
sed -i 's|ExecStart=.*|ExecStart=/etc/setup_ramdisks.sh|g' /etc/systemd/system/ramdisks.service
sed -i 's|ExecStart=.*|ExecStart=/etc/led_manager.sh|g' /etc/systemd/system/nwos-led-manager.service
