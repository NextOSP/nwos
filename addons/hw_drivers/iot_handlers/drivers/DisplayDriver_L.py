...
from nwos.addons.hw_drivers.driver import Driver
from nwos.addons.hw_drivers.event_manager import event_manager
...
class DisplayDriver(Driver):
    ...
    def update_url(self, url=None):
        ...
        # Kill browser instance (can't `instance.pkill()` as we can't keep the instance after NWOS service restarts)
        # We need to terminate it because NWOS will create a new instance each time it is restarted.
    ...
    def load_url(self):
        url = None
        if helpers.get_nwos_server_url():
            # disable certifiacte verification
            urllib3.disable_warnings()
            http = urllib3.PoolManager(cert_reqs='CERT_NONE')
            try:
                response = http.request('GET', "%s/iot/box/%s/display_url" % (helpers.get_nwos_server_url(), helpers.get_mac_address()))
