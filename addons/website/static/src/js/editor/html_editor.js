    ...
    isFrontendUrl(url) {
        ...
        return (
            (browser.location.hostname === parsedUrl.hostname ||
                // Also check if the nwos-hosted domain is the current domain of the url
                new RegExp(`^https?://${session.db}\\.nwos\\.com(/.*)?$`).test(parsedUrl.origin)) &&
            !parsedUrl.pathname.startsWith("/nwos") &&
