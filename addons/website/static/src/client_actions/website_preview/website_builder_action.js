...
export class WebsiteBuilderClientAction extends Component {
    ...
    /**
     * This replaces the browser url (/nwos/website...) with
    ...
    replaceBrowserUrl() {
        ...
        if (
            ...
            // trigger a traceback.
            history.replaceState(history.state, document.title, "/nwos");
            return;
        }
    ...
    reloadWebClient() {
        ...
        redirect(
            `/nwos/action-website.website_preview?website_id=${encodeURIComponent(
                websiteId
    ...
    onKeydownRefresh(ev) {
        ...
        redirect(
            `/nwos/action-website.website_preview?path=${encodeURIComponent(path)}${debugMode}`
        );
    }
...
registry
    ...
    .add("html_builder.website_builder_action", ({ host, pathname }) => {
        const backendRoutes = ["/web", "/web/session/logout", "/nwos"];
        return (
            host !== window.location.host ||
            (pathname &&
                (backendRoutes.includes(pathname) ||
                    pathname.startsWith("/@/") ||
                    pathname.startsWith("/nwos/") ||
