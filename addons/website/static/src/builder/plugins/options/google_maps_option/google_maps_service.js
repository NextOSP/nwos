...
registry.category("services").add("google_maps", {
    ...
    start(env, deps) {
        ...
        let lastKey;
        window.nwos_gmaps_api_post_load = async function nwos_gmaps_api_post_load() {
                    ...
                    gMapsAPILoading = new Promise(async (resolve) => {
                        ...
                        if (key) {
                            if (!promiseKeys[key]) {
                                ...
                                await loadJS(
                                    `https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&callback=nwos_gmaps_api_post_load&key=${encodeURIComponent(
                                        key
                                    )}`
                                );
                            }
                            ...
                            if (!editableMode && user.isAdmin) {
                                ...
                                notification.add(
                                    markup`<div>
                                        ...
                                        <a href="/nwos/action-website.action_website_configuration">${urlTitle}</a>
                                    </div>`,
