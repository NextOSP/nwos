...
patch(Colibri.prototype, {
    protectSyncAfterAsync(interaction, name, fn) {
        ...
        return (...args) => {
            ...
            wysiwyg?.nwosEditor.observerUnactive(fullName);
            const result = fn(...args);
            wysiwyg?.nwosEditor.observerActive(fullName);
            return result;
        };
    },
    addListener(target, event, fn, options) {
        ...
        const wysiwyg = window.$?.("#wrapwrap").data("wysiwyg");
        let stealthFn = fn;
        if (wysiwyg?.nwosEditor && !fn.isHandler && stealth) {
            const name = `${this.interaction.constructor.name}/${event}`;
            stealthFn = (...args) => {
                wysiwyg.nwosEditor.observerUnactive(name);
                const result = fn(...args);
                wysiwyg.nwosEditor.observerActive(name);
                return result;
            };
        }
        return super.addListener(target, event, stealthFn, options);
