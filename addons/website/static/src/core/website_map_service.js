...
export const websiteMapService = {
    ...
    start(env, deps) {
        ...
        let lastKey;
        window.nwos_gmap_api_post_load = async function nwos_gmap_api_post_load() {
                    ...
                    gmapAPILoading = (async () => {
                        ...
                        if (key) {
                            if (!promiseKeys[key]) {
                                ...
                                await loadJS(
                                    `https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&callback=nwos_gmap_api_post_load&key=${encodeURIComponent(
                                        key
                                    )}`
                                );
                            }
