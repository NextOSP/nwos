...
import { click, press, waitFor, waitForNone, queryOne } from "@nwos/hoot-dom";
...
test("LinkPopover opens in full composer", async () => {
    ...
    const node = queryOne(".nwos-editor-editable div");
...
test("link redirection should be prefixed for url of website pages only", async () => {
    ...
    setContent(el, `<p>this is a[] <a href="/nwos/project/1">link</a></p>`);
    await waitForNone(".o-we-linkpopover");
    setContent(el, `<p>this is a <a href="/nwos/project/1">li[]nk</a></p>`);
    await waitFor(".o-we-linkpopover");
    await click(".o-we-linkpopover a");
    expect.verifySteps([]);
...
test("link redirection should not be prefixed when the current page is not a website page", async () => {
    patchWithCleanup(browser, {
        ...
        location: {
            // simulating being on a non-website page (eg. backend) by using /nwos/ URL
            href: browser.location.origin + "/nwos/contactus",
