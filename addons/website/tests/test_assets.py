...
class TestWebsiteAssets(nwos.tests.HttpCase):
    ...
    def test_01_multi_domain_assets_generation(self):
        ...
        self.url_open(domain_1 + '/nwos')
        check_asset()
        self.url_open(domain_2 + '/nwos')
        check_asset()
        self.url_open(domain_1 + '/nwos')
        check_asset()
        self.url_open(domain_2 + '/nwos')
...
class TestWebAssets(nwos.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with nwos.tools.mute_logger('nwos.addons.web.controllers.binary'):
            self.assertEqual(
