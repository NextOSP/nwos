...
from nwos.tools import exception_to_unicode
...
class NWOSIMAP4_SSL(NWOSIMAP4, IMAP4_SSL):
    pass
...
class NWOSPOP3_SSL(NWOSPOP3, POP3_SSL):
    pass
...
class FetchmailServer(models.Model):
    ...
    script = fields.Char(readonly=True, default='/mail/static/scripts/nwos-mailgate.py')
...
nwos-mailgate.py --host=HOSTNAME --port=PORT -u %(uid)d -p PASSWORD -d %(dbname)s
Example configuration for the postfix mta running locally:
/etc/postfix/virtual_aliases: @youdomain nwos_mailgate@localhost
/etc/aliases:
nwos_mailgate: "|/path/to/nwos-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
        """ % conf
    ...
    def _connect__(self, allow_archived=False):  # noqa: PLW3201
        ...
        if connection_type == 'imap':
            ...
            connection = NWOSIMAP4_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else NWOSIMAP4(server, port, timeout=MAIL_TIMEOUT)
            self._imap_login__(connection)
        elif connection_type == 'pop':
            server, port, is_ssl = self.server, int(self.port), self.is_ssl
            connection = NWOSPOP3_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else NWOSPOP3(server, port, timeout=MAIL_TIMEOUT)
