...
class MailTrackingValue(models.Model):
    ...
    def _tracking_value_format_model(self, model):
        ...
        if model:
            ...
            tracked_fields = TrackedModel.fields_get(field_names, attributes={'digits', 'selection', 'string', 'type'})
            source_tracked_fields = TrackedModel.with_context(lang='en_US').fields_get(field_names, attributes={'selection'})
            model_sequence_info = dict(TrackedModel._mail_track_order_fields(tracked_fields)) if model else {}
        else:
            tracked_fields, source_tracked_fields, model_sequence_info = {}, {}, {}
        ...
        def get_field_col_info(tracking):
            if tracking.field_id.ttype != 'properties' and tracked_fields.get(tracking.field_id.name):
                col_info = dict(tracked_fields[tracking.field_id.name])
                if col_info['type'] == 'selection':
                    source_selection = source_tracked_fields.get(tracking.field_id.name, {}).get('selection')
                    if source_selection:
                        col_info['source_selection'] = source_selection
                return col_info
            return {
        ...
        fields_col_info = (get_field_col_info(tracking) for tracking in self)
        ...
        formatted = [
            {
                ...
                'newValue': tracking._format_display_value(col_info, new=True)[0],
                'oldValue': tracking._format_display_value(col_info, new=False)[0],
            }
    ...
    def _format_display_value(self, field_type, new=True):
        ...
        :param str|dict field_type: NWOS field type or field metadata;
        :param bool new: if True, display the 'new' value. Otherwise display
          the 'old' one.
        """
        col_info = field_type if isinstance(field_type, dict) else {'type': field_type}
        ...
        field_mapping = {
            ...
            'selection': ('old_value_char', 'new_value_char'),
        ...
        for record in self:
            ...
            if field_type in {'integer', 'float', 'char', 'text', 'monetary'}:
                ...
                translated_selection = dict(col_info.get('selection') or ())
                source_selection = dict(col_info.get('source_selection') or ())
                translated_by_source = {
                    source_label: translated_selection.get(key, source_label)
                    for key, source_label in source_selection.items()
                }
                translated_by_source.update({
                    key: translated_label
                    for key, translated_label in translated_selection.items()
                })
                result.append(translated_by_source.get(value, value))
