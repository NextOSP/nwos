...
from nwos import _, api, fields, models, Command, tools
...
from nwos.tools.mail import is_html_empty, email_normalize, email_split_and_format
from nwos.tools.misc import clean_context
...
class MailComposeMessage(models.TransientModel):
    ...
    auto_delete = fields.Boolean(
        ...
        help='This option permanently removes any track of email after it\'s been sent, including from the Technical menu in the Settings, in order to preserve storage space of your NWOS database.')
    ...
    def _ai_get_email_settings(self):
    ...
    def _ai_prepare_record_context(self):
        self.ensure_one()
        if not self.model or self.model not in self.env:
            raise UserError(_('AI email drafting needs a related document.'))
        res_ids = self._evaluate_res_ids()
        if len(res_ids) != 1:
            raise UserError(_('AI email drafting is available for one document at a time.'))

        record = self.env[self.model].browse(res_ids[0]).exists()
        if not record:
            raise UserError(_('The related document no longer exists.'))
        record.check_access('read')

        lines = [
            f"Document model: {record._description or record._name}",
            f"Document: {record.display_name}",
        ]
        for field_name in (
            'name', 'state', 'partner_id', 'company_id', 'user_id', 'currency_id',
            'amount_total', 'amount_untaxed', 'amount_tax',
            'date_order', 'date_approve', 'effective_date', 'validity_date',
            'commitment_date', 'invoice_date', 'invoice_date_due',
            'payment_state', 'origin', 'client_order_ref', 'ref',
        ):
            ...
            elif field.type in ('one2many', 'many2many', 'binary', 'html'):
                continue
            lines.append(f"{field.string or field_name}: {value}")

        line_field_name = next((
            candidate for candidate in ('order_line', 'invoice_line_ids', 'line_ids')
            if candidate in record._fields
        ), None)
        if line_field_name:
            detail_lines = []
            for index, line in enumerate(record[line_field_name][:20], start=1):
                if 'display_type' in line._fields and line.display_type:
                    continue
                product = line.product_id.display_name if 'product_id' in line._fields and line.product_id else ''
                description = tools.html2plaintext(line.name or '').strip() if 'name' in line._fields else ''
                quantity = ''
                for quantity_field in ('product_qty', 'product_uom_qty', 'quantity'):
                    if quantity_field in line._fields and line[quantity_field]:
                        quantity = line[quantity_field]
                        break
                uom = ''
                for uom_field in ('product_uom', 'product_uom_id'):
                    if uom_field in line._fields and line[uom_field]:
                        uom = line[uom_field].display_name
                        break
                price_unit = line.price_unit if 'price_unit' in line._fields else ''
                subtotal = line.price_subtotal if 'price_subtotal' in line._fields else ''
                detail_lines.append(
                    f"{index}. {product or description or line.display_name}; "
    ...
    def _ai_prepare_email_messages(self):
        self.ensure_one()
        recipient_names = ', '.join(self.partner_ids.mapped('display_name')) or _('No recipients selected')
        attachment_names = ', '.join(self.attachment_ids.mapped('name')) or _('No attachments')
        current_body_html = str(self.body or '').strip()
        current_body_text = tools.html2plaintext(current_body_html).strip()
        context_lines = [
            f"UI/user language: {self.lang or self.env.context.get('lang') or self.env.user.lang or 'en_US'}",
            f"Sender: {self.email_from or self.env.user.email_formatted or self.env.user.display_name}",
            f"Recipients: {recipient_names}",
            f"Attachments: {attachment_names}",
            f"Current draft subject: {self.subject or '[empty]'}",
            f"Current draft body text: {current_body_text or '[empty]'}",
            f"Current draft body HTML: {current_body_html or '[empty]'}",
        ]
        context_lines.append('Related document:')
        context_lines.extend(self._ai_prepare_record_context())
        return [
            {
                'role': 'system',
                'content': (
                    "You write customer- and vendor-facing ERP emails inside NWOS/Flectra. "
                    "Use only the facts in the provided ERP context. Do not invent prices, dates, "
                    "payment terms, delivery details, bank details, contacts, or attachments. "
                    "Keep existing action links, buttons, and URLs from the current draft exactly "
                    "when they are present. If the UI/user language starts with vi, write natural Vietnamese. "
                    "Otherwise use the same language as the current draft where possible. "
                    "Return only a JSON object with keys subject and body_html. "
                    "body_html must use simple HTML tags only: p, br, ul, li, strong, a."
                ),
            },
            {
                'role': 'user',
    ...
    def _ai_parse_email_response(self, content):
        ...
        subject = str(payload.get('subject') or '').strip() or self.subject or ''
        ...
        if '<' not in body_html or '>' not in body_html:
            body_html = str(tools.plaintext2html(body_html))
    ...
    def action_ai_write_email(self):
        self.ensure_one()
        if not self.can_edit_body:
            raise UserError(_('This email body cannot be edited.'))
        if self.composition_mode != 'comment' or self.composition_batch:
            raise UserError(_('AI email drafting is available for one document email at a time.'))

        settings = self._ai_get_email_settings()
    ...
    def _evaluate_res_domain(self):
        ...
        :return: an NWOS domain (list of leaves) """
        self.ensure_one()
