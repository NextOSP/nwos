...
import { describe, test } from "@nwos/hoot";
import {
    asyncStep,
    Command,
    onRpc,
...
test("Auto-open NWOSBot chat when opening discuss for the first time", async () => {
    // NWOSbot chat has onboarding for using Discuss app.
    // We assume pinned nwosbot chat without any message seen means user just started using Discuss app.
    const pyEnv = await startServer();
    ...
    await start();
    await openDiscuss();
    await contains(".o-mail-DiscussContent-threadName", { value: "NWOSBot" });
});
