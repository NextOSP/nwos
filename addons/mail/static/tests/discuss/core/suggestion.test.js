...
import { beforeEach, describe, test } from "@nwos/hoot";
...
test("use a command for a specific channel type", async () => {
    ...
    await htmlInsertText(editor, "/");
    await click(".o-mail-Composer-suggestion strong", { text: "who" });
    await contains(".o-mail-Composer-html.nwos-editor-editable", { text: "/who" });
});
...
test("mention suggestion displays NWOSBot before archived partners", async () => {
    const pyEnv = await startServer();
