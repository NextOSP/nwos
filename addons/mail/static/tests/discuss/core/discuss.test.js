...
import { tick } from "@nwos/hoot-dom";
...
test("subscribe to presence channels according to store data", async () => {
    ...
    await expect.waitForSteps([
        "subscribe - [nwos-presence-res.partner_1,nwos-presence-res.partner_2]",
    ]);
    // Discovering new presence channels should refresh the subscription.
    store["mail.guest"].insert({ id: 1 });
    await expect.waitForSteps([
        "subscribe - [nwos-presence-mail.guest_1,nwos-presence-res.partner_1,nwos-presence-res.partner_2]",
    ]);
    // Updating "im_status_access_token" should refresh the subscription.
    store["mail.guest"].insert({ id: 1, im_status_access_token: "token" });
    await expect.waitForSteps([
        "subscribe - [nwos-presence-mail.guest_1-token,nwos-presence-res.partner_1,nwos-presence-res.partner_2]",
    ]);
});
