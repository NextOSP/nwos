...
import { animationFrame, leave, pointerDown, press, queryFirst, waitFor } from "@nwos/hoot-dom";
import { advanceTime, mockDate, mockTouch, mockUserAgent, tick } from "@nwos/hoot-mock";
import {
    asyncStep,
...
test("edit a message with styling keeps the styling", async () => {
    ...
    await click(".o-mail-Message [title='Edit']");
    await contains(".o-mail-Composer-html.nwos-editor-editable strong:contains('Hello world')");
});
...
test("edit a message styling", async () => {
    ...
    await click(".o-mail-Message [title='Edit']");
    await focus(".o-mail-Message .o-mail-Composer-html.nwos-editor-editable");
    const editor = {
        document,
        editable: document.querySelector(
            ".o-mail-Message .o-mail-Composer-html.nwos-editor-editable"
        ),
    };
...
test("Update the link previews when a message is edited", async () => {
    ...
    await openDiscuss(channelId);
    await click(".o-mail-Message [title='Edit']");
    await insertText(".o-mail-Message .o-mail-Composer-input", "http://nwos.com", {
        replace: true,
    });
    await click(".o-mail-Message button", { text: "save" });
    await contains(".o-mail-Message-body", { text: "http://nwos.com" });
...
test("should delete link preview along with message", async () => {
    ...
    const linkPreviewId = pyEnv["mail.link.preview"].create({
        ...
        og_type: "article",
        source_url: "https://www.nwos.com",
    });
    const channelId = pyEnv["discuss.channel"].create({ name: "PreviewTest" });
    pyEnv["mail.message"].create({
        body: "<a href='https://www.nwos.com'>https://www.nwos.com</a>",
