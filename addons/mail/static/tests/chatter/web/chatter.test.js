...
import { Deferred, advanceTime } from "@nwos/hoot-mock";
import {
    asyncStep,
    defineActions,
    getService,
...
test("Mentions in composer should still work when using pager", async () => {
    ...
    // all records in DB: Mitchell Admin | Hermit | Public user except NWOSBot
...
test("Update message recipients without saving", async () => {
    const pyEnv = await startServer();
    pyEnv["res.partner"].write([serverState.partnerId], { email: "mitchell@nwos.com" });
    const partnerId = pyEnv["res.partner"].create({
        name: "John Doe",
        email: "john@doe.be",
    });
