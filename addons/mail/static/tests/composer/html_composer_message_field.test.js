...
test("media dialog: upload", async function () {
    ...
    const anchorNode = queryOne(".nwos-editor-editable div.o-paragraph");
...
test("mention a partner", async () => {
    ...
    const editable = queryOne(`.nwos-editor-editable`);
    ...
    expect(getContent(editable)).toBe(
        `<p>\uFEFF<a href="/nwos/res.partner/${serverState.partnerId}" class="o_mail_redirect" data-oe-id="${serverState.partnerId}" data-oe-model="res.partner" target="_blank" contenteditable="false">@Mitchell Admin</a>\uFEFF[]</p>`
    );
});
...
test("mention a channel", async () => {
    ...
    const anchorNode = queryOne(`[name='body'] .nwos-editor-editable div.o-paragraph`);
    setSelection({ anchorNode, anchorOffset: 0 });
    await insertText(htmlEditor, "#");
    await animationFrame();
...
describe("Remove attachments", () => {
    ...
    test("should remove file from html editor if removed from attachment list", async () => {
        ...
        await waitFor(".nwos-editor-editable");
        const anchorNode = queryOne(".nwos-editor-editable div.o-paragraph");
        setSelection({ anchorNode, anchorOffset: 0 });
        ...
        await waitFor(".nwos-editor-editable .o_file_box:has(a:contains('file.txt'))");
        await click("[name='attachment_ids'] button:has(i.fa-times)");
        await waitForNone("[name='attachment_ids'] a:contains('file.txt')");
        await waitForNone(".nwos-editor-editable .o_file_box:has(a:contains('file.txt'))");
    });
    ...
    test("should remove image from html editor if removed from attachment list", async () => {
        ...
        await waitFor(".nwos-editor-editable img[data-attachment-id='1']");
        await click("[name='attachment_ids'] button:has(i.fa-times)");
        await waitForNone("[name='attachment_ids'] a:contains('test.jpg')");
        await waitForNone(".nwos-editor-editable img[data-attachment-id='1']");
    });
});
