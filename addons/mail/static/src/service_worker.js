...
let db;
const unread_store = new Store("nwos-mail-unread-db", "nwos-mail-unread-store");
 ...
 * @param {Object} param1
 * @param {string} [param1.action] nwos client action
...
async function openDiscussChannel(channelId, { action, joinCall = false, source } = {}) {
    const discussURLRegexes = [new RegExp("/(?:nwos|nwos)/discuss")];
    if (action) {
        discussURLRegexes.push(
            new RegExp(`/(?:nwos|nwos)/\\d+/action-${action}`),
            new RegExp(`/(?:nwos|nwos)/action-${action}`)
        );
    }
    let targetClient;
    ...
    if (action) {
        const url = new URL(`/nwos/action-${action}`, location.origin);
...
self.addEventListener("notificationclick", (event) => {
    ...
    if (event.notification.data) {
        ...
        if (model === "discuss.channel") {
            ...
            event.waitUntil(clients.openWindow(`/nwos/${modelPath}/${res_id}`));
        }
    }
});
