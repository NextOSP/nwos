...
import { Component, onMounted, onWillStart, onWillUnmount, onWillUpdateProps } from "@nwos/owl";
...
export class DiscussClientAction extends Component {
    ...
    async restoreDiscussThread(props) {
        ...
        if (!parsedActiveId) {
            ...
            const nwosbotChat = this.store.nwosbot?.searchChat();
            const selfMember = nwosbotChat?.self_member_id;
            if (nwosbotChat && selfMember?.is_pinned && !selfMember.seen_message_id) {
                nwosbotChat.setAsDiscussThread(false);
            }
            return;
        }
