...
# nwos-mailgate
#
# This program will read an email from stdin and forward it to nwos. Configure
# a pipe alias in your mail server to use it, postfix uses a syntax that looks
# like:
#
# email@address: "|/home/nwos/src/nwos-mail.py"
#
# while exim uses a syntax that looks like:
#
# *: |/home/nwos/src/nwos-mail.py
...
def main():
    ...
    op.add_option("-d", "--database", dest="database", help="NWOS database name (default: %default)", default='nwos')
    op.add_option("-u", "--userid", dest="userid", help="NWOS user id to connect with (default: %default)", default=1, type=int)
    op.add_option("-p", "--password", dest="password", help="NWOS user password (default: %default)", default='admin')
    op.add_option("--host", dest="host", help="NWOS host (default: %default)", default='localhost')
    op.add_option("--port", dest="port", help="NWOS port (default: %default)", default=8069, type=int)
    ...
    try:
        ...
        if e.faultString == 'Access denied' and e.faultCode == 3:
            ...
            postfix_exit(EX_NOUSER, "alias does not exist in nwos\n", o.debug)
        else:
...
try:
    ...
    # may include the password for the nwos user.
