...
from nwos.tools import groupby
from nwos.addons.bus.websocket import wsrequest
...
def get_sfu_url(env) -> str | None:
    ...
    if not sfu_url:
        sfu_url = os.getenv("NWOS_SFU_URL")
    if sfu_url:
    ...
    def _add_values(self, values, model_name, index=None):
        ...
        for key, val in values.items():
            ...
            if isinstance(val, Store.Relation):
                ...
                target[key] = nwos.fields.Datetime.to_string(val)
            elif isinstance(val, date):
                target[key] = nwos.fields.Date.to_string(val)
