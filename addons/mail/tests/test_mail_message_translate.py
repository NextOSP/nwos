...
class TestTranslationController(HttpCaseWithUserDemo):
    ...
    def test_access_right(self):
        with self.assertRaises(JsonRpcException, msg="nwos.http.SessionExpiredException"):
        ...
        with self.assertRaises(JsonRpcException, msg="nwos.exceptions.AccessError"), mute_logger("nwos.http"):
            self._mock_translation_request({"message_id": self.message.id})
