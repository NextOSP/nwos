...
from nwos.addons.mail.tests import common
from nwos.tests import new_test_user, tagged, users
...
class TestMailMessage(common.MailCommon):
    ...
    def test_tracking_value_format_translates_selection_labels(self):
        message = self.env['mail.message'].sudo().create({
            'model': 'res.lang',
            'res_id': self.env['res.lang']._get_data(code='vi_VN').id,
            'body': 'Direction changed',
        })
        tracking = self.env['mail.tracking.value'].sudo().create({
            'mail_message_id': message.id,
            'field_id': self.env['ir.model.fields']._get('res.lang', 'direction').id,
            'old_value_char': 'Left-to-Right',
            'new_value_char': 'Right-to-Left',
        })

        formatted = tracking.with_context(lang='vi_VN')._tracking_value_format()

        self.assertEqual(formatted[0]['fieldInfo']['changedField'], 'Hướng')
        self.assertEqual(formatted[0]['oldValue'], 'Trái sang Phải')
        self.assertEqual(formatted[0]['newValue'], 'Phải sang Trái')
