...
from nwos.tests import tagged, new_test_user
from nwos.addons.bus.tests.common import WebsocketCase
from nwos.addons.mail.tests.common import MailCommon, freeze_all_time
...
class TestMailPresence(WebsocketCase, MailCommon):
    def _receive_presence(self, requested_by, target, has_token=False):
        ...
        channel_parts = ["nwos-presence", f"{target_channel._name}_{target_channel.id}"]
        if has_token:
