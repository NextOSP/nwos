...
from nwos.addons.payment_toss_payments import const
from nwos.addons.payment_toss_payments.controllers.main import TossPaymentsController
from nwos.addons.payment_toss_payments.tests.common import TossPaymentsCommon


@tagged('post_install', '-at_install')
class TestProcessingFlows(TossPaymentsCommon, PaymentHttpCommon):
    @mute_logger('nwos.addons.payment_toss_payments.controllers.main')
    ...
    def test_failing_to_confirm_payment_sets_the_transaction_in_error(self):
        ...
        url = self._build_url(const.PAYMENT_SUCCESS_RETURN_ROUTE)
        with patch(
            'nwos.addons.payment.models.payment_transaction.PaymentTransaction._send_api_request',
    ...
    def test_webhook_notification_triggers_processing(self):
        ...
        with (
            patch(
                'nwos.addons.payment_toss_payments.controllers.main.TossPaymentsController'
                '._verify_signature'
            ),
            patch(
                'nwos.addons.payment.models.payment_transaction.PaymentTransaction._process'
