...
import { queryAll, queryFirst, queryOne } from "@nwos/hoot-dom";
import { Component, useState, useExternalListener } from "@nwos/owl";
...
const getShortestSelector = (paths) => {
    paths.reverse();
    let filteredPath = [];
    let hasNWOSClass = false;
    for (
        let currentElem = paths.pop();
        (currentElem && queryAll(filteredPath.join(" > ")).length !== 1) || !hasNWOSClass;
        ...
        const nwosClass = [...currentElem.classList].find((c) => c.match(NWOS_CLASS_REGEX));
        if (nwosClass) {
            currentPredicate = `.${nwosClass}`;
            hasNWOSClass = true;
        }
