import { Component, markup, whenReady, validate } from "@nwos/owl";
    ...
    start: async (env, { orm, effect, overlay }) => {
        ...
        async function resumeTour() {
            ...
            if (tourConfig.mode === "auto") {
                if (!nwos.loader.modules.get("@web_tour/js/tour_automatic/tour_automatic")) {
                    await loadBundle("web_tour.automatic", { css: false });
                }
                const { TourAutomatic } = nwos.loader.modules.get(
                    "@web_tour/js/tour_automatic/tour_automatic"
                );
                new TourAutomatic(tour).start();
            } else {
                await loadBundle("web_tour.interactive");
                const { TourPointer } = nwos.loader.modules.get(
                ...
                const { TourInteractive } = nwos.loader.modules.get(
        ...
        async function tourRecorder() {
            ...
            const { TourRecorder } = nwos.loader.modules.get(
        ...
        if (!window.frameElement) {
            ...
            } else if (toursEnabled && session.current_tour) {
        ...
        nwos.startTour = startTour;
        nwos.isTourReady = (tourName) => getTourFromRegistry(tourName).wait_for.then(() => true);
