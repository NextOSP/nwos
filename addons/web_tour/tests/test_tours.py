...
from nwos.addons.base.tests.common import BaseCommon, HttpCase
...
class WebTourHttp(HttpCase):
    ...
    def test_sanity_automatic(self):
        ...
        code = """
        nwos.define("@web_tour/../tests/sanity_test", [], () => {
            const errors = [];
            for (const module of ["@nwos/hoot-dom", "@web_tour/js/tour_step"]) {
                if (nwos.loader.modules.get(module)) {
                    errors.push(module)
                }
            }
            if (!errors.length) {
        ...
        """
        self.browser_js("/nwos?debug=tests", code, ready=ready, login="admin")
    ...
    def test_sanity_onboarding(self):
        ...
        # We want to boot NWOS as in real life (not loading assets for tests)
        ...
        """
        self.browser_js("/nwos?debug=0", code, ready=ready, login="admin")
        if "website" in IrAsset._get_installed_addons_list():
