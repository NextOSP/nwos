...
full payment experience (method selection, data entry) and notifies NWOS of the outcome via two
independent channels:

- **Return URL** (`/payment/ecpay/return`): ECPay redirects the customer back to NWOS after the
...
that don't map to the selected NWOS payment method are excluded, nudging the customer toward the
...
- `19.0`
  - The first version of the module is merged. nwos/nwos#235069
...
createdb nwos
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -i payment_ecpay
```
...
```bash
./nwos-bin server --addons-path=addons,nwos/addons -d nwos -u payment_ecpay
```
