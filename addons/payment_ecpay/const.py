...
# Mapping of NWOS payment method codes to the values returned by ECPay notifications.
# This map intentionally contains only methods where the returned value lets us pick a brand PM
# in NWOS (e.g., convenience stores and specific wallets). Other methods are still supported via
# PAYMENT_METHODS_MAPPING, but their response values are either generic (e.g., Credit_CreditCard)
# or not mapped to brand PMs in NWOS (e.g., ATM/WebATM bank variants).
