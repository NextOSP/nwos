...
const assertNoRPC = {
    ...
    run: async function () {
        await retryUntil(
            (result) => result?.error?.data?.name === "nwos.addons.auth_timeout.models.ir_http.CheckIdentityException",
            ...
            run() {
                ...
                rpc._rpc = function (...args) {
                    return oldRpc(...args).catch((err) => {
                        if (err.data?.name === "nwos.addons.auth_timeout.models.ir_http.CheckIdentityException") {
                            return new Promise(() => {});
                        } else {
                            throw err;
                        }
