...
from nwos.tests import HttpCase, new_test_user, tagged, TransactionCase
from nwos.tests.common import HOST

from nwos.addons.auth_totp.models.totp import TOTP
from nwos.addons.auth_totp.controllers.home import TRUSTED_DEVICE_COOKIE
...
class TestAuthTimeoutHttp(HttpCase):
    ...
    def set_session_create_time(self, session_id, timestamp):
        session = nwos.http.root.session_store.get(session_id)
    ...
    def assertMustCheckIdentity(self):
        ...
        self.assertEqual(
            ...
            "nwos.addons.auth_timeout.models.ir_http.CheckIdentityException",
        )

    def assertSessionExpired(self):
        result = self.rpc("res.users", "read", [self.user.id], ["login"])
        self.assertEqual(result.get("error", {}).get("data", {}).get("name"), "nwos.http.SessionExpiredException")
    ...
    def test_auth_timeout_tour(self):
        ...
        with patch.object(
            ...
            self.start_tour("/nwos", "auth_timeout_tour_lock_timeout_inactivity", login=self.user.login)
        ...
        with patch.object(
            ...
            self.start_tour("/nwos", "auth_timeout_tour_lock_timeout_inactivity_2fa", login=self.user.login)
