...
from nwos.tests.common import HttpCase, tagged, freeze_time
from nwos.tools import mute_logger

from nwos.addons.account_peppol.tests.common import PeppolConnectorCommon
from nwos.addons.account_peppol.tools.peppol_iap_connector import PeppolIAPConnector
...
class TestPeppolKYC(PeppolConnectorCommon, HttpCase):
    ...
    def _mock_can_connect_method(self, with_auth=False):
        ...
        return patch('nwos.addons.account_peppol.tools.peppol_iap_connector.PeppolIAPConnector.can_connect', return_value={
            'auth_required': with_auth,
            **auth_vals,
        })

    def _mock_create_connection_method(self, id_client='test_id_client', peppol_state='smp_registration'):
        return patch('nwos.addons.account_peppol.tools.peppol_iap_connector.PeppolIAPConnector.create_connection', return_value={
    ...
    def test_connect_with_itsme_successful_auth(self):
        ...
        self.assertIn('/nwos/peppol-auth-callback-action?success=True', response.url)
    ...
    @mute_logger('nwos.addons.account_peppol.controllers.authentication')
    def test_connect_with_itsme_failed_auth_case_2(self):
        ...
        self.assertTrue(response.history[0].is_redirect)
        self.assertEqual(response.status_code, 200)
        self.assertIn('/nwos/peppol-auth-callback-action?success=False', response.url)
