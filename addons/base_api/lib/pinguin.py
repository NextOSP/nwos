...
def transform_dictfields_to_list_of_tuples(record, dct, ENV=False):
    ...
    :param nwos.models.Model record: The model object.
...
def get_model_for_read(model, ENV=False):
    ...
    if not test_mode:
        ...
        # Contrary to ISOLATION_LEVEL_SERIALIZABLE as per nwos Standard
...
def get_dict_from_record(
    ...
    :param nwos.models.Model record: The singleton record to load.
    ...
    for field in _spec:
        ...
        # Normal field, or unspecified relational
            ...
            if not hasattr(record, field):
                raise nwos.exceptions.ValidationError(
                    nwos._('The model "%s" has no such field: "%s".')
                    % (record._name, field)
                )
