...
from nwos import _, fields, models, Command
from nwos.exceptions import UserError
from nwos.tools import formatLang, frozendict, html2plaintext, html_escape, pdf, str2bool, unique
from nwos.addons.account_edi_ubl_cii.models.account_edi_common import (
    EAS_MAPPING,
    FloatFmt,
    GST_COUNTRY_CODES,
    UOM_TO_UNECE_CODE,
...
class AccountEdiUBL(models.AbstractModel):
    ...
    def _import_ubl_invoice_update_move_type(self, collected_values):
        ...
        prefix = 'out' if nwos_document_type == 'sale' else 'in'
    ...
    def _import_ubl_invoice_add_ref(self, collected_values):
        ...
        if ref := tree.findtext('./{*}ID'):
            if nwos_document_type == 'sale' and collected_values['invoice'].quick_edit_mode:
    ...
    def _import_ubl_invoice_add_tax_total_values(self, collected_values):
        ...
        for subtotal_elem in tree.findall('./{*}TaxTotal/{*}TaxSubtotal'):
            ...
            tax_values = taxes_values.setdefault(tax_key, {
                'amount_type': 'percent',
                'type_tax_use': nwos_document_type,
                'amount': percentage,
    ...
    def _import_attachments(self, invoice, tree):
        ...
        try:
            invoices_by_nwos_xmlid = 'account_edi_ubl_cii.action_report_account_invoices_generated_by_nwos'
            if not self.env.ref(invoices_by_nwos_xmlid, raise_if_not_found=False):
                _logger.warning("Missing template while generating substitute PDF attachment for invoice %s", invoice.id)
                return additional_docs
            report_xmlid = invoices_by_nwos_xmlid
            ...
            if not self.env.ref('account_edi_ubl_cii.layout_invoices_generated_by_nwos', raise_if_not_found=False):
                # TODO: Remove in master
                # add a watermark to the generated pdf
                with io.BytesIO(pdf_raw) as pdf_stream:
                    new_pdf_stream = pdf.add_banner(pdf_stream, _('Generated by NWOS'), logo=False)
                    pdf_raw = new_pdf_stream.getvalue()
                    new_pdf_stream.close()

            invoice_name = invoice.display_name.replace(_('Draft'), '')
            pdf_filename = _('%(invoice_name)s - Generated by NWOS', invoice_name=invoice_name)
    ...
    def _ubl_import_invoice(self, invoice, file_data, new=False):
        ...
        collected_values = {
            'invoice': invoice,
            'company': company,
            'nwos_document_type': 'sale' if invoice.journal_id.type == 'sale' else 'purchase',
            'tree': tree,
            'file_data': file_data,
            'logs': [],
            'to_write': {},
