...
from nwos import http, tools
from nwos.addons.hw_drivers.tools import helpers
from nwos.addons.hw_drivers.main import iot_devices
from nwos.addons.web.controllers.home import Home
from nwos.addons.hw_drivers.connection_manager import connection_manager
from nwos.tools.misc import file_path
from nwos.addons.hw_drivers.server_logger import (
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('nwos.addons.hw_posbox_homepage', "views")
...
class IotBoxOwlHomePage(Home):
    ...
    # ---------------------------------------------------------- #
    @http.route('/hw_posbox_homepage/restart_nwos_service', auth='none', type='http', cors='*')
    ...
    def get_iot_logs(self):
        with open("/var/log/nwos/nwos-server.log", encoding="utf-8") as file:
            return json.dumps({
                'status': 'success',
                'logs': file.read(),
            })
    ...
    def get_homepage_data(self):
        ...
        return json.dumps({
            ...
            'server_status': helpers.get_nwos_server_url() or 'Not Configured',
    ...
    def update_wifi(self, essid, password, persistent=False):
        ...
        res_payload = {
            ...
            'server': {
                ...
                'message': 'Redirect to NWOS Server' if server else 'Redirect to IoT Box'
            }
        }
    ...
    def connect_to_nwos_server(self, token=False, iotname=False):
        if token:
            try:
    ...
    def update_log_level(self, name, value):
        ...
        if name == 'log-to-server':
            need_config_save |= check_and_update_nwos_config_log_to_server_option(
                value
            )
        ...
        if name == 'root':
            ...
            need_config_save |= self._update_logger_level(
                'nwos', value, AVAILABLE_LOG_LEVELS)
            need_config_save |= self._update_logger_level(
    ...
    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        ...
        LOG_HANDLERS = tools.config[NWOS_TOOL_CONFIG_HANDLER_NAME]
        ...
        tools.config[NWOS_TOOL_CONFIG_HANDLER_NAME] = log_handlers_without_logger
        new_level_upper_case = new_level.upper()
        if not IS_NEW_LEVEL_PARENT:
            new_entry = [LOGGER_PREFIX + new_level_upper_case]
            tools.config[NWOS_TOOL_CONFIG_HANDLER_NAME] += new_entry
