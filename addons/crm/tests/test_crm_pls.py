...
from nwos import exceptions, tools
from nwos.addons.crm.tests.common import TestCrmCommon
from nwos.addons.mail.tests.common import mail_new_test_user
from nwos.fields import Date
from nwos.tests import Form, tagged, users
...
class CrmPlsCommon(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        ICP.set_param('crm.ai_lead_scoring.enabled', False)
        ICP.search([('key', 'in', [
            'base.ai.enabled',
            'base.ai.provider',
            'base.ai.endpoint',
...
class TestCrmPls(CrmPlsCommon):
    ...
    def test_pls_tooltip_ai_provider(self):
        ...
        ICP.set_param('crm.ai_lead_scoring.model', 'test/model')
        ICP.set_param('crm.ai_lead_scoring.api_key', 'test-key')

        lead = self.env['crm.lead'].create({
            'name': 'AI Provider Lead',
            'type': 'opportunity',
            'email_from': 'lead@example.com',
            'phone': '+12025550142',
            'team_id': self.pls_team.id,
        })
        lead.message_post(
            body='Customer says the price is too expensive.',
            subtype_xmlid='mail.mt_comment',
        )
        lead.message_post(
            body='Internal note should stay out.',
            subtype_xmlid='mail.mt_note',
        )
        response_content = {
            'probability': 67.5,
            'positives': ['Clear revenue potential', {'reason': 'Assigned sales team'}],
            'negatives': ['No expected closing date'],
        }

        with patch('nwos.addons.crm.models.crm_lead.requests.post') as mock_post:
            ...
            tooltip_data = lead.prepare_pls_tooltip_data()

        self.assertEqual('OpenRouter', tooltip_data['ai_provider_name'])
        self.assertEqual(tools.float_compare(tooltip_data['probability'], 67.5, 2), 0)
        self.assertEqual(['ai_reason', 'ai_reason'], [entry['field'] for entry in tooltip_data['top_3_data']])
        self.assertEqual(['Clear revenue potential', 'Assigned sales team'], [entry['value'] for entry in tooltip_data['top_3_data']])
        self.assertEqual(['No expected closing date'], [entry['value'] for entry in tooltip_data['low_3_data']])

        request_url = mock_post.call_args.args[0]
        request_kwargs = mock_post.call_args.kwargs
        self.assertEqual('https://openrouter.ai/api/v1/chat/completions', request_url)
        self.assertEqual('Bearer test-key', request_kwargs['headers']['Authorization'])
        self.assertEqual('NWOS CRM', request_kwargs['headers']['X-OpenRouter-Title'])
        self.assertEqual('test/model', request_kwargs['json']['model'])
        lead_payload = request_kwargs['json']['messages'][1]['content']
        lead_payload_data = json.loads(lead_payload)
        self.assertIn('"has_email": true', lead_payload)
        self.assertIn('"has_phone": true', lead_payload)
        self.assertNotIn('lead@example.com', lead_payload)
        self.assertNotIn('+12025550142', lead_payload)
        self.assertEqual(
            ['Customer says the price is too expensive.'],
            [comment['body'] for comment in lead_payload_data['recent_chatter_comments']],
        )
        self.assertNotIn('Internal note should stay out', lead_payload)

    def test_pls_tooltip_global_ai_provider(self):
        ICP = self.env['ir.config_parameter'].sudo()
        ICP.set_param('crm.ai_lead_scoring.enabled', True)
        ICP.set_param('base.ai.enabled', True)
        ICP.set_param('base.ai.provider', 'openai')
        ICP.set_param('base.ai.endpoint', 'https://api.openai.com/v1')
        ICP.set_param('base.ai.model', 'gpt-4o-mini')
        ICP.set_param('base.ai.api_key', 'global-key')

        lead = self.env['crm.lead'].create({
            'name': 'Global AI Provider Lead',
            'type': 'opportunity',
            'team_id': self.pls_team.id,
        })
        ...
        self.assertEqual('OpenAI', tooltip_data['ai_provider_name'])
        self.assertEqual('https://api.openai.com/v1/chat/completions', mock_post.call_args.args[0])
        self.assertEqual('Bearer global-key', mock_post.call_args.kwargs['headers']['Authorization'])
        self.assertEqual('gpt-4o-mini', mock_post.call_args.kwargs['json']['model'])
...
class TestLeadLost(TestCrmCommon):
    ...
    @users('user_sales_salesman')
    @mute_logger('nwos.addons.base.models')
