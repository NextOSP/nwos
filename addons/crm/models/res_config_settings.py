...
from nwos import api, exceptions, fields, models, _
from nwos.tools.ai import AI_PROVIDER_DEFAULTS, AI_PROVIDER_SELECTION
from nwos.tools import format_list
...
class ResConfigSettings(models.TransientModel):
    ...
    crm_ai_lead_scoring_enabled = fields.Boolean(
        string='AI Probability Insights',
        config_parameter='crm.ai_lead_scoring.enabled')
    crm_ai_provider = fields.Selection(
        AI_PROVIDER_SELECTION,
        string='AI Provider',
        default='openrouter',
        config_parameter='crm.ai_lead_scoring.provider')
    crm_ai_endpoint = fields.Char(
        string='Base URL',
        default=AI_PROVIDER_DEFAULTS['openrouter']['endpoint'],
        config_parameter='crm.ai_lead_scoring.endpoint')
    crm_ai_model = fields.Char(
        string='Model',
        default=AI_PROVIDER_DEFAULTS['openrouter']['model'],
        config_parameter='crm.ai_lead_scoring.model')
    crm_ai_api_key = fields.Char(
        string='API Key',
        config_parameter='crm.ai_lead_scoring.api_key')
    ...
    @api.onchange('crm_ai_provider')
    def _onchange_crm_ai_provider(self):
        for setting in self:
            defaults = AI_PROVIDER_DEFAULTS.get(setting.crm_ai_provider)
            if not defaults:
                continue
            if setting.crm_ai_provider != 'custom':
                setting.crm_ai_endpoint = defaults['endpoint']
                setting.crm_ai_model = defaults['model']
