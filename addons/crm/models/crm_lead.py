...
from nwos import api, fields, models, modules, tools
from nwos.addons.iap.tools import iap_tools
from nwos.addons.mail.tools import mail_validation
from nwos.addons.phone_validation.tools import phone_validation
from nwos.exceptions import UserError, AccessError, ValidationError
from nwos.fields import Domain
from nwos.tools.translate import _
from nwos.tools.ai import get_ai_settings
from nwos.tools import date_utils, email_normalize_all, is_html_empty, groupby, parse_contact_from_email, SQL, str2bool
...
CRM_AI_LEAD_SCORING_CHATTER_LIMIT = 5
CRM_AI_LEAD_SCORING_CHATTER_BODY_LIMIT = 500
class CrmLead(models.Model):
    ...
    # -----------------------
    @api.model
    def _crm_ai_get_lead_scoring_config(self):
        ICP = self.env['ir.config_parameter'].sudo()
        if not str2bool(ICP.get_param('crm.ai_lead_scoring.enabled', default=False), default=False):
            return {}

        global_ai_enabled = ICP.get_param('base.ai.enabled', default=None)
        use_global_ai = global_ai_enabled is not None
        if use_global_ai and not str2bool(global_ai_enabled, default=False):
            return {}

        settings = get_ai_settings(
            self.env,
            profile='small' if use_global_ai else 'default',
            legacy_prefix=False if use_global_ai else 'crm.ai_lead_scoring',
        )
        if not settings['enabled'] or not settings['endpoint'] or not settings['model'] or not settings['api_key']:
            return {}
    ...
    @api.model
    def _crm_ai_chat_completions_url(self, endpoint):
        endpoint = endpoint.rstrip('/')
        if endpoint.endswith('/chat/completions'):
            return endpoint
        return f'{endpoint}/chat/completions'

    def _crm_ai_prepare_chatter_comments(self):
        self.ensure_one()
        ...
        messages = self.env['mail.message'].search([
            ...
            ('body', '!=', False),
        ], order='id desc', limit=CRM_AI_LEAD_SCORING_CHATTER_LIMIT)

        comments = []
        for message in messages:
            body = tools.html2plaintext(message.body, include_references=False)
            body = re.sub(r'\s+', ' ', body or '').strip()
            if not body:
                continue
            if len(body) > CRM_AI_LEAD_SCORING_CHATTER_BODY_LIMIT:
                body = f'{body[:CRM_AI_LEAD_SCORING_CHATTER_BODY_LIMIT - 3]}...'
            comments.append({
                'posted_at': fields.Datetime.to_string(message.date) if message.date else '',
                'body': body,
            })
        return comments

    def _crm_ai_prepare_lead_scoring_payload(self, local_probability):
        self.ensure_one()
        return {
            'opportunity_name': self.name or '',
            'type': self.type,
            'stage': self.stage_id.display_name or '',
            'sales_team': self.team_id.display_name or '',
            'salesperson_assigned': bool(self.user_id),
            'expected_revenue': self.expected_revenue or 0.0,
            'recurring_revenue': self.recurring_revenue or 0.0,
            'recurring_plan': self.recurring_plan.display_name or '',
            'currency': self.company_currency.name or '',
            'priority': self.priority or '',
            'tags': self.tag_ids.mapped('name'),
            'source': self.source_id.display_name or '',
            'medium': self.medium_id.display_name or '',
            'campaign': self.campaign_id.display_name or '',
            'country': self.country_id.display_name or '',
            'state': self.state_id.display_name or '',
            'language': self.lang_id.display_name or '',
            'expected_closing_date': fields.Date.to_string(self.date_deadline) if self.date_deadline else '',
            'days_to_assign': self.day_open,
            'days_to_close': self.day_close,
            'has_customer': bool(self.partner_id),
            'has_company_name': bool(self.partner_name),
            'has_contact_name': bool(self.contact_name),
            'has_email': bool(self.email_from),
            'email_quality': self.email_state or '',
            'has_phone': bool(self.phone),
            'phone_quality': self.phone_state or '',
            'activities_count': len(self.activity_ids),
            'meetings_count': len(self.calendar_event_ids),
            'recent_chatter_comments': self._crm_ai_prepare_chatter_comments(),
            'local_statistical_probability': local_probability,
        }

    @api.model
    def _crm_ai_normalize_reasons(self, reasons):
        ...
        for reason in reasons:
            if isinstance(reason, dict):
                reason = reason.get('reason') or reason.get('text') or reason.get('value') or ''
    ...
    @api.model
    def _crm_ai_parse_lead_scoring_response(self, content, provider_name):
        ...
        raw_content = re.sub(r'^```(?:json)?\s*|\s*```$', '', raw_content, flags=re.IGNORECASE | re.DOTALL).strip()
        ...
        probability = round(max(0.01, min(99.99, probability)), 2)

        positives = self._crm_ai_normalize_reasons(
            score_data.get('positives') or score_data.get('positive_reasons') or score_data.get('top_positives') or []
        )
        negatives = self._crm_ai_normalize_reasons(
            score_data.get('negatives') or score_data.get('negative_reasons') or score_data.get('top_negatives') or []
        )
        return {
            'ai_provider_name': provider_name,
            'low_3_data': [
                {'field': 'ai_reason', 'value': reason, 'color': False}
    ...
    def _crm_ai_request_lead_score(self, local_probability):
        self.ensure_one()
        config = self._crm_ai_get_lead_scoring_config()
        if not config:
            return None

        lead_payload = self._crm_ai_prepare_lead_scoring_payload(local_probability)
        request_payload = {
            'model': config['model'],
            'messages': [
                {
                    'role': 'system',
                    'content': (
                        'You are a CRM sales analyst. Score the chance that the opportunity will be won. '
                        'Use recent_chatter_comments as high-signal free-text buyer context. '
                        'Return only valid JSON with keys: probability, positives, negatives. '
                        'probability must be a number from 0 to 100. positives and negatives must each be '
                        'an array of up to 3 short business reasons.'
                    ),
                },
                {
                    'role': 'user',
                    'content': json.dumps(lead_payload, sort_keys=True, default=str),
                },
            ],
            'temperature': 0.1,
        }
        ...
        if config['provider'] == 'openrouter':
            ...
            headers['X-OpenRouter-Title'] = 'NWOS CRM'

        try:
            response = requests.post(
                self._crm_ai_chat_completions_url(config['endpoint']),
            ...
            return self._crm_ai_parse_lead_scoring_response(content, config['label'])
        except (IndexError, KeyError, TypeError, ValueError, requests.RequestException) as error:
            _logger.warning(
                'CRM AI lead scoring failed for provider %s and model %s: %s',
                config['provider'], config['model'], error,
            )
            return None
    ...
    def prepare_pls_tooltip_data(self):
        ...
        ai_tooltip_data = self._crm_ai_request_lead_score(tooltip_result['probability'])
        if ai_tooltip_data:
            tooltip_result.update(ai_tooltip_data)
        ...
        # that the local computation was not possible. Sample data will be used instead unless
        # an external AI provider returns a valid score.
        probability_values = {'automated_probability': tooltip_result['probability']}
        if self.is_automated_probability:
            probability_values['probability'] = tooltip_result['probability']
